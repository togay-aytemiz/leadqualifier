...
export async function updateBookingSettingsAction(
    ...
    const connectionSyncMode = typeof payload.google_write_through_enabled === 'boolean'
        ? (payload.google_write_through_enabled ? 'write_through' : 'busy_overlay')
        : null
...
export async function replaceAvailabilityRulesAction(
    rules: Array<Pick<BookingAvailabilityRule, 'day_of_week' | 'start_minute' | 'end_minute' | 'label' | 'active'>>
) {
    const { supabase, organizationId } = await requireOrganizationContext()
...
export async function updateServiceCatalogDurationsAction(
    ...
    const updates = await Promise.all(entries.map(async (entry) => {
        const normalizedDuration = typeof entry.durationMinutes === 'number' && Number.isFinite(entry.durationMinutes)
        ...
        const { data, error } = await supabase
            ...
            .update({
                duration_minutes: normalizedDuration && normalizedDuration > 0 ? normalizedDuration : null,
