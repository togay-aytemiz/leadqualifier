...
function exposesInternalSourceMechanics(value: string) {
  const normalized = normalizeForMatch(value)
  return [
    /\bfiyat alani\b/,
    /\btablo satiri\b/,
    /\braw (?:table|cell|row)\b/,
    /\bburslu\s*\/?\s*no\s*fee\b/,
    /\bretrieval\b/,
    /\bretrieved context\b/,
    /\bchunk\b/,
...
function isCandidateAnswerSafe(originalAnswer: string, candidateAnswer: string) {
  ...
  if (exposesInternalSourceMechanics(candidateAnswer)) return false
...
function composeAnswer(answer: string, engagementQuestion: string, shouldAddEngagement: boolean) {
  ...
  if (normalizeForMatch(trimmedAnswer).includes(normalizeForMatch(trimmedEngagement)))
...
For tabular facts, convert raw column or cell wording into natural user language. Use "ücret" and "kontenjan" naturally. When a burslu option has no amount, say that burslu kontenjanlarda eğitim ücreti alınmaz. Never output raw table symbols, slash labels, column labels, or source mechanics.
...
function fallbackResult(
  answer: string,
  model: string,
  usage: RagAnswerPolishUsage | null = null
): RagAnswerPolishResult {
  return {
    answer,
    usedPolish: false,
...
export async function polishGroundedRagAnswer(input: {
  ...
  if (!completion) {
    console.error(
      'RAG answer polish failed after retry; using original extractive answer',
      lastError
    )
  ...
  return {
    answer,
    usedPolish:
      normalizeForMatch(payload.answer) !== normalizeForMatch(originalAnswer) ||
      shouldAddEngagement,
