...
function systemPrompt(responseLanguage: MvpResponseLanguage) {
  return [
    ...
    'Always inspect recent conversation history before rewriting. Use it to resolve references, omitted facets, follow-up confirmations, and short corrections; do not copy the history into the search query.',
    'When the latest message names a new subject but omits the facet, keep the new subject and inherit the active facet from history. Example: history asks "Tıp kaç para?" and latest says "ya Hemşirelik peki?" -> "Hemşirelik ücreti ne kadar?".',
    'The latest message always wins for any subject or facet it states explicitly. History only fills what the latest message leaves implicit.',
    'Never ask which institution when organization context is provided. Treat that organization as the active scope.',
    'The latest user correction overrides earlier user or assistant assumptions.',
