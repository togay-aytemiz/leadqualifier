...
const SETTINGS_SHELL_PATH = path.join(
  process.cwd(),
  'src/components/settings/SettingsResponsiveShell.tsx'
)
const SETTINGS_LOADING_PATH = path.join(
  process.cwd(),
  'src/app/[locale]/(dashboard)/settings/loading.tsx'
)
const SETTINGS_ORGANIZATION_PAGE_PATH = path.join(
  process.cwd(),
  'src/app/[locale]/(dashboard)/settings/organization/page.tsx'
)
const SETTINGS_ORGANIZATION_CLIENT_PATH = path.join(
  process.cwd(),
  'src/app/[locale]/(dashboard)/settings/organization/OrganizationSettingsClient.tsx'
)
const SETTINGS_BILLING_PAGE_PATH = path.join(
  process.cwd(),
  'src/app/[locale]/(dashboard)/settings/billing/page.tsx'
)
const SETTINGS_PLANS_PAGE_PATH = path.join(
  process.cwd(),
  'src/app/[locale]/(dashboard)/settings/plans/page.tsx'
)
const SETTINGS_PLAN_MANAGER_PATH = path.join(
  process.cwd(),
  'src/app/[locale]/(dashboard)/settings/plans/SubscriptionPlanManager.tsx'
)
const SETTINGS_PLAN_CATALOG_PATH = path.join(
  process.cwd(),
  'src/app/[locale]/(dashboard)/settings/plans/SubscriptionPlanCatalog.tsx'
)
const SETTINGS_TOPUP_CARD_PATH = path.join(
  process.cwd(),
...
const CALENDAR_PAGE_PATH = path.join(
  process.cwd(),
...
const KNOWLEDGE_PAGE_PATH = path.join(
  process.cwd(),
...
describe('navigation performance source guards', () => {
  it('keeps primary nav routes warm while preserving configurable shell prefetch control', () => {
    ...
    expect(settingsShellSource).not.toMatch(
      /href=\{item\.href\}\s+prefetch\s+active=\{item\.active\}/
    )
  ...
  it('keeps unread checks lightweight and desktop tab title sync tied to shared shell state', () => {
    ...
    expect(mainSidebarSource).toContain('listenForInboxUnreadState')
    expect(tabTitleSyncSource).toContain('listenForInboxUnreadState')
    expect(tabTitleSyncSource).toContain("if (routeId !== 'inbox' || !organizationId) return")
  ...
  it('lazy-loads low-frequency settings detail trees and checkout dialogs', () => {
    ...
    expect(organizationClientSource).toContain(
      "dynamic(() => import('@/components/settings/OfferingProfileSection')"
    )
    expect(organizationClientSource).toContain(
      "dynamic(() => import('@/components/settings/ServiceCatalogSection')"
    )
    expect(organizationClientSource).toContain(
      "dynamic(() => import('@/components/settings/RequiredIntakeFieldsSection')"
    )
  ...
  it('keeps dashboard layout on the slim org-context path during initial render', () => {
    ...
    expect(layoutSource).not.toContain(
      'resolveActiveOrganizationContext(undefined, { includeAccessibleOrganizations: true })'
    )
  })
