...
export function DropdownMenu({
  ...
  const setOpen = (nextOpen: boolean) => {
    setIsOpen((currentOpen) => {
      if (!shouldNotifyDropdownOpenChange(currentOpen, nextOpen)) {
        return currentOpen
      }
  ...
  return (
    <DropdownContext.Provider value={{ isOpen, setIsOpen: setOpen, toggle }}>
      <div
        className={cn('relative text-left', fullWidth ? 'block w-full' : 'inline-block')}
...
export function DropdownMenuItem({
  ...
  return (
    <button className={baseClassName} onClick={handleClick}>
      {children}
    </button>
  )
}
