...
describe('ChannelsList source guards', () => {
    it('memoizes derived cards before prefetching route targets', () => {
        const source = fs.readFileSync(CHANNELS_LIST_PATH, 'utf8')

        expect(source).toContain('useMemo')
        expect(source).toContain('const channelCards = useMemo(() => getChannelCardConfigs(channels), [channels])')
        expect(source).toContain('const channelPrefetchHrefs = useMemo(')
        expect(source).toContain('}, [channelPrefetchHrefs, router])')
    })
})
