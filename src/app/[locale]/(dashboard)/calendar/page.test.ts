...
describe('calendar page source', () => {
  ...
  it('shows a first-visit calendar intro modal scoped by user and organization', () => {
    const pageSource = fs.readFileSync(CALENDAR_PAGE_PATH, 'utf8')
    const clientSource = fs.readFileSync(CALENDAR_CLIENT_PATH, 'utf8')

    expect(pageSource).toContain('userId={orgContext.userId}')
    expect(clientSource).toContain('WorkspaceIntroModal')
    expect(clientSource).toContain('storageScope="calendar"')
    expect(clientSource).toContain("useTranslations('calendar.introModal')")
  })
  ...
  it('opens booking details in the shared modal across day, week, month, and agenda views', () => {
    ...
    const agendaViewSource =
      source
        .split('const renderAgendaView = () => {')[1]
        ?.split('const renderDayView = () => {')[0] ?? ''
    const dayViewSource =
      source
        .split('const renderDayView = () => {')[1]
        ?.split('const renderWeekView = () => {')[0] ?? ''
    const weekViewSource =
      source
        .split('const renderWeekView = () => {')[1]
        ?.split('const renderMonthView = () => {')[0] ?? ''
