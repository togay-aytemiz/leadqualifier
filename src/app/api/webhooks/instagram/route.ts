...
type InstagramWebhookSupabase = SupabaseClient
...
function summarizeInstagramPayloadShape(payload: unknown) {
  ...
  const entryShapes = payload.entry.slice(0, 3).map((entry) => {
...
async function findActiveInstagramChannelByAccountId(
  supabase: InstagramWebhookSupabase,
...
async function reconcileChannelByInstagramAccountId(
  ...
  for (const row of channels ?? []) {
    ...
    try {
      ...
      await supabase.from('channels').update({ config: nextConfig }).eq('id', channel.id)
      ...
      console.warn(
        'Instagram Webhook: Channel reconciliation failed for channel',
        channel.id,
        error
      )
    }
...
async function ensureInstagramConversationForContact(
  ...
  const fallbackNames = [contactIdentity.username, contactIdentity.contactName]
    .map((value) => readTrimmedString(value))
    .filter(
      (value, index, array): value is string => Boolean(value) && array.indexOf(value) === index
    )

  const conversation = await findInstagramConversationByContactId(
    supabase,
    organizationId,
    contactId
  )
  ...
  if (createConversationError) {
    ...
    console.error(
      'Instagram Webhook: Failed to create conversation for outbound echo',
...
async function persistInstagramExternalOutboundMessage(params: {
  ...
  const { error: insertError } = await params.supabase.from('messages').insert({
    id: uuidv4(),
  ...
  const { error: updateConversationError } = await params.supabase
    ...
    .update({
      contact_name:
        params.contactIdentity.contactName || conversation.contact_name || params.contactId,
      contact_avatar_url:
        params.contactIdentity.avatarUrl || conversation.contact_avatar_url || null,
  ...
  if (updateConversationError) {
    console.error(
      'Instagram Webhook: Failed to update conversation after external outbound message',
...
export async function POST(req: NextRequest) {
  ...
  if (events.length === 0) {
    if (debugEnabled) {
      console.info(
        'Instagram Webhook: No extractable events',
        summarizeInstagramPayloadShape(payload)
      )
    }
  ...
  for (const event of events) {
    ...
    if (!channel) {
      ...
      if (!resolvedChannel) {
        console.warn(
          'Instagram Webhook: Channel not found for business account id',
          event.instagramBusinessAccountId
        )
        continue
      }
