-- Allow service-role API callers to run the manual recurring renewal helper
-- even when PostgREST does not expose request.jwt.claim.role in the function
-- settings. Authenticated users still go through the org membership check.
...
BEGIN
    ...
    role_claim := COALESCE(NULLIF(current_setting('request.jwt.claim.role', true), ''), auth.role()::TEXT, '');
