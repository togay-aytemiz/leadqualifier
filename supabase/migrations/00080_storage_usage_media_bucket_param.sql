-- Make organization storage usage aggregation media-bucket aware.
-- This keeps usage totals correct when WHATSAPP_MEDIA_BUCKET differs by environment.

DROP FUNCTION IF EXISTS public.get_organization_storage_usage(UUID[]);
DROP FUNCTION IF EXISTS public.get_organization_storage_usage(UUID[], TEXT[]);

CREATE OR REPLACE FUNCTION public.get_organization_storage_usage(
    target_organization_ids UUID[] DEFAULT NULL,
    target_media_bucket_ids TEXT[] DEFAULT NULL
)
RETURNS TABLE (
...
DECLARE
    allowed_ids UUID[];
    media_bucket_ids TEXT[];
BEGIN
    IF auth.uid() IS NULL THEN
        RAISE EXCEPTION 'Not authenticated';
    END IF;

    IF target_media_bucket_ids IS NULL OR array_length(target_media_bucket_ids, 1) IS NULL THEN
        media_bucket_ids := ARRAY['whatsapp-media']::TEXT[];
    ELSE
        SELECT COALESCE(array_agg(DISTINCT bucket_name), ARRAY[]::TEXT[])
        INTO media_bucket_ids
        FROM (
            SELECT NULLIF(TRIM(bucket_name_raw), '') AS bucket_name
            FROM unnest(target_media_bucket_ids) AS bucket_name_raw
        ) cleaned
        WHERE bucket_name IS NOT NULL;

        IF media_bucket_ids IS NULL OR array_length(media_bucket_ids, 1) IS NULL THEN
            media_bucket_ids := ARRAY['whatsapp-media']::TEXT[];
        END IF;
    END IF;
        ...
        SELECT
            ...
            COALESCE(SUM(
                CASE
                    ...
                    WHEN (objects.metadata->>'contentLength') ~ '^[0-9]+$' THEN (objects.metadata->>'contentLength')::BIGINT
                    ELSE 0
                END
            ), 0)::BIGINT AS whatsapp_media_bytes
        FROM target_orgs
        INNER JOIN storage.objects
            ON objects.bucket_id = ANY(media_bucket_ids)
...
GRANT EXECUTE ON FUNCTION public.get_organization_storage_usage(UUID[], TEXT[]) TO authenticated;
