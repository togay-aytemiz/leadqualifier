...
> **Last Updated:** 2026-05-26 (Yüksek İhtisas demo RAG now has funded retest capacity, zero broken URL text findings in the current corpus, URL line-break normalization for future chunking, high-signal keyword extraction for natural conversational questions, original-query-first retrieval-only LLM query planning with a bounded fail-fast planner timeout plus strong-evidence short-circuit for planned variants, query-intent scoped focused evidence probes, immediate pending Demo Chat POSTs, direct extractive address/location answers, Supabase trigram/scoped indexes for lexical fallback speed, recoverable vector-timeout warnings, topic-related RAG engagement guidance with generic closing-tail cleanup, public-demo canary/source-trace tooling, 33-question live screenshot-plus-variant QA coverage, and evidence-first answer repair for eligibility, unit contact, and factual policy answers.)
