...
> **Last Updated:** 2026-05-28 (Public Demo Chat and shared extractive RAG now have source-grounded LLM polish for warmer wording and KB-validated engagement follow-ups, fallback protection when factual values are dropped, compound-question retrieval that avoids partial answers and full-compound focused-search timeouts, abortable polish calls, and demo UI source labels that render one or multiple canonical source URLs as short localized links while preserving raw stored source text.)
