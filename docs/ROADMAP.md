...
> **Implementation Update (2026-06-13):** YIU Public Demo non-Skill replies now run through the simple standalone-query pipeline. The active path performs one history/state-aware query rewrite, sends only the standalone query to direct OpenAI Vector Store Search (`rewrite_query:false`, `ranker:auto`, default `score_threshold:0.1`, default `max_num_results:12`), generates one grounded answer, and applies only mechanical chunk-id/protected-value validation. Skills remain unchanged. Pipeline errors terminate with an approved-source no-info response instead of falling through to the legacy extractive/shared RAG path.
...
> **Last Updated:** 2026-06-13 (YİÜ Public Demo non-Skill replies now use one standalone-query rewrite, one direct OpenAI Vector Store Search request, one grounded answer call, and a mechanical guard; Skills remain unchanged and legacy RAG is not used as an error fallback.)
