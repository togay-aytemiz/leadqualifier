...
> **Update Note (2026-03-30):** WhatsApp `Disconnect` is no longer treated as a silent local-only delete. For Cloud API numbers, the app now requires a real provider-side phone deregistration before removing the local channel record; if Meta reports the official coexistence limitation, operators are explicitly sent to WhatsApp Business app `Settings > Account > Business Platform` to disconnect Cloud API there first.
...
> **Last Updated:** 2026-03-30 (Inbox browser-tab unread indicator now updates from live inbox unread state snapshots while the operator is already inside Inbox, so new inbound messages can light the tab without waiting for refresh or delayed unread queries; Inbox realtime now self-recovers after long idle tabs by rebuilding Supabase subscriptions and reconciling the first conversation page plus selected thread on browser resume events and broken channel states, so queued messages surface without manual refresh; Instagram deleted-message handling now suppresses deleted-only Inbox noise end to end so a first/only DM that is later deleted does not leave a visible thread, established conversations may still show the deleted state without being treated like a fresh unread inbound turn, and stale deleted-only Instagram rows no longer leak into list counts or Inbox/browser unread indicators; unresolved IG identities still use a localized generic contact fallback instead of raw numeric ids; Instagram Inbox reaction events now persist structured reaction metadata, parse legacy raw `[Instagram reaction] ...` rows, and render as compact reaction rows with target-message-aware wording when the reacted provider message id resolves to a known outbound turn; lead extraction now emits a sector-agnostic semantic `intent_stage` (`none`, `informational_commercial`, `qualification`, `booking_ready`) so first-message commercial inquiries can rise to `warm` without relying on static language-specific pricing keywords, while `hot` still requires stronger qualification evidence; lead extraction also recovers the canonical catalog service from current conversation plus summary evidence when raw `service_type/services` are empty, required-intake repair scans recent customer turns for category-compatible answers including relative timeline phrasing such as `1 ay içinde`, and lead scoring/status are recalibrated from merged extraction evidence so a later `Teşekkürler`-style acknowledgement cannot overwrite previously captured date/intent signals with `cold/0`; `Settings > Applications` now keeps Google Calendar as a passive coming-soon surface with localized Turkish placeholder copy (`Google Takvim` + `Yakında`) and no clickable connect CTA yet; `Settings > Calendar` and `Settings > Applications` now disable their save CTA until the active tab/surface is actually dirty; `Settings > Plans` now keeps the one-time extra-credit purchase as the primary expansion action by moving the custom-package contact card directly beneath it and softening that CTA to a quieter secondary treatment; Supabase migration version prefixes are globally unique again; the duplicate March 27 premium debit-order rollout now ships only through `00102_reassert_premium_usage_debits_topup_first.sql`, avoiding remote `schema_migrations` collisions on `00099`; `Settings > Plans` extra-credit balance now keeps the original trial limit visible when premium extra credits come from trial carryover, so a remaining balance like `153.5` renders against `200` instead of collapsing to `153.5 / 153.5`; settings-to-settings navigation now keeps the shell visible by skipping the dashboard-wide fullscreen pending overlay on settings routes, using a detail-pane loading skeleton, and hydrating billing-lock nav state on the client so the settings layout no longer waits on billing snapshot reads; heavy settings detail routes now stream behind local `Suspense` boundaries so `Settings > Billing`, `Settings > Plans`, and `Settings > Organization` can show page chrome before the slowest server payload resolves, and `Settings > Plans` no longer blocks GET renders on `billing_region` persistence because that write now runs via `after()`; settings bundle trimming now lazy-loads low-frequency UI trees so the `Organization` general tab does not ship the organization-details sections up front and plan/top-up checkout flows defer legal-consent modal code until open; top-level workspace routes now lazy-load the heavy `Inbox`, `Calendar`, `Leads`, `Skills`, and `Knowledge` client containers behind route-scoped dynamic boundaries, and `/inbox` no longer waits for the first selected-thread payload before the shell can paint; desktop dashboard typography now uses a slightly smaller shared content scale across operator surfaces while the desktop sidebar is compensated one step upward so navigation stays balanced; desktop main sidebar now uses a lighter Linear-style section IA with collapsible group headers, more breathing room between categories, softer active pills, and slightly smaller desktop nav typography; `Settings > Plans` premium status now hides redundant total/extra-credit cards when extra credits are exhausted so the surface falls back to the monthly package card only, while premium debit priority remains `extra credits first, then monthly package`; Inbox list header filter action and selected-thread read/unread toggle now use plain icon chrome instead of bordered pill buttons, Inbox template picker now keeps first-load empty panes in a dedicated loading state with skeleton feedback until template fetches resolve, Inbox selected-thread header now has a manual read/unread toggle that preserves manually unread threads until manual re-read or a later revisit, React state-sync anti-patterns were removed from the owned modal components with regression tests added, cross-surface best-practices hardening now covers semantic Inbox/Knowledge/Skills interactions, effect-safe state handling in shared primitives plus calendar/settings/mobile-nav/auth flows, stable prefetch derivations, and dedicated WhatsApp/Telegram/Instagram channel webhook lookup indexes, Inbox bot labels now keep the generic assistant name when the stored `bot_name` is still the default assigned value, Inbox mobile composer now keeps template/send actions icon-only on phone widths so the reply field holds the remaining width, and Calendar / scheduling / booking now ships with a full-width `/calendar` workspace, `Tercihler > Takvim` for booking rules, `Entegrasyonlar > Uygulamalar` for Google Calendar management, compact business-hour rows, inline timing help, simplified Google connection cards, a single calendar-settings entry in the workspace header, knowledge-aligned calendar header action sizing with a dark `Yeni randevu` CTA, modal-localized manual booking validation for past-date and slot-availability failures, client-side buffered calendar navigation for faster day/week/month switching, mutation-time cache invalidation, AI scheduling continuity across follow-up availability questions, stricter booking-intent gating, deterministic human handoff on scheduling failures, minimum-notice enforcement across slot generation + exact-slot checks + booking writes, safe Google disconnect cleanup for mirrored future events, per-booking duration override and customer-email capture in the booking modal, shared select-field visual cleanup, full-width `Settings > Channels` gallery usage, Turkish-first copy cleanup, Instagram webhook/page-id hardening with pending-vs-verified connection state, consistent readiness copy across channel cards and manage pages, Instagram Login webhook provisioning during connect, direct-Instagram outbound DM sync into Inbox with business identity + avatar hydration, persisted Instagram share/story preview URLs for inline Inbox rendering when Meta includes a previewable asset link, limited unsupported-payload debug snapshots for Instagram attachment diagnosis, unsupported Instagram attachment fallback copy, image-aware inline Inbox preview fallback for image-like inbound file messages, caption/text-aware lead extraction for media-backed inbound turns, direction-aware outbound media preview labels, stricter Instagram request-badge classification, hidden Instagram secondary ids in details, aligned billing plan CTA rows, conservative generic required-intake fallback matching plus semantic custom-field resolution for Inbox/Leads, inbox refresh latency hardening with parallel Instagram contact hydration + duplicate lead-fetch removal on thread open + targeted inbox query indexes, title-row Inbox list filtering for unread + customer-score views with reset support, colored score chips, read-on-leave behavior, and paginated Inbox filter backfill before empty-state rendering, direct post-auth landing, app-shell latency hardening via deferred sidebar hydration plus targeted prefetch rebalancing, non-blocking Settings shell pending-suggestion hydration with stale-response guards, lighter shell refresh IO via unread existence checks, desktop-only sidebar subscriptions, settings-link prefetch reduction, route-scoped dashboard i18n payloads, self-healing inbox sidebar preview hydration for missed realtime message inserts, deploy-time chunk self-recovery, dashboard-wide intent-based route warming with immediate skeleton bridging, optimistic sidebar/mobile/settings active-state feedback, lightweight post-auth route resolution without admin org validation, a root dashboard loading boundary, slim active-org dashboard refreshes, non-blocking inbox thread-open hydration with parallel message/lead fetch, server-bootstrapped Inbox thread payload caching, client-cached Leads sort/search/page interactions, pure-read Skills list loading for non-empty orgs, and aggregate Knowledge collection counts via RPC.)
...
- [x] **Channels Discovery UX**
  ...
  - [x] Make WhatsApp disconnect provider-aware: deregister Cloud API before local channel deletion, and block coexistence disconnects with explicit WhatsApp Business app `Settings > Account > Business Platform` guidance instead of silently removing only the local record
