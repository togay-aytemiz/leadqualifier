...
- [x] Add 20 realistic prospective-student clarification fixtures with slot-only replies and same-session live execution.
- [x] Separate File Search pipeline failures from factual no-info in evaluation reports.
- [ ] Restore OpenAI API quota and rerun the 79 invalid routing rows on the same fixed selection.
- [x] Add an explicit router clarification response contract without changing the one-step File Search `answer | no_info | refuse` schema; `needsClarification` now returns early and persists pending slots.
- [x] Skip raw exact matching on the slot-only reply to an active router clarification, then bind it to the original subject/facet through the history-aware rewriter.
- [x] Compare same-seed OpenAI spend by stage and adopt a 20-case smoke gate before full 100-question evaluations (`docs/evaluations/yiu-openai-cost-comparison-2026-06-20.md`).
- [ ] Rerun and manually review all 20 two-turn clarification conversations after the contract is implemented.

> **Implementation and Evaluation Update (2026-06-20):** Public Demo RAG fallback now uses one GPT-5.5 Responses API call with hosted File Search and strict `answer | no_info | refuse` output instead of custom vector search, GPT-4.1-mini answer generation, and a second GPT-4.1-mini evidence verifier. The balanced 20-case live gate passed after manual boundary review: `10/10` directly supported answers remained answerable, all `10/10` exact-unknown/bounded cases avoided unsafe positive claims, and runtime errors were zero. Medium reasoning was required to preserve exact institutional relationships; final p50/p90 were `10.21s/12.68s`, so the same-seed 100 is the next latency/generalization gate (`docs/evaluations/yiu-one-step-file-search-focused-review-2026-06-20.md`).

- [x] Replace the custom multi-stage fallback with one GPT-5.5 Responses + hosted File Search call.
- [x] Preserve File Search results, citations, usage, tenant context, dictionary context, and prepared standalone queries in the one-step path.
- [x] Pass a balanced `10` supported + `10` bounded/unsupported focused live gate without a second verifier or new threshold.
- [x] Rerun the exact same ordered 100-question seed and manually review RAG correctness plus p90/token usage before a disjoint sample.
- [x] Increase the one-step response budget from `800` to `2000`; the previously failing broad success-ranking turn emitted valid structured answers in `2/2` production-build reruns.
- [ ] Close the two remaining adjacent-evidence positives for registration-procurement and current training-hospital identity without a second verifier or question-specific runtime rule.
- [ ] Rerun the focused three-case red set before a disjoint random 100.

> **Same-Seed Evaluation Update (2026-06-20):** The one-step path completed the same ordered `100/100` with zero transport errors and `35/35` manually acceptable Skill routes. Compared with the previous path, known false positives for Psychology existence, meal pricing, affiliated-hospital city/emergency service, Tıbbi Görüntüleme fee substitution, and hospital-project existence were removed or explicitly bounded. The gate is not yet passed: registration pricing still inferred a no-negotiation rule from a purchasing regulation, a regulation-defined health center was presented as the answer to the current training-hospital question, one request reproducibly exhausted all `800` output tokens on reasoning before JSON, selector timeouts rose from `2` to `3`, and p90 increased from `15.26s` to `28.95s`. Full review: `docs/evaluations/yiu-one-step-file-search-same-seed-100-review-2026-06-20.md`.

> **Output-Budget Follow-up (2026-06-20):** `max_output_tokens` was raised from `800` to `2000` after the broad `Başarı sıralamaları nedir?` turn reproducibly spent all `800` tokens on reasoning and emitted no JSON. A configuration regression test failed before the code change and passed afterward; two fresh production-build reruns both returned valid structured answers. The case remains a latency/cost outlier at `53.03s`/`57.64s` and `29,320`/`56,834` File Search-stage tokens, so the output failure is closed while broad-query performance remains open.
...
> **Evaluation Update (2026-06-19):** The YİÜ same-seed random 100 was rerun against production after reopening the demo by setting `demo_chat_channels.maintenance_enabled = false` for `yiu-tanitim-gunleri-2026`. The valid run completed `100/100` with zero errors and route counts of `38` Skill answers, `29` grounded RAG answers, `22` no-info replies, `7` clarifications, `2` direct answers, `1` assistant identity answer, and `1` refusal. Manual Codex review scored the answers `7.14/10` (`52` strong, `28` usable, `13` weak, `7` bad). Compared with the previous same seed, the latest fixes closed several red cases (`Antrenörlük Eğitimi ücretli mi?`, `1 tercih indirimi`, `Tıbbi Görüntüleme Teknikleri ücreti nedir?`, and `Kampüste revir var mı?`), but quality is still held back by a false no-info on `Ebelik kontenjanı`, broad questions being matched to one-program Skills, and RAG over-inference on hospital/accreditation/career questions (`docs/evaluations/yiu-routing-random-100-codex-review-2026-06-19T06-32-12-885Z.md`).

- [x] Reopen the production YİÜ demo by disabling DB-backed Public Demo maintenance for `yiu-tanitim-gunleri-2026`.
- [x] Rerun the same-seed YİÜ random 100 against production after reopening the demo.
- [x] Independently review answer correctness, no-info validity, Skill misses, and RAG quality for the rerun.
- [ ] Persist Skill-miss diagnostics on fallback/RAG bot messages so missed Skill candidates and verifier decisions are visible in eval reports.
- [x] Tighten Skill verifier broad-subject coverage so one-program Skills cannot answer all-program ranking/campus/fact questions.
- [ ] Prevent non-Skill RAG from answering broad career/practice/accreditation/hospital questions by locking onto a single unrequested program or stale operational source.

> **Implementation Update (2026-06-19):** YİÜ program Skills now generate broad natural phrase coverage for each canonical brochure program, including `ücretli mi`, `kaç para`, `fiyatı nedir`, `kontenjan bilgisi`, `puanı kaç`, `sıralaması nedir`, `hangi fakültede`, `nerede okunuyor`, `burslu var mı`, and `%50 indirimli var mı`. The SHMYO campus Skill was corrected to the verified 2025 campus map, transport/address and registration Skills now answer more directly, and the broad student-life Skill no longer captures unsupported revir/spor-salonu questions. Simple RAG now rejects positive fee/quota/campus/existence claims for non-current program subjects even when stale pages mention them, while avoiding false no-info for academic-unit program lists. The production YİÜ Skills DB was republished and verified with `70` active Skills, `26` generated program Skills, and `2059/2059` active embedding rows.

- [x] Expand generated YİÜ program Skill phrases for natural fee/quota/campus/existence wording.
- [x] Correct stale SHMYO campus mappings and narrow broad student-life triggers.
- [x] Add a current-program catalog guard for stale/unsupported program facts in simple RAG.
- [x] Add transient retry to YİÜ Skill push embedding generation/insert operations.
- [x] Republish and verify the YİÜ production Skill pack with refreshed embeddings.
...
> **Evaluation Update (2026-06-18):** The same-seed YİÜ Public Demo random 100 was rerun after response-fact Skill embeddings and routing diagnostics. The run completed `100/100` with zero errors, route counts of `39` Skill answers, `34` grounded RAG answers, `13` no-info replies, `10` clarifications, `2` direct answers, `1` identity answer, and `1` refusal. Independent Codex review improved from `6.75/10` to `7.37/10`; strong/usable answers rose from `72` to `82`, while failed answers dropped from `17` to `9`. Remaining blockers are obvious Skill misses for short fact phrasing, stale/wrong SHMYO campus Skill content, and adjacent-program RAG fee generation for unsupported program questions (`docs/evaluations/yiu-routing-random-100-codex-review-2026-06-18T21-21-56-122Z.md`).
...
- [x] Rerun the same-seed YİÜ random 100 and independently review answer correctness/no-info validity after the recall changes.
...
- [x] Replace per-entry dictionary cards with one compact editable table, a single shared meanings hint, accessible active switches, and icon-only delete actions.
- [x] Expand the YİÜ dictionary from verified brochure program names, separate `FTR` from two-year `FZT`, and preserve tenant-added terms during seed updates.
...
- [x] Prevent Internal Agent activation from overriding matched Skill replies.
- [x] Refuse IBAN/card/payment-credential requests before RAG retrieval.
- [x] Clean up YİÜ Skill phrase overlap and push refreshed demo embeddings.
- [x] Run a same-seed 100-question YİÜ routing eval after the Skill/guard changes.

> **Evaluation Update (2026-06-17):** The latest same-seed YİÜ Public Demo random 100 routing run completed `100/100` with `0` errors and route counts of `34` Skill answers, `40` grounded RAG answers, `10` clarifications, `10` no-info replies, `5` refusals, and `1` assistant-identity reply. Average latency was `9.4s` with p50 `10.3s` and p90 `15.2s`. Manual Codex review found the Skill override fix materially improved answer quality for matched Skills such as `myo nerde`, while remaining risks are now concentrated in matcher misses for obvious Skill candidates and fallback RAG over-inference/source-mechanics (`docs/evaluations/yiu-routing-random-100-2026-06-17T20-45-35-256Z.md`).

> **Implementation Update (2026-06-17):** The first focused post-review YİÜ quality sprint tightened the simple RAG path without adding a new orchestration layer. Source-less `respond` rewrites now catch shorthand institutional fact questions such as `hastaneniz varmı` and force them through retrieval. Facility/equipment training-location claims now require direct support even when the user asks `nerede veriliyor`, so program-campus evidence alone cannot justify `cihaz eğitimi Balgat'ta` style answers. The YİÜ Skill Pack V2 was also expanded and pushed to the production demo Skills DB for weak shorthand/facet cases including `say bölümleri`, `ea var mı`, `baglum kampus`, `tıp hastane kaçıncı sınıf`, `burslu öğrenciler ücret ödüyor mu`, `laboratuvarda çalışmak istiyorum`, `yurt var mı`, and `kayıt tarihleri ne zaman`; `12` skills changed and `439` embedding rows were refreshed.

- [x] Ban source-less direct answers for shorthand institution facts in the simple Public Demo RAG path.
- [x] Add direct-evidence checks for facility/equipment training-location claims, not only `var mı` availability questions.
- [x] Expand YİÜ Skill shorthand phrases and repair weak matched-answer templates for wrong-facet replies.
...
- [x] Ban source-less direct answers for institution facts in the simple Public Demo RAG path.
- [x] Add direct-evidence checks for facility, service, equipment, location, office, and practice-location claims.
- [x] Expand YİÜ Skill shorthand phrases and repair weak matched-answer templates for wrong-facet replies.
...
- [x] Prevent source-less simple RAG `respond` answers for knowledge questions by forcing search or refusal.
- [x] Audit remaining YİÜ no-info answers against the active approved corpus after the random rerun.

> **Evaluation Update (2026-06-13):** Added a reproducible local Public Demo evaluation harness for YİÜ route attribution and affirmative follow-up behavior. The first approved run sampled 100 questions from the 508-question customer pool through the current local `main` demo API and recorded `67` Skill answers, `14` non-Skill RAG answers, `19` RAG no-info answers, and `0` errors. A second 50-question follow-up run sent acceptance variants after first-turn Skill answers and recorded `25` follow-up Skill answers, `5` follow-up RAG answers, and `20` cases where the first turn was not a Skill match. The results are captured in `docs/evaluations/yiu-routing-random-100-2026-06-13T18-49-01-425Z.md` and `docs/evaluations/yiu-followup-skill-random-50-2026-06-13T19-01-23-792Z.md`.

- [x] Add reproducible Public Demo route-attribution and Skill follow-up evaluation harness for the YİÜ 508-question pool.

> **Implementation Update (2026-06-13):** Public Demo Chat now keeps raw exact Skill triggers as the only zero-LLM fast path. Every non-exact turn runs through a compact `gpt-4.1-mini` intent rewrite that produces a standalone query plus subject, requested facet, and clarification state; recent history is used only to resolve references and accepted assistant offers. The standalone query retrieves up to three semantic Skill candidates, then a second compact verifier must select a supplied candidate whose subject and facet both match or explicitly return `NO_SKILL`. Only the verified candidate is passed into the shared inbound pipeline, preventing that pipeline from silently replacing the decision with a new top semantic match. `NO_SKILL`, rewrite failures, verification failures, and genuine ambiguity continue to the existing non-Skill RAG path.

- [x] Add history-aware follow-up Skill query rewriting before Public Demo Chat falls back to non-Skill RAG.
- [x] Add intent-aware top-three Skill candidate verification with an explicit `NO_SKILL` route and verified-candidate pipeline handoff.
- [x] Answer Public Demo assistant identity questions from configured bot identity before Skill/RAG retrieval so `Sen kimsin?`, `ChatGPT misin?`, or `gerçek insan mısın?` cannot drift into corpus-backed institution facts.
...
- [x] Design short affirmative follow-up rewriting that can resolve `evet/göster` style replies to the prior assistant suggestion before Skill matching.
...
> **Implementation Update (2026-06-13):** The simple YIU Public Demo non-Skill RAG path now includes two bounded best-practice safeguards on top of the standalone-query design. After direct OpenAI Vector Store Search, a small non-LLM retrieval guard removes chunks scoped to another university and removes international/YÖS/USD fee evidence for ordinary domestic fee questions. If the first retrieval has no usable evidence, or if grounded generation returns no-info or a cross-organization answer, the pipeline runs exactly one broader retry query with lower score threshold before abstaining. The answer guard also rejects speculative facility/equipment availability claims unless the selected chunks directly state availability or quantity. This keeps the path simple while preventing the observed wrong-organization, wrong-audience fee, and adjacent-facility inference failures.

> **Implementation Update (2026-06-13):** YIU Public Demo non-Skill replies now run through the simple standalone-query pipeline. The production corpus was rebuilt in persistent OpenAI vector store `vs_6a2cfa3598b88191ad8a77ab9a9c6a09` after the former seven-day store expired. The active path performs one organization/history/state-aware query rewrite, sends only the standalone query to direct OpenAI Vector Store Search (`rewrite_query:false`, `ranker:auto`, default `score_threshold:0`, default `max_num_results:20`), and uses `gpt-4.1-mini` for grounded answer generation plus mechanical chunk-id/protected-value validation. Skills remain unchanged. Operational pipeline failures return an explicit temporary source-unavailable message instead of a false no-information claim or legacy-RAG fallback.

> **Design Decision (2026-06-13):** The approved next RAG iteration removes the optional layers from the YIU demo non-Skill path and adopts a simple standalone-query pipeline: explicit state plus recent history and the latest message feed one query rewriter; only the resulting standalone query is sent to direct OpenAI Vector Store Search; one grounded answer generator receives the top chunks plus state/history; a small non-LLM grounding guard validates chunk IDs and protected values. Multi-query expansion, retry, table fast paths, separate composer/polish, and runtime LLM judging are deferred until a measured evaluation proves they are needed. Retrieval uses `ranker:auto`, `score_threshold:0`, `max_num_results:20`, disables the provider's second query rewrite, and keeps vector-store expiration disabled for the production corpus (`docs/superpowers/specs/2026-06-13-simple-standalone-query-rag-design.md`).

- [x] Replace the YIU demo non-Skill path with the simple standalone-query Vector Store Search pipeline.
- [x] Remove polish, multi-query retry, table fast-path, and runtime judge calls from the active simple path.
- [x] Replace the expired YIU OpenAI vector store with a persistent 135-file production store and expose operational source failures separately from factual no-info.
- [x] Restore organization-scoped rewrites for campus, catalog, domestic admissions-table, and program-duration questions without adding organization-specific answer rules.
- [x] Add bounded retrieval filtering, one recall retry, cross-organization answer rejection, domestic-vs-international fee evidence filtering, and direct facility-availability evidence checks to the simple RAG path.
...
> **Update Note (2026-06-12):** The simplified YİÜ Public Demo non-Skill path now has two small quality guards instead of a new router stack: the LLM planner is instructed to ask one specific clarification for terse fragments that truly lack an entity/type/year, and retrieved markdown brochure tables are parsed before answer composition for concrete program metrics such as quota, rank, base score, point type, code, and price. This keeps Skills unchanged and keeps the main flow as LLM planner -> OpenAI File Search -> evidence/table validation -> polish.

- [x] Add targeted clarification guidance and generic brochure-table fact extraction to the LLM-first File Search pipeline.

> **Update Note (2026-06-12):** A production Public Demo Chat UI smoke executed a systematic 100-question selection across the 508-question YİÜ set using a fresh browser conversation per case. The strict `gpt-5.4-mini` judge averaged `7.11/10` (`53` at 8-10, `24` at 6-7, `23` at 1-5); browser rendering stayed healthy, but response latency averaged `21.3s` with p90 `40.2s` and one 120-second first-attempt timeout. The next generic quality work is now explicitly prioritized around overly broad no-info boundaries, entity/facet disambiguation, clinical/campus routing, and final source-mechanics suppression rather than organization-specific phrase patches (`docs/evaluations/yiu-live-ui-smoke-100-2026-06-12.md`).

> **Update Note (2026-06-12):** The YİÜ Public Demo non-Skill path now uses one LLM-first File Search pipeline: ordered conversation history plus the latest message are resolved by a `gpt-4.1-mini` planner into `search`, `clarify`, or `refuse`; search plans run one OpenAI File Search request; a compact grounded composer validates evidence ids and protected values; and the existing organization-aware polish layer is the only presentation step. Skills remain exact and unchanged. The previous demo-only strict router/catalog/evaluator/controller stack is bypassed for this path. Live smoke now answers the campus-address question and preserves the Turkish-versus-English Medicine ranking comparison after the user rejects an incorrect fee answer.

- [x] Replace the YİÜ Public Demo non-Skill reply path with the LLM-first File Search pipeline while preserving exact Skill replies.

> **Update Note (2026-06-12):** Direct table/catalog fact answers now have a customer-facing fallback renderer before any LLM polish runs. Table-derived answers use natural terms such as `ücret` and `kontenjan`, avoid exposing brochure/table/row/field mechanics, and still pass through the organization-aware final polish boundary when available. This is a platform presentation rule for every tenant/source path, not a YİÜ-only copy fix.

> **Update Note (2026-06-12):** Live follow-up smoke now treats pending clarification state as authoritative when the user supplies a concise slot value and the LLM unnecessarily asks to clarify again. This is tenant-independent: if the previous turn asked for a missing program/service/scope/entity and the latest message is a short non-off-topic value, the original metric is preserved and retrieval/catalog lookup proceeds. Language shorthand such as `ing` is also recognized as an English variant signal in table planning, strict normalization, direct catalog answers, and referential follow-ups, so turns like `taban puanlar nedir` -> `tıp ing ücretli` keep the taban-puan metric and select the English paid row.

> **Update Note (2026-06-12):** The activated Internal Agent Controller now fails open to the current validated provider when its own multi-round plan claims become inconsistent (`plan_claim_mismatch`). This keeps the controller from turning recoverable planner/repair instability into a customer-facing no-info boundary, while still recording activation diagnostics for QA. The current provider remains the trusted fallback for structurally invalid controller execution.

> **Update Note (2026-06-12):** The live follow-up smoke follow-on fixes now persist pending clarification metadata even when the activated controller, rather than the current provider, owns the clarification turn. Controller-owned clarification diagnostics are converted into reusable RAG pending state for the next turn, strict catalog refusal boundaries such as approved catalog absence can support controller claims instead of being replaced by generic no-info, and `%50 indirimli program var mı` style variant-existence questions can answer from the program fee catalog before retrieval. This closes the state-loss class behind `kaç para` -> program-name and `hazırlık var mı` -> program-name follow-ups.

> **Update Note (2026-06-12):** Live Public Demo Chat follow-up smoke after controller activation exposed two remaining platform-level gaps: ambiguous short follow-ups were resolving response language from the latest fragment only, and activated controller requests did not receive pending clarification state unless a typed state had already been persisted. Demo File Search now resolves final response language from recent user history when the latest turn is ambiguous, activation/shadow requests hydrate typed conversation state from pending clarification metadata before planner execution, and the planner contract repair now treats pending-state `no_info` as a repairable error instead of a valid final boundary. This is generic across tenant/channel activation paths rather than YİÜ-specific.

> **Update Note (2026-06-12):** Live Public Demo Chat smoke exposed two controller/orchestration gaps: safe structured fee facts could be hidden behind mistaken contextual clarify/refuse decisions, and short follow-ups such as `ingilizcesi?` could be treated as standalone translation/off-topic turns instead of preserving the previous subject and metric. The validated File Search and Internal Agent Controller path now repair short referential follow-ups from recent user/bot history, require real recent context before using referential state, let safe catalog facts override unnecessary contextual clarification/refusal, preserve language/variant hints for program fee answers, pass demo conversation/channel identity into the controller, avoid treating typed conversation state as factual claim support, and use a larger bounded activation latency budget for planner-plus-provider execution. Local production-like smoke now answers `tıp kaç para` and the follow-up `ingilizcesi?` with the correct English Medicine fee variant.

> **Update Note (2026-06-12):** Demo validated File Search presentation polish is now an unconditional final customer-facing boundary. Provider-level direct/table/catalog polish diagnostics are treated as upstream diagnostics only; the demo bridge still runs one organization-aware grounded polish pass before returning the browser reply, strips ambient cookie/forwarding headers from OpenAI polish requests to avoid `431 request headers too large` failures, and retries the polish request once before falling back to the original grounded answer.

> **Update Note (2026-06-12):** The Bounded Internal Agent Controller activation boundary is now implemented for the validated File Search provider, Simulator, and shared inbound text replies across Demo Chat, WhatsApp, Instagram, and Telegram. Activation is tenant-independent: the controller runs before/around customer-facing replies, owns clarify/refuse/no-info decisions, executes research through approved read-only internal bridge tools, and preserves the current provider as verified evidence/fail-open fallback for answer decisions. Behavior-aware presentation is part of the boundary for controller-produced clarification/refusal/no-info replies. Activation is enabled when `OPENAI_API_KEY` is available unless `INTERNAL_AGENT_ACTIVATION=0` disables it; `INTERNAL_AGENT_ACTIVATION_ORG_IDS` can restrict canaries. The latest synthetic gate remains `33/33` passed with `10/10` average, zero critical failures, zero shadow errors, and a `go` decision.

> **Update Note (2026-06-12):** Because no real customer conversations exist yet for the Bounded Internal Agent Controller rollout, activation readiness now has a synthetic customer-style shadow acceptance gate. The `agent:shadow:acceptance` runner exercises 33 direct fact, table fact, catalog, clinical/practical, valid follow-up, fresh-after-follow-up, off-topic, and unsafe cases against the internal planner. The generic planner review/repair layer now catches stale pending-state reuse, repeated clarification, off-topic clarification, missing document-evidence tools, and malformed/invalid planner JSON; if the LLM repair repeats a contract error, a bounded internal fallback routes the turn to refusal or approved internal evidence without using org-specific phrase patches. The latest run reached `33/33` passed with `10/10` average, zero critical failures, zero shadow errors, and a `go` decision. Report: `docs/evaluations/internal-agent-synthetic-shadow-acceptance-2026-06-12.md`.

> **Update Note (2026-06-11):** The Bounded Internal Agent Controller foundation slice is implemented in shadow mode. The platform now has provider-neutral agent contracts, an internal-only tool registry, typed evidence graph, generic planner, bounded controller, runtime shadow request builder, validated File Search shadow diagnostics, shared inbound/simulator shadow hooks, and an agent shadow report command. The slice remains observation-only: current Skills/RAG/catalog/fallback/polish paths still own customer answers and billing-visible usage until a separate activation gate passes. Verification report: docs/evaluations/internal-agent-shadow-foundation-2026-06-11.md.

> **Update Note (2026-06-11):** The next platform-level RAG phase is approved as a Bounded Internal Agent Controller shared by every customer-facing channel and response path. The design introduces provider-neutral agent/tool contracts, atomic claim decomposition, a registered internal tool catalog, a typed evidence graph, claim-level verification, bounded retry/clarify/refuse decisions, source authority/freshness handling, and one behavior-aware presentation boundary. It reuses existing Skills, catalogs, tables, hybrid retrieval, approved-corpus File Search, typed state, policy compiler, ledgers, and polish; excludes live external web research; and rolls out through shadow comparison before channel activation (`docs/superpowers/specs/2026-06-11-bounded-internal-agent-controller-design.md`).

> **Implementation Plan (2026-06-11):** The first executable slice is split into provider-neutral contracts, an internal-only tool registry, typed evidence graph, generic LLM planner, bounded controller loop, and fail-open shadow diagnostics across validated File Search/shared customer reply paths. Existing answers retain ownership during this slice; a measured 30-case answer-parity/tool-agreement gate must pass before a separate activation plan gives the controller real tool and reply ownership (`docs/superpowers/plans/2026-06-11-bounded-internal-agent-controller-foundation-plan.md`).

> **Update Note (2026-06-11):** Customer-preview RAG now has a platform-level Behavior Policy Compiler, Universal Claim Ledger, and Typed Conversation State. Organization AI behavior text is compiled into structured policy diagnostics (`businessScopeHints`, `outOfScopeHints`, `evidenceRequiredFor`, `sourcePriority`, `refusalClasses`, `tone`) and injected into the contextual orchestrator prompt. Strict validation now merges a policy-driven universal claim ledger with the existing strict ledger, so tenant-required evidence categories such as pricing, payments, dates, contacts, credentials, and clinical/staj facts can block unsupported values, contact details, IBANs, and policy markers across domains. Provider diagnostics also emit typed conversation state (`fresh`, `pending_clarification`, `resolved_from_pending`, `split_pending`, `ignored_pending`) with active intent, requested metric, missing slots, source preference, original question, and latest clarification for future turns.

> **Update Note (2026-06-11):** The production YİÜ Tanıtım Günleri demo organization (`YİÜ Tanıtım Test` / `yiu-tanitim-test`) now has its structured AI behavior fields seeded for the admissions assistant. The settings define the represented institution and in-scope topics, require brochure-first then website/PDF-backed answers for factual claims, mark unrelated/general/safety-sensitive requests as out of scope or refusal, and instruct the final user-facing tone to stay warm, concise, student-friendly, and free of internal source mechanics.

> **Update Note (2026-06-11):** Validated File Search now runs the contextual orchestrator as a global intake gate for Public Demo Chat, including first-turn messages with no history. The gate can refuse off-domain/unsafe/impossible requests before retrieval, ask slot-specific clarifications for under-specified questions, and preserve richer diagnostics (`route`, `domain_relevance`, `missing_slots`, `safety_class`, `answer_policy`). The bounded LLM research planner can also return zero-hop boundary plans (`off_topic_boundary`, `safety_refusal`, `impossible_boundary`, `no_retrieval`) so general advice requests do not become File Search hops. Pending-state repair now rejects unrelated recipe/market/tax-style follow-ups even when an LLM mistakenly says `use`, and strict clarification fallbacks cover bare `puan kaç` / `kaç gün` prompts. A focused 12-case adversarial rerun moved the subset average to `8.5/10` with `11/12` cases at `8-9` and no `1-5` scores.

> **Update Note (2026-06-11):** A 200-case broad student/adversarial validated File Search smoke was run across student-style questions, nonsense/impossible requests, under-specified prompts, trolling/safety attempts, off-topic requests, brochure/PDF questions, relevant follow-ups, and irrelevant follow-ups. The run averaged `6.66/10` with `90/200` cases at `8-10`, and identified the next global quality priorities: domain/off-topic boundary before retrieval, stronger under-specified slot clarification, stale-state ignore for unrelated follow-ups, final source-mechanics polish coverage, and safer prompt/guarantee/tone refusals. Report: `docs/evaluations/yiu-broad-student-adversarial-smoke-2026-06-11.md`.

> **Update Note (2026-06-11):** Public Demo Chat mobile composer sizing is now pinned to an explicit `16px` textarea font size instead of relying on the theme `text-base` token. This keeps iOS Safari from auto-zooming when visitors focus the message input, and the source guard now checks the explicit mobile-safe class so future theme-scale changes cannot silently reintroduce the zoom behavior.

> **Update Note (2026-06-11):** Customer-facing demo File Search replies now pass through the existing grounded RAG polish layer before they are returned to the browser, regardless of whether the answer came from retrieval, a table fact, strict catalog fast path, guardrail/refusal, or clarification flow. Provider-level direct/table/catalog paths also expose presentation-polish diagnostics when they polish early, but the demo bridge still runs one final polish pass using the organization's AI behavior settings (`prompt` and `bot_name`), then reattaches traceable source URL lines. This makes source mechanics such as brochure/table/row/field wording internal by default while still preserving citation traceability and exact factual values.

> **Update Note (2026-06-11):** Pending clarification handling now uses an explicit LLM state decision instead of relying on conservative question-shape heuristics. When pending clarification state is present, the contextual orchestrator must emit `state_decision` (`use`, `ignore`, `split`, or `clarify`), `state_confidence`, `state_reason`, and `consumed_pending_state`. `use` consumes short or long student-style clarification answers even when they are typo-heavy or phrased like a question; `ignore` lets fresh independent questions bypass stale state; `split` rewrites messages that both answer the pending clarification and ask a new related facet; and `clarify` asks one more short question when the follow-up is still too vague. Deterministic pending-state repair is now the fallback/guard around the LLM decision rather than the primary decision-maker. A 32-case student follow-up suite covers shorthand, typos, long natural answers, split requests, fresh questions, sensitive/action questions, and still-ambiguous replies. The live validated File Search smoke runner first scored the same suite at `8.25/10` average orchestration-contract quality (`23` x `10`, `5` x `6`, `4` x `1`), exposing gaps in question-like clarification answers, mixed `split` follow-ups, unsafe-fresh diagnostics, and still-vague replies. Generic fallback/split/no-progress guards then moved the 9 low-score cases to `9/9` at `10/10` with state distribution `use` x `1`, `split` x `5`, `ignore` x `1`, and `clarify` x `2`; report: `docs/evaluations/rag-pending-state-v2-student-followups-2026-06-11.md`.

> **Update Note (2026-06-11):** Validated File Search now treats clarification context as explicit state, not only inferred chat text. When the assistant asks a clarification, the provider emits a generic `pendingClarification` diagnostic with the original user question, clarification question, missing slots, requested metric/facet, retrieval intent, source preference, risk level, and assistant text that must not be retrieved. Demo Chat preserves message metadata in recent history, reads the latest pending clarification state from prior bot replies, and passes that state back into the File Search orchestrator on the next turn. The contextual orchestrator prompt now receives both ordered conversation history and the pending state; if the LLM misclassifies a short or long clarification answer as standalone, the generic state resolver rewrites the query from the original user question plus the latest user clarification while preserving the requested facet. Fresh new questions are not consumed by stale pending state. This moves follow-up handling away from organization-specific keyword patches and toward a tenant-agnostic slot/state contract for Codex-like multi-turn behavior.

> **Update Note (2026-06-10):** Validated File Search now receives recent ordered conversation history from Public Demo Chat before any strict understanding, catalog, retrieval, or evaluator decision. A generic contextual orchestrator can classify the latest message as standalone, rewrite it from history, ask a clarification, or refuse before retrieval, so short continuation turns such as `olur et` are no longer searched literally. It also treats answers to prior assistant clarification questions as conversation state, not retrieval text: short or long replies such as `tümü` or `genel olarak tüm bölümler hakkında bilgi almak istiyorum` are rewritten with the original user question, and a structural repair prevents the assistant clarification sentence itself from becoming the search query if the LLM misclassifies the turn. The orchestration prompt now requires a `turn_type`, richer output schema, few-shot examples, explicit `do_not_retrieve_text`, a confidence threshold that clarifies instead of retrieving when the state is uncertain, and mini retrieval-plan signals (`retrieval_intent`, `requested_metric`, `source_preference`, `risk_level`) that are recorded in diagnostics. Demo Chat sends the last 10 ordered user/bot messages to this layer when available and fails open without history if history lookup is unavailable. The provider also supports generic source-priority groups and a bounded LLM research planner: the YİÜ tanıtım demo now tries brochure source groups first for non-table retrieval, can ask a planner model for 1-3 File Search hops when separate evidence facets are needed, and then falls back to deterministic website/PDF/approved corpus search only when planned hops do not retrieve evidence. Exact fee/quota/table-metric questions stay locked to brochure table evidence, and clarification-answer turns preserve the original metric/facet so row variants such as `ücretli` do not become fee questions when the user asked for taban puan or başarı sırası. Planner usage, hop count, required evidence, source groups, and blackboard attempts are recorded in diagnostics; `DEMO_CHAT_FILE_SEARCH_RESEARCH_PLANNER=0` disables the planner and `DEMO_CHAT_FILE_SEARCH_RESEARCH_PLANNER_MODEL` / `OPENAI_RAG_RESEARCH_PLANNER_MODEL` override its model. Brochure/table routing also treats variant-existence questions such as `burslu program var mı` and `ücretli program var mı` as primary table-row questions before broader scholarship or website/PDF evidence. A 10-case two-turn clarification follow-up smoke now records clarification quality, history resolution, final-answer quality, credits, and Codex-style scores; the run improved from `2/10` strict pass and `5.7/10` average to `10/10` strict pass and `8.6/10` average after metric carryover, clarification-first missing-subject gates, direct program-list catalog output, and preparation/staj boundaries.

> **Update Note (2026-06-08):** The strict validated File Search pipeline now has a direct-answer quality rubric, source-of-truth score reporting, and an auto catalog-candidate generator for quality lifts. Strict direct answers are classified as `grounded_direct_fact` (`9/10`), `safe_actionable_boundary` (`8/10`), or `needs_user_clarification` (`7/10`) in diagnostics; the report CLI can show score-band candidate gaps with `--with-candidates`, and the report/batch scorer now treats `strictQuality.suggestedScore` as the score authority. The broad grounded-direct rerun retested `155` candidate rows and improved `125`; the final current-7 closure rerun retested `15` questions and moved all `15/15` to `8/10` through generic decision-safe boundaries for preference advice, academic process, subjective reputation, housing/local-life, and inflected facility-resource prompts. Current artifact-merged 508-question distribution is `0` x `7`, `346` x `8`, `159` x `9`, and `3` x `10`.

> **Update Note (2026-06-07):** The strict validated File Search catalog now includes Credential / Accreditation / Recognition, Campus / Housing / Transport, and Contact / Official Next Step coverage. The targeted artifact `tmp/customer-question-batches/yiu-score-1-8-retest-2026-06-07T21-17-30-926Z.json` retested `64` old low/mid-score questions, improved `63`, and finished with all `64/64` answers at `8/10`. The new direct catalog separates accreditation from diploma validity and YÖK/denklik, handles country-specific diploma validity before generic recognition, maps 100. Yıl/Bağlıca/Balgat/Bağlum campus-program facts, guards service/transport/dining/housing details without unsupported positives, and routes direct contact/registration actions to official next-step boundaries.

> **Update Note (2026-06-07):** The strict validated File Search catalog now includes Finance / Payment Policy and Admissions / Tercih Decision coverage. The targeted artifact `tmp/customer-question-batches/yiu-score-1-10-retest-2026-06-07T20-45-20-343Z.json` retested `56` finance/payment/admissions questions, improved `26`, and finished with `53` answers at `8/10`, `3` at `9/10`, and `0` below `8/10`. The new direct catalog expands 2025 program fee rows, broad fee overview handling, KDV/taksit/IBAN/online-payment/source-conflict boundaries, payment-card safety separation, broad taban-puan / başarı-sırası boundaries, personalized placement-decision guards, and TYT/SAY/EA point-type listings.

> **Update Note (2026-06-07):** The strict validated File Search current-`7/10` lift now has deterministic clinical/staj/lab direct facts and policy boundaries. The targeted artifact `tmp/customer-question-batches/yiu-score-1-10-retest-2026-06-07T20-27-42-713Z.json` retested `18` current-7 clinical/staj/lab-like questions and moved all `18/18` to `8/10` with zero LLM/File Search calls. The effective 508-question distribution is now `27` x `7`, `421` x `8`, and `60` x `9`, still with `0` questions below `7`. The new catalog covers Tıp clinical timing/location, Uygulamalı Eğitimler Yönergesi staj policy, Ergoterapi staj/lab facts, broad active-practice scope guards, affiliated-hospital shorthand location guards, and actionable lab-visit candidate-event boundaries.

> **Update Note (2026-06-07):** The strict validated File Search current-`7/10` lift now has deterministic finance/scholarship/fee direct facts for the YİÜ demo. The targeted artifact `tmp/customer-question-batches/yiu-score-1-10-retest-2026-06-07T18-41-13-542Z.json` retested `6` finance-like edge cases, moved `5` current-7 answers to `8/10`, and leaves only the clinical `Staj ücretli mi?` clarification at `7/10`. The effective 508-question distribution is now `45` x `7`, `403` x `8`, and `60` x `9`, still with `0` questions below `7`. The new catalog separates `Burslu` quota rows from discount bursaries, refuses unsupported preference-scholarship scope claims, and answers shorthand `İlk ve Acil Yardım` fee questions without LLM/File Search calls.

> **Update Note (2026-06-07):** The strict validated File Search current-`7/10` band was fully rerun after the actionable boundary layer. The actual current-7 set had `108` questions; the main artifact `tmp/customer-question-batches/yiu-score-1-10-retest-2026-06-07T17-55-28-333Z.json` plus final edge patch-smoke `tmp/customer-question-batches/yiu-score-1-10-retest-2026-06-07T18-07-14-626Z.json` moved `58/108` to `8/10`, leaving `50` at `7/10` and no over-promoted `9/10` in this rerun. The full 508-question effective distribution is now `50` x `7`, `398` x `8`, and `60` x `9`, with `0` questions below `7`. The patch-smoke also added generic guards for bot identity questions and unsupported WhatsApp advisory-line claims.

> **Update Note (2026-06-07):** The strict validated File Search current-`7/10` lift now has an actionable no-info boundary layer. Bare "no clear information" answers are repaired into category-specific decision boundaries for finance/payment, scholarships, campus/service, clinical/staj/lab, registration/process, credential/outcome, and contact-style questions, stating what is missing and which official facet must be verified. A targeted 83-question smoke `tmp/customer-question-batches/yiu-score-1-10-retest-2026-06-07T16-13-08-678Z.json` improved `65/83`, moving average score from `5.72` to `7.89` with `64` answers at `8/10` and `6` at `9/10`; the final hospital-transport scope smoke `tmp/customer-question-batches/yiu-score-1-10-retest-2026-06-07T16-22-50-019Z.json` moved `#220` from `5` to `8` by preventing adjacent campus/transport evidence from proving unsupported hospital access.

> **Update Note (2026-06-07):** The strict validated File Search current-`7/10` lift now has a deterministic supported-fact promotion pass. The strict catalog answers institution/campus location, affiliated-hospital terminology, agreement dormitory facts, library/study-area/student-community facts, and çift anadal policy/pair/second-diploma boundaries before retrieval. The targeted 14-question smoke `tmp/customer-question-batches/yiu-score-1-10-retest-2026-06-07T10-46-58-927Z.json` improved `12/14`, scored all `14` at `8/10`, and required `0` LLM/File Search calls because every answer routed through `catalog_direct`. Projected against the previous current-7 slice, this moves the distribution from `164` x `7` and `9` x `8` to `152` x `7` and `21` x `8` while preserving unsupported-scope boundaries.

> **Update Note (2026-06-06):** The strict validated File Search current-`7/10` band has now been broadly rerun with the retry + blackboard layer over `173` customer questions. The final broad artifact is `tmp/customer-question-batches/yiu-score-1-10-retest-2026-06-06T14-46-39-844Z.json` (`315.10` estimated credits, `811,827` total tokens, `95` File Search tool calls). Manual audit of the non-7 results added global hardening for off-topic tutoring requests, career/outcome comparison prompts, hospital case-volume concerns, accidental `lab` substring facet matches, and LLM evaluator repairs that add unsupported payment methods. The targeted post-audit artifact `tmp/customer-question-batches/yiu-score-1-10-retest-2026-06-06T15-04-35-725Z.json` verifies the adjusted current-7 slice at `0` x `6`, `164` x `7`, `9` x `8`, and `0` x `9`, keeping the quality floor while preventing over-promoted unsupported claims.

> **Update Note (2026-06-06):** The strict validated File Search pipeline now has an evidence-seeking retry loop plus research blackboard diagnostics. When the deterministic critic rejects an answer for a missing/mismatched facet or unsupported institutional claim, the provider can run one targeted File Search retry for the missing facet before falling back to repair/refusal. Strict diagnostics now record the normalized query, selected source groups, citation counts, facets, retry reason/outcome, and final verdict so QA can see where the tool decision failed. The critic also treats capitalized institutional assertions such as `Evet` as assertions, and LLM evaluator repair output is rejected if it contains unsupported speculation such as `genellikle`, `olabilir`, or `muhtemelen`. A focused service/transport smoke over `#273`, `#274`, `#277`, and `#429` improved `4/4`, moving average score from `3.25` to `7.00` with contextual no-info repairs and no speculative fee/service claim.

> **Update Note (2026-06-06):** The strict validated File Search pipeline now has a facet-aware answer contract. It classifies the requested answer facet before accepting drafts, adds that facet to research-plan `expectedClaims`, and rejects adjacent-evidence answers where the response proves the wrong thing. This closes the observed v2 false positives where lab/facility questions were scored as improved even though the answer only proved program existence. The focused smoke for `#306 Ebelik uygulama laboratuvarı var mı?` and `#310 Tıbbi Laboratuvar programı için laboratuvar var mı?` now returns deterministic zero-token facility-boundary answers instead of program-existence claims.

> **Update Note (2026-06-06):** The strict validated File Search pipeline now has a Codex-like research-plan and claim-ledger layer. Strict diagnostics record the selected route, tool chain, evidence requirements, source groups, expected claims, and risk level before retrieval/answering; high-risk answer drafts also produce a claim ledger showing supported vs unsupported exact policy/value claims. The deterministic critic consumes the ledger, raw File Search placeholder text such as `retrieval complete` is no longer treated as a supported answer, and LLM evaluator repair paths can use available evidence citations without leaking sources into final no-info refusals. Generation token usage is now preserved even when a generated draft is rejected and repaired/refused.

> **Update Note (2026-06-06):** The strict File Search pipeline now has claim-term verification v1 for high-risk institutional statements: broad source overlap such as "fee table exists" no longer supports exact claims like `KDV dahildir` unless the claim terms themselves are present in evidence. The current YİÜ demo catalog also has deterministic campus-life scope guards for Wi-Fi, kafe/kantin, spor salonu, vejetaryen yemek, servis saat/güzergah, campus safety, nearby apart/rental housing, and related current-facility prompts, plus a specific `Mavi diploma` credential boundary. Contextual no-info templates were tightened for burs/payment/crypto prompts, and strict LLM evaluator refusals now preserve the deterministic contextual refusal when the evaluator does not provide a revised answer. A targeted `7/10` smoke over 16 old score-7 questions improved `2/16` to `8/10` while moving most new campus-life boundaries to zero-token deterministic guards; a follow-up crypto-payment smoke kept the score at `7` but produced a cleaner direct payment-method boundary.

> **Update Note (2026-06-06):** The YİÜ strict File Search pipeline now has a quality-floor pass for the full 508-question customer audit. After the mid-score retest, updated evaluator retry-path sanity run, residual closure for campus-event plus university-negative boundary prompts, and full original-`7/10` rerun, the current breakdown is `0` questions in `1-4`, `0` in `5-6`, and `508` in `7-9`; exact scores are `173` at `7/10`, `280` at `8/10`, and `55` at `9/10`. The catalog now covers faculty/listing aliases, lisans vs ön lisans program lists, Fizyoterapi vs Fizyoterapi ve Rehabilitasyon distinction, affiliated/own-hospital scope, broad clinical/staj clarification, program-specific clinical/lab/staj scope guards, registration-process boundaries, current tanıtım/kampüs event boundaries, reputation/negative-review boundaries, housing scope boundaries, and professional-authority guardrails. A first targeted `7`-band guard smoke moved hospital-campus proximity to `8/10` and prevented gendered dormitory/application questions from inventing unsupported dormitory names or processes. Bare no-information answers are repaired into contextual meeting-ready boundaries instead of the old generic copy for repeated ambiguity classes, and the generic `unsupported_institutional_claim` critic now blocks unsupported operational/procedural claims from passing merely because nearby evidence mentions the same broad topic. The remaining `173` score-7 answers are now all retested and diagnosed; the largest lift buckets are general/boundary (`46`), campus life (`35`), clinical/staj/lab (`34`), catalog/policy (`26`), and finance (`22`).

> **Update Note (2026-06-05):** The YİÜ strict File Search pipeline low-score retest hardening now covers the main boardroom-risk failures found in the 508-question customer set. The structured catalog blocks unsupported positive claims for own/affiliated hospital identity/status, answers Tıp clinical training timing deterministically, handles international diploma validity as country-specific equivalency instead of a guarantee, separates `yurtdışı` from dormitory `yurt` routing, answers/guards dormitory scope questions, and routes broad `taban puanlar` / `başarı sıralamaları` questions to brochure table sources. The retest of the original 106 questions scored `1-4` improved all `106/106`, moving average score from `2.61` to `6.88` with no remaining question below `6`.

> **Update Note (2026-06-05):** The YİÜ Tanıtım Günleri strict File Search demo now has an optional LLM global evaluator after deterministic validation. In strict mode the demo bridge enables this critic unless `DEMO_CHAT_FILE_SEARCH_LLM_EVALUATOR=0`; the evaluator checks whether the answer is grounded, safe, directly responsive, and meeting-room ready for rector/dean/board-chair review, then can pass, repair, clarify, refuse, or trigger exactly one stronger File Search retry before final deterministic validation. Evaluator usage is included in the same RAG usage metadata and can use `DEMO_CHAT_FILE_SEARCH_EVALUATOR_MODEL` / `OPENAI_RAG_EVALUATOR_MODEL`.

> **Update Note (2026-06-05):** The YİÜ Tanıtım Günleri validated File Search demo now has a strict answer quality mode on by default. Strict mode normalizes colloquial/typo-heavy questions before routing (`dkt kaç tl` -> `Dil ve Konuşma Terapisi ücreti ne kadar?`), answers high-confidence program/faculty existence and listing questions from a structured brochure-derived catalog before retrieval, blocks unsupported positive claims such as absent faculties/programs, and returns category-specific safe answers for sensitive identity/payment/credential/fraud/abuse requests. The demo bridge passes `qualityMode='strict'` unless `DEMO_CHAT_FILE_SEARCH_STRICT_QUALITY=0`; non-demo/global provider defaults remain unchanged.

> **Update Note (2026-06-05):** Customer-facing RAG/fallback now has a reusable clarification gate before no-information copy. The gate recognizes under-specified price, contact, location, and generic information turns, asks one contextual question, and avoids token-consuming retrieval when the missing subject is clear. The YİÜ validated File Search provider uses education wording for turns such as `Okumak kaç para?` or `Nerede?`, while concrete turns such as `Tıp kaç para?` and `Tıp Fakültesi nerede?` still proceed to evidence-backed retrieval. Standard fallback uses the same sector-neutral gate for other tenants.

> **Update Note (2026-06-05):** The onboarding completion bot-mode modal now gives visible saving/error feedback after a mode choice instead of looking inert while the server action refreshes the page. The unlock server action also verifies membership against the selected organization id rather than the user's first membership row, preventing multi-organization users from getting a silent `Forbidden` failure on the completion overlay. The YİÜ Tanıtım Test org was verified with `bot_mode='active'`, `bot_mode_unlock_required=false`, and a populated `bot_mode_unlocked_at`.

> **Update Note (2026-06-05):** Browser-rendered assistant replies now display Markdown-style bullet and numbered lists as semantic list blocks instead of plain wrapped text. The shared `MessageRichText` renderer also normalizes common assistant artifacts such as `Intro: - item - item` into readable bullets, so Public Demo Chat, Inbox, and other web chat surfaces present policy/fee/checklist answers more cleanly without changing outbound WhatsApp/Instagram/Telegram formatting or RAG provider behavior.

> **Update Note (2026-06-05):** The customer-preview `yiu-tanitim-gunleri-2026` public demo now uses the Yüksek İhtisas Üniversitesi icon from `public/yuksek-ihtisas-universitesi.png`. The production asset returns HTTP 200 and the live demo HTML now preloads and renders `/yuksek-ihtisas-universitesi.png` from the `demo_chat_channels.logo_url` field while leaving the validated File Search provider, corpus, and maintenance state unchanged.

> **Update Note (2026-06-04):** Demo Chat organizations can now use an enabled `demo_chat_channels` row as the onboarding/customer-entry connection signal. This keeps real WhatsApp/Instagram/Telegram channel state untouched while allowing demo-only organizations such as `YİÜ Tanıtım Test` to complete the channel step, unlock bot-mode selection, and show Demo Chat conversations in Inbox without connecting a live messaging provider. The YİÜ Tanıtım Test org was also explicitly marked with `channel_connection_completed_at` and `bot_mode='active'` / `bot_mode_unlock_required=false` for the live customer demo.

> **Update Note (2026-06-04):** The YİÜ Tanıtım Günleri validated OpenAI File Search demo is now deployed to production at `https://app.askqualy.com/demo/yiu-tanitim-gunleri-2026` from Netlify deploy `6a21d9edb77d4ce04fbb1571`. The final local 20-question report over brochure fee/scholarship/link details, website HTML/contact/bilgi-paketi pages, approved PDF regulation routing, and unsupported admissions guardrails passed `20/20`, produced follow-ups in `20/20` questions with `13` distinct follow-up lines, averaged `6992.2ms`, p95 `25119ms`, max `30863ms`, and preserved safe refusals for unsupported admissions promises. A production-link smoke over brochure fee, scholarship, student-affairs contact, BİDB routing, and unsupported appointment promise passed `5/5`. Report: `tmp/rag-evals/yiu-approved-corpus/rag-eval-file-search-validated-2026-06-04T14-40-11-779Z.md`.

> **Update Note (2026-06-04):** The isolated YİÜ Tanıtım Test organization is configured for customer-preview testing. Organization `50102447-4bb2-4bd5-a332-fb721a3c7949` now uses name `YİÜ Tanıtım Test`, slug `yiu-tanitim-test`, premium status with `10000` monthly package credits, and demo channel `yiu-tanitim-gunleri-2026` with maintenance off and a server-side shared secret. The org Knowledge Base has the approved 135-source corpus as `YİÜ Tanıtım Günleri 2026 - Approved Corpus` (`113` PDFs, `22` article/markdown docs, `879` chunks), while OpenAI File Search uses vector store `vs_6a213186dd6c81919ec51dca2a9913d5`. Public Demo Chat polling now tries a preview-only validated OpenAI File Search bridge for the configured demo slug before falling back to the standard Supabase RAG/shared pipeline; the existing `yiu-qualy-ai-demo` and global default remain unchanged. Generalizing this from slug/env gating to DB-backed provider config remains a follow-up before broader rollout.

> **Update Note (2026-06-04):** The brochure-first validated OpenAI File Search hardening is complete locally on the isolated spike branch. The provider now routes by intent/source group, validates brochure table facts from one typed row, uses evidence-gated approved structured facts and document routing, performs at most one narrowed retry, scores answer/source/follow-up dimensions independently, and appends a separately validated conversion follow-up. After adding a domain-independent generic engagement fallback, the final current-model run (`gpt-4.1-mini` retrieval + `gpt-4o-mini` answer) passed `50/50`, produced follow-ups in `46/46` eligible cases, averaged `5282.5ms`, p95 `8894ms`, and kept refusal cases without follow-ups. Retrieval-only `gpt-5.4-mini` also passed `50/50` and reduced average latency to `4018.5ms`; answer-only `gpt-5.4-mini` passed `50/50` without a meaningful quality gain. Keep the current pair as default, treat faster `gpt-5.4-mini` retrieval as optional until provider-stage model-aware actual-cost metering exists, and keep isolated org/demo setup plus customer signoff as preview blockers. GPT-5.5 was intentionally not run.

> **Update Note (2026-06-04):** The same YİÜ full-corpus 50-question benchmark was rerun with `file-search-validated`. The validated run passed `28/50`, averaged `10566.8ms` latency with p50 `8915ms`, p95 `14317ms`, max `52244ms`, and estimated `154.3000` credits total (`3.0860` average). It improved the unsupported "kesin kontenjan ayırır mısınız?" guardrail, but it underperformed raw File Search on supported PDF routing and still missed critical brochure fee/table facts. A brochure-focused validated retry with `max_results=20` passed `20/30`, averaged `10663.5ms`, and cost `5.7333` credits per question on average, so simply increasing retrieval depth is not the solution. The validated run produced `0/50` grounded follow-up questions, making admissions follow-up generation a separate must-fix before preview.

> **Update Note (2026-06-04):** The YİÜ admissions/tanıtım günleri approved corpus now has a first realistic 50-question raw OpenAI File Search benchmark across the website packages, 113 approved PDFs, categorized brochure markdown, campus/contact, and unsupported admissions scenarios. The run used vector store `vs_6a213186dd6c81919ec51dca2a9913d5` and produced `tmp/rag-evals/yiu-approved-corpus/rag-eval-file-search-2026-06-04T08-36-43-834Z.md` plus JSON output. Raw File Search passed `36/50`, averaged `6745.3ms` latency with p50 `6167ms`, p95 `10482ms`, max `22298ms`, and used an estimated `126.8000` credits total (`2.5360` average). Brochure scholarship and PDF regulation-routing questions were strong, while table edge cases, broad website/contact lookup, source selection, and adjacent old-context contamination confirm that the validated File Search pipeline is still required before customer preview.

> **Update Note (2026-06-04):** The YİÜ admissions/tanıtım günleri approved corpus with brochure is now ingested locally into OpenAI File Search. The customer brochure PDF was not uploaded raw-only because its text layer is unreliable; instead, the verified extraction was split into 8 categorized markdown sources and combined with the existing 14 website packages plus 113 approved PDFs. The resulting 135-file corpus was ingested into temporary vector store `vs_6a213186dd6c81919ec51dca2a9913d5` with 135 completed files, 0 failed, 0 in-progress, and `13120263` usage bytes. Readiness helper output is `ready: true`; this remains a local/eval artifact and does not switch the existing public YİÜ demo.

> **Update Note (2026-06-04):** YİÜ File Search pre-brochure corpus ingestion is complete locally. The approved website scope is no longer 1790 individual files: the existing non-PDF crawl is packaged into 14 source-indexed markdown bundles, combined with 113 PDFs from the two user-approved YİÜ mevzuat/yönergeler links, and ingested as a 127-file compact corpus into temporary vector store `vs_6a20bc28099081918fed4bfef3569c02` with 127 completed files, 0 failed, 0 in-progress, and `13080355` usage bytes. The remaining gate is the customer brochure PDF and question list, then benchmark/signoff before any preview or provider switch.

> **Update Note (2026-06-04):** The YIU File Search effort is now narrowed from whole-site/all-PDF migration to a 2026 admissions/tanitim-gunleri brochure bot. The approved direction is brochure-only OpenAI File Search as the primary demo provider, wrapped with Qualy validation and targeted retry; current Supabase RAG may be used only as a benchmark baseline. File Search is not becoming the default for every new organization: provider choice should be database-controlled per organization/demo, resolving demo override first, organization setting second, and global Supabase RAG fallback last. Optional Skills/FAQ can cover deterministic high-value turns, customer-visible preview uses a new Qualy organization plus `yiu-tanitim-gunleri-2026` demo slug, and the existing `yiu-qualy-ai-demo` provider is unchanged. Source links should remain visitor-friendly like the current demo: answer first, validated supporting sources appended after. The assistant should also be conversion-focused for admissions season by adding grounded, contextual follow-ups that move visitors toward event attendance, program exploration, approved contact, or optional interest capture without unsupported sales claims or personal-data pressure. Those behavior rules should be seeded through existing organization AI settings (`assistant_role`, `assistant_intake_rule`, `assistant_never_do`, `assistant_other_instructions`) rather than hard-coded, while grounding/source/refusal validation remains enforced in code. Website-embedded chatbot delivery is a later phase after Q&A quality, source quality, refusal safety, follow-up quality, latency, and cost are proven.

> **Update Note (2026-06-04):** Public Demo Chat deterministic fast RAG now applies organization AI behavior settings before source append. Source-backed deterministic repair still avoids the heavier grounded-generation step, but it loads locale-aware AI settings, runs the bounded source-grounded polish layer, records polish usage when tokens are spent, and persists `rag_polish` metadata so QA can see whether the behavior layer changed the answer. If polish cannot safely improve the reply, the original deterministic answer remains.

> **Update Note (2026-06-04):** Organization AI behavior settings now have an explicit customer-facing boundary. Shared live WhatsApp/Instagram/Telegram/Demo Chat LLM reply paths already load locale-aware settings and pass the compiled prompt into RAG generation, RAG polish, legacy RAG completion, and fallback. Simulator and standalone fallback now resolve the response language before loading settings too. Deterministic Skill replies stay exact operator-authored text: they are not polished, rewritten, or given extra follow-ups by AI behavior settings.

> **Update Note (2026-06-04):** The YIU admissions brochure File Search spec now includes customer-preview readiness gates: OpenAI vector-store ingestion must be completed with matching file counts and no failed/in-progress files, source citations must map through an approved visitor-safe manifest, local/preview vector-store lifecycle and usage size must be tracked, matched Skills must remain exact operator-authored templates without polish/rewrite, and customer signoff is required on benchmark answers, refusals, sources, and follow-up examples before external preview.

> **Update Note (2026-06-04):** The first PDF-independent implementation slice for the YIU admissions brochure File Search demo is complete on the spike branch. The local/eval layer now has brochure vector-store readiness validation, approved citation-manifest parsing, visitor-safe filename-to-source mapping, readiness-aware ingest output with vector-store status/file counts/usage/lifecycle metadata plus source manifest skeletons, and a disabled-by-default provider config resolver that preserves the fallback order of demo override, organization setting, then global Supabase RAG.
...
> **Update Note (2026-06-03):** OpenAI File Search RAG migration now has a completed local side-by-side benchmark over the user-approved YİÜ mevzuat PDF links, without changing the public demo provider. The run downloaded 113 PDFs from the two approved source pages, generated a 50-question PDF-limited case set, indexed them into a temporary OpenAI vector store, and produced a question/answer report comparing current Supabase RAG against OpenAI File Search. With source-title slug normalization, current RAG passed 43/50 and File Search passed 41/50; File Search was faster on average (`6.56s` vs `15.19s`) and lower in estimated Qualy credits (`2.41` vs `3.53` average), but rollout decision remains pending manual review (`tmp/rag-evals/yiu-link-pdfs/rag-eval-compare-2026-06-03T13-13-50-146Z-replay.md`).

> **Update Note (2026-06-03):** The File Search spike now also has a 25-question scenario benchmark focused on realistic user wording, tone, source-link requests, and unsupported/contact/price/deadline questions from the same approved PDF corpus. The replayed report uses any-of source matching for acronym sources, maps File Search citation filenames back to public PDF URLs, and recognizes Turkish no-clear-information refusals during replay. Result: current Supabase RAG passed 12/25 while OpenAI File Search with the Qualy instruction profile passed 18/25, with File Search averaging `6.51s` and `2.39` estimated credits versus current RAG at `15.41s` and `3.79` estimated credits. File Search is clearly promising for supported PDF questions, but unsafe contact/detail invention still blocks any default provider switch until an answer validator/repair layer catches unsupported or adjacent-source filler (`tmp/rag-evals/yiu-link-pdfs/rag-eval-compare-2026-06-03T14-38-19-526Z-replay.md`).

> **Update Note (2026-06-03):** The File Search spike now includes a validated provider path that runs `file_search` retrieval through Qualy's Evidence Pack, grounded answer validation, critical-value checks, generic footer-contact rejection, and source URL append from selected citations only. On the same 25-question scenario set, `openai_file_search_validated` passed 21/25: supported PDF questions passed 14/18 and unsupported/contact questions passed 7/7. This improves safety over raw File Search (`2/7` unsupported) while preserving most supported-answer quality, but the remaining supported misses show that targeted retry by document title/source intent is still needed before preview or production rollout (`tmp/rag-evals/yiu-link-pdfs/rag-eval-file-search-validated-2026-06-03T20-16-23-927Z.md`).
...
> **Update Note (2026-06-02):** Public Demo Chat now has DB-backed runtime maintenance via `demo_chat_channels.maintenance_enabled`, so operators can hide or reopen a demo slug from Supabase without a Netlify redeploy. When enabled, demo pages render a localized Qualy-branded maintenance screen after demo-channel lookup but before browser access-token minting, and demo chat `POST`/polling `GET` requests return `503` with `Retry-After` before auth, Inbox, RAG, or AI pipeline work can run. `DEMO_MAINTENANCE_MODE=1` remains an emergency global override. Admins can keep maintenance active while testing privately by setting `DEMO_MAINTENANCE_BYPASS_TOKEN` and opening the demo with `?maintenance_bypass=<token>`, which redirects through a server route, stores only a hashed HttpOnly bypass cookie, drops all stale query parameters from the clean URL, and lets the page plus demo API continue for that browser until `?maintenance_bypass=off` clears it.
...
> **Update Note (2026-06-01):** Public Demo Chat now treats scope-help prompts such as `sana başka hangi konularda soru sorabilirim` as demo meta questions and answers them before RAG or shared AI recovery. Existing pending polls carrying the same message can recover with the deterministic scope reply instead of staying in repeated `202 pending` loops.
...
> **Update Note (2026-06-01):** Public Demo Chat focused RAG latency now avoids hidden LLM planner work on deterministic context-expanded fast-path searches; broad fallback still owns planned semantic variants when focused evidence cannot answer. Focused lecture-note retrieval tries one high-signal required evidence group before wider OR scans, scopes that probe to named academic units when the question supplies one, and health-report/mazeret-exam retrieval returns from required remedy evidence instead of paying the broad health-policy OR scan when the remedy rows already ground the answer. Production-style profiling against the YİÜ corpus showed the target local focused searches drop from multi-second waits to sub-second evidence retrieval without adding customer-specific phrases.

> **Update Note (2026-05-29):** Public Demo Chat contextual follow-ups now try context-expanded focused evidence before broad RAG search, so references such as `bu program` can reuse the latest explicit topic without paying the slower broad/planned-search path when focused evidence is enough. Persisted demo bot metadata now includes `rag_diagnostics` timing/search-strategy fields for production latency triage. Deterministic fast-path answers also get a source-safe micro-polish layer that may expand terse course-code answers from retrieved document titles, but only when the title itself supports the acronym; no unsupported engagement or factual wording is introduced.

> **Update Note (2026-05-29):** Production Demo Chat smoke exposed two general RAG gaps and both are now covered by regression tests: contextual deterministic repair uses the context-expanded retrieval question instead of the bare follow-up text, contextual focused retrieval tries only that strongest expanded query before broad fallback, and source-link ranking treats Turkish word-number durations such as `altı yıldır` as concrete evidence so compound answers can keep distinct sources for distinct facts.

> **Update Note (2026-05-29):** Public Demo Chat fast RAG now short-circuits before organization-settings lookup, grounded generation, polish, and usage metering when deterministic repair can already build a non-empty answer from retrieved chunks. This keeps clear source-backed facts fast while preserving the slower source-quote-validated generation path for cases that actually need answer authoring. Medicine training/staj repairs also compact dense regulation evidence into a short answer that keeps the six-year duration, preclinical/clinical/intörnlük structure, and the absence of a separate `yaz stajı` claim grounded in source text.

> **Update Note (2026-05-29):** RAG-generated answers now pass through the same deterministic repair and source-grounded polish safety net as extractive answers before reaching Public Demo Chat or normal inbound channels. This prevents quote-valid but intent-drifted generated replies, such as final/bütünleme answers sliding into final-exemption rules, from bypassing existing repair. Broad policy-duration answers also preserve source grouping when evidence separates general brackets from exception brackets, so annual-leave answers do not flatten multiple regimes into one ambiguous list. Contact repair now avoids presenting a generic institution/footer phone as a program-specific phone when only the program e-mail is source-matched, and contact source links are filtered to sources that contain the concrete e-mail/phone in the answer. Generic `Daha fazla bilgi almak ister misin?` tails and visitor-profile prompts such as asking which department/program the user is considering are stripped unless the engagement is a source-grounded adjacent-detail offer. Public Demo Chat contextual follow-ups now filter candidate chunks to the latest explicit user-topic anchor before building RAG context, preventing generic follow-up terms from drifting into stale or unrelated sources. Shared-predicate compound questions such as `A ve B kampüsleri nerede?` now expand into separate per-subject retrieval queries so answer generation sees evidence for each requested subject, and campus/location repair completes omitted requested subjects when the retrieved chunks contain their addresses.

> **Update Note (2026-05-28):** Public Demo Chat polling now starts with bounded fast RAG recovery before the heavier shared inbound pipeline, so clear KB-backed questions can answer without dragging Netlify GET requests toward platform timeout. The fast path still prefers source-quote-validated LLM answer generation for natural wording and organization voice, uses short demo-specific AI timeouts, and falls back to deterministic repair only when grounded generation cannot safely answer. For context-dependent follow-ups such as "this program" or "that exam", demo fast RAG now reads recent demo history, searches first with recent explicit user-question context, skips prior ambiguous follow-ups as anchors, and runs history-aware broad/planned retrieval instead of generic focused evidence fallback. If quote-validated generation cannot verify a contextual answer, deterministic repair is allowed only when retrieved chunks contain the prior explicit topic anchor; otherwise the fast path refuses repair rather than answering from an unrelated `staj`-style source. Grounded RAG generation also gets the same source-grounded polish safety net as extractive answers when the first generated reply is factually valid but terse. The polish pass may warm the organization voice and add one engagement offer only if its evidence quote exists in retrieved Knowledge Base context, and its usage/metadata is recorded separately. Demo/outbound formatting also keeps canonical source URL lines separate after e-mail sentences, and source-link ranking filters weak extra links when they do not contain explicit answer values such as durations or percentages.

> **Update Note (2026-05-28):** Shared RAG answer generation now uses conversation-aware retrieval and grounded generation before deterministic/extractive fallback. Knowledge Base search passes recent conversation turns into the query planner for follow-up resolution, and grounded RAG answer generation receives recent history so references such as "this program" can be resolved without guessing. Public Demo Chat uses the same grounded answer authoring inside its bounded focused-evidence recovery path, while the heavier shared inbound AI pipeline remains the fallback when fast RAG cannot safely answer. The model may write in the organization's configured voice only when it returns exact support quotes from retrieved Knowledge Base context; generated answers are rejected if support quotes are missing or if critical values such as numbers, phone numbers, e-mails, document codes, or URLs are not present in the retrieved evidence. Deterministic repair remains as a fallback safety net instead of the primary answer-writing mechanism.

> **Update Note (2026-05-28):** RAG contact-answer repair now scores all retrieved e-mail/phone candidates against the named unit/program in the user question before overriding a model answer. For specific contact questions, generic university/contact-page values are ignored unless their local evidence window matches the requested subject, so broad contact tables cannot answer `Bilgi İşlem` with `yiu@yiu.edu.tr` or TLT program contact questions with an unrelated library e-mail. General institution contact questions remain generic, and phone extraction prefers `Telefon/Tel` values over nearby `Fax/Faks` values.

> **Update Note (2026-05-28):** RAG language and answer repair now treat ASCII-written Turkish university/policy questions as Turkish, so demo and shared RAG polish keeps organization voice instructions in Turkish even when users type `Tibbi`, `programinda`, `kac is gunu`, or similar non-diacritic text. Contact-number questions are routed to contact evidence before document-code repair, document code repair now requires document/form/regulation intent rather than the broad word `numara`, final/bütünleme referential evidence handles optional punctuation and `bu sınav` wording, English retrieval boilerplate is stripped from Turkish grounded answers, and UI autolinking no longer turns document codes such as `EÖB.KLV.0001` into fake domains.

> **Update Note (2026-05-28):** RAG answer polish now applies explicit organization tone/personality/style instructions more strongly than the terse extractive answer while preserving the same source-grounded facts. Extractive RAG replies persisted from Public Demo Chat and the shared inbound pipeline now include `rag_polish` metadata with `usedPolish`, `addedEngagement`, and `model`, so production QA can tell whether the organization voice layer actually changed a reply or safely fell back.

> **Update Note (2026-05-28):** Public Demo Chat source-correctness validation now checks that the cited source supports the answer wording, not only that the link opens. When retrieved evidence supports a broader object such as `ders içeriği paylaşımı` but not the narrower `ders notları`, RAG answer repair keeps the supported platform names while broadening the customer-facing wording to `ders içerikleri` so the answer and cited source stay aligned without adding customer-specific phrase rules.

> **Update Note (2026-05-28):** Public Demo Chat and shared extractive RAG now run grounded fast-path answers through a bounded LLM polish step that can make wording warmer and add one engagement follow-up only when the model provides an exact evidence quote from retrieved Knowledge Base context. If the quote is missing or the polish drops factual values such as numbers, phones, e-mails, or other explicit values, the system falls back to the original extractive answer. The polish prompt treats the organization AI settings as the voice and behavior contract, and demo/shared RAG settings are loaded with the detected response language so Turkish structured instructions are actually compiled into Turkish answers. Compound knowledge questions are split into independent retrieval queries so a multi-source answer is returned only when every clause has evidence instead of answering just the first clause; demo polling now also avoids running the heavier focused-evidence scan on the full compound sentence and aborts timed-out polish calls so slow LLM/network work cannot keep a Netlify invocation open until the platform timeout. Demo source links now render as compact inline `Kaynağı aç / Open source` or numbered `Kaynak 1 / Source 1` labels while raw stored URLs remain unchanged. Source correctness checks now prefer direct evidence pages over listing/index pages when both are retrieved, campus/address repair preserves multiple distinct campus entries from the same source instead of collapsing a unit to the first matching address, lecture-note platform names are removed when the retrieved evidence does not support them, and source ranking treats short platform names such as UZEM/MEDU/ÖBS as concrete answer evidence.

> **Update Note (2026-05-27):** Public Demo Chat now renders standalone RAG source URL lines as a localized short action link in assistant replies. The stored bot text and channel/inbox rendering still keep raw URLs, so traceability and messenger compatibility remain unchanged while the customer-facing demo reads cleaner.

> **Update Note (2026-05-27):** Public Demo Chat RAG retrieval now narrows high-signal campus and medical-school policy scans with required evidence groups before falling back to broad OR searches, preserves standalone subject-cue questions as the first search query even when history/router rewrites drift, and boosts document-title initialism matches so abbreviation questions such as TLT/SBF-style prompts find the right program or unit evidence. Answer/source-link repair now can recover a no-information abbreviation answer from retrieved document titles, strips generic engagement prefaces and role-assumptive clarification tails with a general trailing-sentence filter, repairs final/bütünleme answers that drift into adjacent final-exemption policy, and fills missing in-year board-grade `%96 + Hekimliğe Uyum / Kanıta Dayalı Tıp` formula details when the evidence supports them. The latest full live YİÜ screenshot-plus-variant challenge passed 33/33 at `tmp/crawl-output/live-yiu-challenge-pipeline-qa-2026-05-27T18-20-44-717Z.md`, and the extra generic-tail scan had no QA flags.

> **Update Note (2026-05-27):** Public Demo Chat now detects expired or malformed signed browser access tokens before sending a message, shows localized TR/EN refresh copy, and reloads the dynamic demo page to mint a fresh token instead of surfacing a `401 Demo access denied` request failure after a tester leaves the tab open past the token TTL.

> **Update Note (2026-05-27):** Knowledge Base chunking now has a general evidence-enrichment rule for future PDF uploads and crawler imports: structural sections are preserved even when there is only one heading, markdown table rows and high-signal contact/address/code/value lines become standalone `table-row` / `evidence-row` chunks with evidence metadata, and imports insert chunks in bounded batches with recursive bisection for large vector writes. A new service-role reindex script can rebuild existing organization documents with the same rules, and the Yüksek İhtisas test organization was reimported/rechunked from the M1 crawl/PDF corpora into 6,153 ready documents and 49,863 chunks. The latest corpus-health scan reported 0 ready documents without chunks, 0 missing source URLs, 0 broken URL text findings, and the live checks passed 33/33 plus public demo canary 12/12 (`tmp/crawl-output/live-yiu-challenge-pipeline-qa-2026-05-27T12-10-08-953Z.md`, `tmp/crawl-output/public-demo-canary-2026-05-27T12-16-14-803Z.md`, `tmp/crawl-output/yiu-corpus-health-2026-05-27T11-25-40-634Z.md`).

> **Update Note (2026-05-27):** Public Demo Chat pages now force dynamic rendering with zero revalidation so the short-lived signed API token embedded in the browser client cannot be served stale from Netlify/Next cache and trigger `401 Demo access denied` when a tester asks a question.

> **Update Note (2026-05-27):** Public Demo Chat RAG recovery now uses the full retrieved candidate set, not only the shortened completion context, when validating/repairing answers and appending source links. Final-exemption policy evidence is normalized across PDF line breaks and kept separate from bütünleme intent, TLT ÇAP recovery runs before generic eligibility repair, annual-leave duration evidence prefers service-year bracket lists over unrelated request-deadline sentences, and canonical source-link post-processing strips generic/role-assumptive helper tails before adding the trusted URL. The latest 33-question live YİÜ screenshot-plus-variant challenge passed 33/33 at `tmp/crawl-output/live-yiu-challenge-pipeline-qa-2026-05-27T07-13-35-832Z.md`.

> **Update Note (2026-05-27):** Public Demo Chat final/bütünleme recovery now recognizes referential policy evidence where a document first names the exam as `bütünleme sınavı` and the next clause says eligible students enter `bu sınav`, so clear PDF-backed answers can stay on the bounded extractive RAG path instead of falling into the slow full inbound pipeline. A broader RAG modernization plan was added to move future improvements toward hybrid retrieval, richer chunk metadata, evidence validation, reranking, and eval-driven correctness rather than customer-question-specific patches (`docs/plans/2026-05-27-rag-modernization-plan.md`).

> **Update Note (2026-05-27):** Public Demo Chat RAG duration repair now compacts long course-table evidence rows before replying and preserves complete duration-bracket lists for broad policy questions. When a staj/duration question is answered from a dense curriculum row, the repair step extracts the matching `Yaz Stajı` course label plus the explicit duration such as `20 iş günü` instead of exposing unrelated neighboring course-table text; when the user asks a broad policy-duration question without specifying a threshold, evidence rows such as annual-leave brackets are returned together rather than stopping at the first matching duration. This keeps acronym/program and policy-duration answers grounded and readable without adding customer-phrase-specific exceptions.

> **Update Note (2026-05-26):** Public Demo Chat RAG focused evidence now prioritizes high-confidence current-campus/location evidence before wider address scans, and individual evidence queries are bounded so one slow Supabase lexical scan cannot keep a Netlify polling invocation open until the platform timeout. Acronym/academic-unit questions use subject-coverage, initialism, and current-listing evidence rules instead of a customer-question phrase map; SHMYO-style campus retrieval resolves from the current campus listing in under a second locally, and TLT-style internship duration queries can match expanded `Tıbbi Laboratuvar Teknikleri` evidence through both retrieval and extractive no-information repair before source-linking runs.

> **Update Note (2026-05-26):** Public Demo Chat polling now has a deterministic Knowledge Base fast path for grounded extractive answers before it falls back to the full inbound AI pipeline. If focused evidence retrieval or broad Knowledge Base search can repair a no-information seed into a factual answer with a canonical source link, the polling `GET` persists that bot reply directly against the demo message id with an idempotency check; if not, the shared pipeline remains the fallback. This keeps the improvement general to RAG evidence/repair behavior rather than tied to a specific customer phrase, and avoids Netlify 30s polling failures for clear PDF/crawl-backed facts.

> **Update Note (2026-05-26):** RAG demo polish now lets the LLM add one short engagement question only when it is topic-related and teaches or offers an adjacent grounded detail from the same source context without assuming the user is a student, applicant, personnel member, or admin. Generic assistant closing tails, role-assumptive department/status prompts, source-link preface tails, and vague "more info / contact the department" deferrals are stripped from otherwise grounded Knowledge Base answers. Policy-duration repair now extracts matching list/bullet evidence such as annual-leave brackets without letting generic eligibility repair overwrite correct duration answers, and standalone user questions are searched before drifted router rewrites. Vector-search timeout responses are treated as recoverable warnings while lexical/focused evidence continues, retrieval-only query planning now searches the original query before spending planner latency, focused evidence probes are scoped by query intent, planned semantic variants are skipped when the original query already returns enough strong grounded evidence, Demo Chat POSTs return pending immediately, and clear address/location answers can be sent through grounded extractive repair before completion-model latency. The latest stricter live YİÜ challenge, including generic/role-assumptive closing checks, passed 33/33 at `tmp/crawl-output/live-yiu-challenge-pipeline-qa-2026-05-26T18-28-49-968Z.md`.

> **Update Note (2026-05-26):** RAG answer quality now treats "answer if the evidence exists" as the default for university demos: informal Erasmus eligibility questions can retrieve the governing denial evidence instead of a coordinator link, unit contact questions such as library e-mail prefer the exact root contact-table value over unrelated staff e-mails, and factual policy answers are sanitized to remove unnecessary deferral/source-link tails when contact or a link was not requested.

> **Update Note (2026-05-26):** Public demo readiness now includes a repeatable public Demo Chat canary with conversation/source trace metadata, a YİÜ corpus-health report, and a 33-question customer-screenshot-plus-variant challenge pass before asking stakeholders to retest. RAG hardening remains general: missed-exam health/rapor queries prefer remedy evidence over generic attendance text, final/bütünleme contradictions are repaired from eligibility language, elective-count policy answers repair truncated evidence, internship evidence is scoped to the academic unit/program in the question so one program's `yaz stajı` text cannot answer another unit's query, source links are ranked by concrete answer evidence, and RAG replies continue even if usage-recording writes fail. The demo org received a 10,000-credit top-up for retesting, existing broken URL text in the YİÜ corpus was cleaned to zero findings, future chunking normalizes URL line-break artifacts before indexing, keyword extraction now preserves high-signal subject terms that appear after conversational prefixes, Supabase trigram/scoped indexes speed up large-corpus lexical fallback searches, and a feature-flagged retrieval-only LLM query planner can add semantic search variants without customer-specific phrases. The latest live challenge report passed 33/33 at `tmp/crawl-output/live-yiu-challenge-pipeline-qa-2026-05-26T11-06-11-285Z.md`.
...
> **Update Note (2026-05-25):** Yüksek İhtisas demo RAG now uses general evidence-first retrieval and answer validation instead of screenshot-specific patches. Focused/lexical evidence can answer before broad vector search, RAG completion timeouts recover from selected chunks before generic fallback, eligibility/permission answers are checked against contradictory source evidence, official source terminology is preserved, and the latest 33-question live screenshot-plus-variant challenge passed 33/33 (`tmp/crawl-output/live-yiu-challenge-pipeline-qa-2026-05-25T19-20-23-015Z.md`).
...
> **Update Note (2026-05-21):** Policy-duration retrieval should run before expensive embedding/vector work when the user asks a specific duration or limit question. If token-bound policy evidence already includes the distinctive subject modifier and an explicit duration, the runtime can answer from that evidence immediately instead of spending the demo request budget on broad vector and lexical fallbacks.
...
> **Update Note (2026-05-21):** Public Demo Chat must recover from deploy environments where deferred `after()` work does not finish after a `202 pending` response. Polling `GET` requests now can re-run the shared inbound AI pipeline for the original demo message id without duplicating the stored contact message, and the demo shell keeps header plus composer fixed while only the conversation timeline scrolls on mobile and desktop.

> **Update Note (2026-05-21):** University-pilot RAG must handle PDF policy duration questions as a general evidence pattern, not a one-question exception. Retrieval now boosts chunks whose subject terms match the question and whose policy text contains an explicit duration/limit value, demotes broad semantically related pages with poor subject coverage, and repairs model no-information responses from retrieved duration sentences such as unpaid leave limits or mazeret-exam application windows.
...
> **Update Note (2026-05-21):** Live QA cleanup now also covers earlier RAG-test conversations that appear in Inbox as `Codex Live QA` with `codex-live-rag-qa-` phone prefixes, so lingering Codex test traffic can be removed from pilot/demo organizations without widening deletion beyond deterministic QA markers.

> **Update Note (2026-05-20):** Public Demo Chat sessions now stay browser-local per demo slug: a refresh keeps the same session and local message history, while the header reset action clears local messages and creates a new session so operators see the next test as a separate Inbox conversation. Demo Chat no longer appends the AI-disclaimer footer under every bot answer; the disclosure moved to the composer area, the input is vertically centered, loading copy rotates more slowly with a pulsing status dot, and `demo_chat` output preserves markdown emphasis plus repaired raw links for browser/operator rendering.

> **Update Note (2026-05-20):** Public university demo chat should read as a reusable demo assistant, not only an admissions chat. The Yüksek İhtisas demo now uses the university logo asset, a neutral `Üniversite demo asistanı` subtitle, a non-message demo information panel with the `www.askqualy.com` promotion link, clickable links and WhatsApp-style bold, a light/dark header switch, mobile-friendly widths, plain unframed assistant answers with a top-to-bottom reveal animation, clearer dark mobile contrast, and rotating friendly loading copy while the AI response is being prepared.
...
> **Update Note (2026-05-19):** Knowledge Base retrieval now supports large crawl/PDF corpora with vector search plus bounded keyword/title fallback, so exact mevzuat/yönerge questions can find early PDF chunks for Senato decisions, scope, document numbers, and AKTS definitions instead of drifting to broad semantic neighbors.

> **Update Note (2026-05-19):** Sidebar credit balance and `Settings > Usage` must reconcile after the text-embedding pricing correction. Existing current-period embedding debits that were charged with the old chat-token formula now get an idempotent account/ledger correction: billing accounts receive the overcharged credits back, a localized `Bilgi Bankası indeksleme kredi düzeltmesi` adjustment appears in credit history, and sidebar remaining credits read from the corrected account balance.
...
> **Update Note (2026-05-15):** Signup email-confirmation routing is hardened: `/login?code=...` now hands off to a server-side Supabase auth callback, exchanges the code into a session, and redirects through the standard post-auth onboarding/workspace resolver. Successful auth must remain usable even when optional post-auth onboarding/workspace lookups fail; those failures log and fall back to `/inbox` instead of surfacing a login POST 500. Supabase client methods such as `rpc` must keep their client context so billing/onboarding reads do not crash with `Cannot read properties of undefined (reading 'rest')`.
...
> **Update Note (2026-04-30):** App entry performance now avoids an extra onboarding-state read on the root locale route before redirecting authenticated operators into the workspace. The dashboard root loading boundary also uses a branded Qualy opening state with localized copy, logo motion, and a slim progress bar so unavoidable server-render waits do not look like a broken app.
...
> **Update Note (2026-04-24):** `Settings > Usage` credit history now supports current-month, previous-month, and all-time exploration with lazy-loaded ledger pages. Operators can filter all movements, usage debits, or positive credit loads; see package grants and extra-credit purchases alongside AI usage debits; switch between record-level and daily/weekly/monthly aggregate views; load aggregate history in complete windows (10 days, 3 weeks, or 3 months); and scan a cleaner labeled filter row with period, movement, and view controls side by side on desktop.

> **Update Note (2026-04-24):** Runtime bot-mode reads now fail closed on customer-facing webhook paths. If organization AI settings cannot be loaded or are unexpectedly missing during WhatsApp/Instagram shared inbound processing or Telegram processing, effective bot mode resolves to `Kapalı`, preventing transient settings-read failures from bypassing the `Dinleyici` no-reply guarantee.
...
> **Update Note (2026-04-07):** Existing Skill image-only edits no longer create Organization Detail AI suggestions. The server now compares stored Skill title/response/triggers with the submitted values before refreshing embeddings or proposing offering-profile/service-list/required-field updates, so adding or replacing only the optional image is treated as media maintenance instead of a service-scope change.

> **Update Note (2026-04-07):** Instagram skill-image delivery and Inbox echo handling are now hardened together. AI bot replies persist provider `instagram_message_id` metadata for text and image sends so webhook echo events can dedupe correctly, new skill uploads are normalized to stored JPEG files instead of WebP, older WebP skill images are still proxied through a public JPEG conversion route before Instagram delivery, and Inbox hides duplicated `instagram_is_echo` rows both when a matching bot provider id exists and when older polluted rows repeat the same AI-disclaimer content within the same short reply window, so legacy threads stop showing double bubbles and mixed avatars too.

> **Update Note (2026-04-07):** Freeform Knowledge Base authoring now includes a persistent `Qualy AI ile doldur` helper banner above the title field. The helper uses the filled sidebar AI icon inside a softer colorful card, keeps the CTA stacked under the copy, opens a reusable mobile-friendly bottom-sheet modal, renames the guidance field to `Yapay zeka asistanının dikkat etmesi gerekenler`, shows an explicit in-modal loading state while drafting, and still replaces the content body on each re-run while leaving a non-empty title untouched. Usage continues to roll into the existing `İçerik işleme / Content processing` billing breakdown instead of a new visible bucket.

> **Update Note (2026-04-07):** Offering-profile suggestion generation is now hardened for AI-filled Knowledge Base saves. Automatic/manual regeneration must use real ready Knowledge Base body content and recent Skill guidance instead of title-only summaries, retry one stricter second repair when the first AI draft is still a single paragraph, and fail loudly on suggestion insert errors so the pending-suggestions banner in `Knowledge` and `Settings > Organization` remains reliable.

> **Update Note (2026-04-07):** The `Knowledge` suggestion helper now stays visible during extraction too. While any organization Knowledge Base document is still `processing`, the page shows a wait-state banner (`Hizmet profili önerileri hazırlanıyor`); once processing completes, it can switch to the existing ready-state `İncele` banner.

> **Update Note (2026-04-07):** Pending Meta channel UX must now tell the operator exactly what to do: after connecting WhatsApp or Instagram, send a test message to the connected account and Qualy will automatically verify the connection when that first inbound message arrives. Pending gallery/onboarding surfaces should refresh when the operator returns focus to the app, onboarding setup pages should show the connected account explicitly, keep the lead paragraph short to avoid duplicated instructions, and reuse the same detailed action copy only inside a full-width warning helper with an `Aksiyon gerekli / Action required` title. Bot mode must still remain an explicit, neutral operator choice after verification instead of auto-switching to `Dinleyici`.

> **Update Note (2026-04-05):** The Skills action-button editor is now temporarily hidden in the Skills UI until its editor-side regression coverage is restored. Existing `skill_actions` data and WhatsApp runtime handling remain intact; only the operator-facing editor controls are suppressed.

> **Update Note (2026-04-06):** Skills now support one optional uploaded image per skill. Operators can upload up to `5 MB`; the client normalizes the asset to flattened `JPEG` (target quality `92`, never below `90`, long edge `1600px`) before storage, the Skills editor shows a loading/disabled state while the image is processing or uploading, matched skills send text first and image second across WhatsApp/Instagram/Telegram, the Simulator mirrors the same sequence, and `requires_human_handover` still escalates after the image attempt while media failures stay visible to operators and do not block the text reply.

> **Update Note (2026-04-06):** Skill-image delivery now enforces active-organization ownership for storage upload/delete actions, uses unique upload object keys even for same-second edits, and keeps failed skill-image sends operator-visible in Inbox by falling back to the failure notice text instead of rendering a normal image bubble.

> **Update Note (2026-04-06):** `Settings > Qualy AI > Behavior and Logic` now replaces the raw `AI Prompt` field with structured `Yapay zeka asistan talimatları`: four optional instruction fields, locale-aware sector-agnostic starter content, legacy custom-prompt carryover into `Diğer talimatlar`, and a mobile-friendly `Nasıl çalışır?` explainer modal with an inner scroll area, bullet-based examples, explicit dialog semantics, Turkish product copy that avoids English pipeline jargon, and a low-emphasis text-link help action placed directly under the section description. Customer-facing replies use the compiled instructions, while internal extraction/scoring/routing/summary prompts remain unchanged.

> **Update Note (2026-04-04):** Default guardrail skills must be provisioned when a workspace gets its owner membership, not lazily on the first `Skills` page visit. New organizations now seed localized default skills from signup locale when available, fall back to billing-region language when it is not, and backfill existing organizations that were created before create-time seeding existed.

> **Update Note (2026-04-04):** Dashboard chunk-mismatch recovery must retry the intended destination, not reload the currently visible page. If a sidebar/mobile/settings navigation hits a real Next chunk failure during transition, the recovery path should preserve the pending route so operators do not see a misleading "same page reloaded" loop.

> **Update Note (2026-04-04):** `Settings > Plans > Tek Seferlik Ek Kredi` outer card now keeps only the short one-time-purchase explainer plus CTA. VAT/conversation-range disclaimer copy remains in the purchase modal and is no longer duplicated on the card itself.

> **Update Note (2026-04-03):** `/calendar` and `/simulator` now introduce themselves with a one-time, mobile-friendly educational modal for first entry. The modal reuses onboarding-style icon rows, keeps the description row text-only without a separate hero icon, uses direct `sen` tone in Turkish, gives one relevant next-step CTA, and stays dismissed per browser-user-workspace-surface after the operator closes it.

> **Update Note (2026-04-03):** In `Settings > Plans`, the active monthly-package status card should label the next package cycle as `Yenileme / Renews at`, not `Sıfırlanma / Resets at`, so the renewal timing reads like billing language instead of a destructive reset.

> **Update Note (2026-04-03):** One-time extra-credit checkout must follow the same in-app hosted Iyzico experience as subscription checkout. `Settings > Plans` top-up purchases should open on a local embedded checkout route instead of sending the operator to a separate full-page provider URL, reducing auth-context loss and keeping the purchase flow visually consistent.

> **Update Note (2026-04-03):** In-app billing price surfaces must now explicitly state that displayed subscription and one-time extra-credit amounts include VAT/KDV, followed immediately by the same conversation-range variability note used on the landing pricing section. This applies to unsubscribed plan cards, active-subscriber package management UI, and extra-credit purchase surfaces in `Settings > Plans`.
...
> **Update Note (2026-04-03):** Legal and billing policy wording must now distinguish monthly package credits from one-time extra credits. Monthly package credits remain non-transferable, non-cash-value, active-subscription-only, and non-rollover; one-time extra credits remain non-transferable, non-cash-value, and active-subscription-only, but persist across billing periods until consumed while the paid subscription stays active.
...
>
> **Update Note (2026-04-02):** Active monthly-package UI is now consolidated inside `Settings > Plans > Mevcut plan`. When a workspace already has a managed monthly subscription, the old top `Plan durumu` membership card disappears, the current monthly package card takes its place, `Yükselt veya değiştir / Ödeme yöntemini güncelle` render side by side as text actions inside that same card, the duplicate `Aylık paketler` section disappears, and `Fatura bilgileri` moves to the bottom of the page with no repeated inner title/description wrapper.
>
...
>
> **Update Note (2026-04-02):** `Settings > Plans` subscription-management actions now use softer hierarchy. `Aboneliği iptal et` is no longer a sibling button beside the main upgrade/switch CTA; it now lives as low-emphasis helper text inside the manage-plan modal, while payment-recovery actions use lighter text-style links instead of stacked outline buttons.
>
> **Update Note (2026-04-02):** In the plan-selection modal, downgrade CTAs now keep the same clean `Select this plan and continue` button copy as other selectable plans. The timing caveat stays below the button as an amber helper line instead of being embedded into the CTA label.
>
...
>
> **Update Note (2026-04-01):** The final `Başlangıç` channel-connection step now has a hard prerequisite: it stays incomplete until the first four onboarding steps are complete, even if the workspace connected a live channel earlier. Once those four steps are done, any already-ready live channel can immediately satisfy the final step and trigger the existing bot-mode choice modal instead of auto-activating the bot.
>
> **Update Note (2026-04-01):** `Settings > Organization` now surfaces a localized non-blocking AI extraction banner whenever Knowledge Base document-derived business setup is still processing. The page keeps manual editing available, explains that service profile / required info / service list extraction is still running, and auto-refreshes when processing finishes.
>
> **Update Note (2026-04-01):** Trial-active workspaces now show a cached global onboarding banner with localized `Getting started` / `Başlangıç` and `Upgrade now` CTAs. The getting-started CTA disappears once onboarding is complete, but the banner itself remains until the workspace upgrades or trial ends.
>
> **Update Note (2026-04-01):** New workspaces now get an organization-scoped onboarding checklist: it auto-opens once on first dashboard entry, keeps the next incomplete step expanded by default only on entry, no longer forces that step back open after the user manually collapses it, still allows completed steps to be reopened manually, stays accessible as a highlighted purple `Getting started` / `Başlangıç` card above `Workspace` in desktop sidebar and in mobile `Other` navigation while incomplete, greets the user by name with a simple wave emoji and a smaller, more restrained title size, leaves a bit more vertical breathing room below the trial banner, keeps its progress bar and primary actions in the same purple onboarding theme, uses a cleaner respond.io-style left-aligned accordion layout with single-column stacked step bodies, removes the intro chips, uses more compact filled helper icons plus outline/filled step-state icons, centers single-line helper rows vertically, localizes `Bilgi Bankası` / `Yetenekler` terminology, keeps the intro copy business-focused instead of team-focused while also mentioning appointment flow, reframes `Yetenekler` around low-credit deterministic replies that the user defines directly, reuses the same `Bilgi Bankası` / `Yetenekler` icons already used in sidebar navigation, and now explicitly says that business profile / required info / service list are extracted automatically by AI from Knowledge Base documents but can still be entered or adjusted manually; the `Yapay zeka asistanını hazırlayın` step now completes when the workspace has at least one Knowledge Base document or at least one non-default custom skill, the business-review CTA opens `Settings > Organization > Organization Details` directly, there is now a dedicated persisted `Qualy AI ayarlarını gözden geçirin` onboarding step, it also shows available channel options on the final step with direct larger icons and `Messenger` marked as coming soon, avoids duplicating the same progress ratio twice inside the sidebar card, and now treats any ready live channel as completion for the final connection step instead of assuming WhatsApp only.
>
> **Update Note (2026-04-01):** Onboarding intro actions now fail softly when the `organization_onboarding_states` relation is temporarily unavailable instead of crashing the dashboard, and the intro CTA opens the next checklist step immediately before the shell refresh syncs progress/sidebar/banner state.
>
> **Update Note (2026-04-01):** After the first-ever Knowledge Base document is added, the create flow now pauses on a one-time educational modal instead of redirecting immediately. The modal explains that Qualy AI will automatically extract the business profile, required customer fields, and starter service-list structure from the uploaded content, while offering quick routes back to `Başlangıç` or directly into `Settings > Organization > Organization Details` without falsely completing the business-review onboarding step.
>
> **Update Note (2026-04-01):** The `Qualy AI ayarlarını gözden geçirin` step now completes immediately when its CTA is used, stays complete after navigating away and back, and also falls back to a browser cookie when the `ai_settings_reviewed_at` column is not yet available in the deployed schema so the onboarding shell does not regress on partial rollouts.
>
> **Update Note (2026-04-01):** Newly created workspaces now start with bot mode effectively locked to `Kapalı`. Until `Başlangıç` reaches `5/5`, both the desktop sidebar quick switch and `Settings > Qualy AI > General` keep bot mode disabled with onboarding-required helper copy, and runtime still enforces `Kapalı` even if a stale client tries to switch it. When onboarding reaches `5/5`, Qualy shows a mandatory completion modal that lets the operator explicitly choose `Dinleyici`, `Aktif`, or keep it `Kapalı`; existing workspaces are backfilled unlocked and are not force-switched off.
>
> **Update Note (2026-04-01):** Bot-mode lock UI now also respects live onboarding completion state instead of relying only on the stored unlock flag. If `Başlangıç` is still incomplete (for example `4/5`), both sidebar quick switch and `Settings > Qualy AI` render as locked, force the effective mode to `Kapalı`, and show progress-aware helper text that says the control opens at `5/5`.
>
> **Update Note (2026-04-01):** Bot-mode lock messaging is now simplified to one bold onboarding-required banner in both sidebar quick switch and `Settings > Qualy AI > General`. Locked option cards no longer keep hover affordances, the duplicate helper line inside the sidebar popup is removed, and the popup lock icon is vertically centered beside the message.
>
> **Update Note (2026-04-01):** The `Qualy AI ayarlarını gözden geçirin` onboarding step no longer tells operators to review bot mode while onboarding is still incomplete. Its copy now focuses on bot-name personalization, reply behavior, and human handoff instead.
>
> **Update Note (2026-04-01):** WhatsApp disconnect UX copy now explicitly tells operators that `Disconnect` removes only Qualy's local channel relationship and does not unlink the customer's WhatsApp Business / Meta setup.
>
> **Update Note (2026-04-01):** WhatsApp `Disconnect` now removes only Qualy's local channel relationship. Operators should not be forced to disconnect their WhatsApp Business app or provider-side WABA setup just to stop using the channel inside Qualy.
>
> **Update Note (2026-03-31):** WhatsApp template UX now distinguishes Meta WABA templates from Qualy's regular saved messages. The channel-manage action is labeled `WhatsApp templates`, the main modal stays focused on that distinction, the `How it works` guide carries the WABA requirement + Meta overview link, and closing that nested guide now dismisses only the guide.
>
> **Update Note (2026-03-31):** WhatsApp onboarding now puts the WhatsApp Business app / coexistence path first, selects it by default, simplifies the option copy, explicitly expands `WABA`, and marks that path as recommended for the common SMB case.
> **Update Note (2026-03-31):** Pending WhatsApp channels now support non-destructive webhook recovery. `Settings > Channels > WhatsApp` can request webhook subscription again without disconnecting the channel, and the first valid inbound WhatsApp webhook POST now flips the channel to verified automatically so locally pending rows self-heal as soon as real delivery starts.

> **Update Note (2026-04-02):** Live Iyzico billing now supports self-serve recovery for `past_due` subscriptions. `Settings > Plans` exposes `Update payment method` and `Retry payment`, card changes run through a hosted Iyzico card-update form embedded in-app, failed-renewal order references stay persisted for later retry, and access remains locked until provider confirmation arrives through webhook sync.
...
> **Update Note (2026-03-31):** WhatsApp disconnect failures now stay local and operator-friendly. If local channel removal fails, the page should render a localized Qualy-side error instead of a generic action-level failure.
...
> **Update Note (2026-03-31):** Sign Up confirm-email completion now returns client-consumable redirect state from the register server action, so successful no-session signups reliably transition from the register form to `/register/check-email` instead of leaving the user on the same page.
> **Update Note (2026-03-30):** WhatsApp existing-number Embedded Signup completion is now mode-aware. The UI passes `new` vs `existing` into the server action, `Business app number` onboarding skips Cloud API phone registration plus two-step PIN persistence, and only the real new-number path performs `/{phone_number_id}/register`.
> **Update Note (2026-03-30):** WhatsApp Embedded Signup mode propagation is now regression-guarded at both client entry points, and invalid mode payloads return a dedicated server validation error for easier troubleshooting.
> **Update Note (2026-03-30):** WhatsApp onboarding support now opens the team WhatsApp chat directly instead of a mail draft, and the landing-card copy explicitly tells personal-account users to start from the `new API account` path while keeping the `Business app number` path reserved for existing WhatsApp Business app numbers.
> **Update Note (2026-03-30):** WhatsApp onboarding support now opens the team WhatsApp chat directly instead of a mail draft, and the landing-card copy explicitly tells personal-account users to start from the `new API account` path while keeping the `Business app number` path reserved for existing WhatsApp Business app numbers.
> **Update Note (2026-03-29):** Live Iyzico plan downgrades now need provider-backed `next period` scheduling plus local `pending plan` persistence on the active subscription row so `Settings > Plans` can still show the upcoming package and effective date after a later revisit. Plans copy must also avoid promising an in-app auto-renew resume path after provider-backed cancellation. Active premium plan-change popups must likewise distinguish direct subscription actions from hosted checkout, show current vs target monthly pricing plus effect timing, and avoid quoting an exact upgrade charge unless the integration can preview it reliably.
> **Update Note (2026-03-29):** Inbox browser-tab unread indicator now updates from live inbox unread state snapshots instead of waiting for a manual refresh or delayed database roundtrip. New inbound realtime messages publish an immediate unread-state event so the tab title dot stays in sync while the operator is already inside Inbox.
> **Update Note (2026-03-29):** Inbox selected thread now self-refreshes when a conversation realtime update proves the list preview has advanced beyond the loaded timeline. This closes the gap where the sidebar row could show the new inbound text before the open conversation panel appended it.
> **Update Note (2026-03-29):** Instagram deleted-message handling now suppresses deleted-only Inbox noise end to end. If the first or only customer DM is later deleted, Inbox should not keep a visible thread at all; established conversations may still show the deleted state without treating it like a fresh unread inbound turn, Inbox list fallback hides older deleted-only Instagram rows whose sole preview message is that deleted event, and those stale rows no longer leak into queue counts or Inbox/browser unread indicators. Unresolved IG identities still use a localized generic contact label instead of raw numeric ids.
> **Update Note (2026-03-28):** Inbox realtime now self-recovers after long idle tabs. Browser resume events (`visibilitychange`, `focus`, `pageshow`, `online`) rebuild Supabase subscriptions and reconcile the first conversation page plus the selected thread, and broken channel states (`CHANNEL_ERROR`, `TIMED_OUT`, `CLOSED`) trigger the same self-healing path so operators no longer need a manual refresh to see queued messages.
> **Update Note (2026-03-28):** Instagram Inbox reaction events now persist structured reaction metadata, parse legacy raw `[Instagram reaction] ...` rows, and render as compact reaction rows with target-message-aware wording when the reacted provider message id resolves to a known outbound turn.
> **Update Note (2026-03-28):** Instagram reaction labels in Inbox now read ICU translation templates via raw i18n access and inject the emoji at render time, preventing `FORMATTING_ERROR` crashes when localized strings intentionally keep an `{emoji}` placeholder.
> **Update Note (2026-03-28):** Lead extraction now emits a sector-agnostic semantic `intent_stage` (`none`, `informational_commercial`, `qualification`, `booking_ready`) so first-message commercial inquiries can rise to `warm` without relying on static language-specific pricing keywords, while `hot` still requires stronger qualification evidence.
> **Update Note (2026-03-27):** AI QA Lab consistency cleanup now recognizes normalized clear-summary phrasing and English `budget` cues, so stale did-not-ask/follow-up/missing-field claims no longer survive after transcript or intake-coverage clears; low-information placeholder penalties still apply to abstract filler while grounded general-information offer copy stays safe.
> **Update Note (2026-03-27):** Lead extraction now recovers the canonical catalog service from current conversation plus summary evidence when raw `service_type/services` come back empty, required-intake repair scans recent customer turns for category-compatible answers including relative timeline phrasing such as `1 ay içinde`, and lead scoring/status are now recalibrated from merged extraction evidence so later acknowledgement turns cannot collapse an already-qualified lead back to `cold/0`.
> **Update Note (2026-03-27):** Turkish `Settings > Applications` calendar copy now stays terminology-consistent by using `Google Takvim` inside the descriptive paragraph as well, instead of mixing `Google Calendar` into the same sentence.
> **Update Note (2026-03-27):** `Settings > Applications` now keeps Google Calendar in a passive coming-soon state on the UI: Turkish copy uses `Google Takvim` plus a non-clickable `Yakında` badge, and both `Settings > Calendar` and `Settings > Applications` disable their save CTA until the active tab/surface has a real unsaved change.
> **Update Note (2026-03-27):** `Settings > Plans` now keeps the one-time extra-credit purchase as the primary expansion action: the custom-package contact card sits directly beneath it and uses a quieter secondary CTA treatment.
> **Update Note (2026-03-27):** Supabase migration version prefixes are now kept globally unique again. The duplicate March 27 premium debit-order rollout was collapsed into the later `00102_reassert_premium_usage_debits_topup_first.sql` migration so remote `schema_migrations` no longer fails on duplicate `00099`.
> **Update Note (2026-03-27):** `Settings > Plans` extra-credit balance now keeps the original trial limit visible when premium extra credits come from trial carryover. A remaining balance like `153.5` should therefore render against `200`, not collapse to `153.5 / 153.5`.
> **Update Note (2026-03-27):** Desktop main sidebar now uses a lighter Linear-style section IA: category groups have more vertical separation, each expanded-sidebar section can be collapsed from its own header, active items use a softer pill treatment, and desktop nav typography is slightly smaller for a calmer shell.
> **Update Note (2026-03-27):** Settings-to-settings navigation now keeps the shell visible end to end: the dashboard-wide fullscreen pending overlay no longer paints over settings routes, the settings loading state is scoped to the detail pane, and billing-lock nav state hydrates on the client so the server layout no longer waits on billing snapshot reads.
> **Update Note (2026-03-27):** Heavy settings detail routes now stream behind local `Suspense` boundaries. `Settings > Billing` and `Settings > Plans` should paint their page chrome before the slowest data blocks resolve, `Settings > Organization` now streams through a dedicated async content loader, and the plans screen no longer blocks GET renders on `billing_region` persistence because that write runs via `after()` after the response is sent.
> **Update Note (2026-03-27):** Settings bundle trimming now lazy-loads low-frequency UI trees. `Settings > Organization` keeps the default tab light by deferring the organization-details tab sections until opened, and `Settings > Plans` defers checkout legal-consent modal code until an operator actually opens a plan/top-up checkout flow.
...
> **Update Note (2026-03-27):** Inbox template picker now keeps first-load empty panes in a dedicated loading state with skeleton feedback. Operators should see `templates are loading` copy until the fetch resolves; the `no templates yet` warning appears only after the initial request completes with zero results.
> **Update Note (2026-03-27):** Inbox selected-thread header now has a manual read/unread toggle. Threads marked unread from the header no longer fall back to the existing switch-away auto-read path; they stay unread until manual re-read or a later revisit from another conversation.
> **Update Note (2026-03-27):** React state-sync anti-patterns were removed from the owned modal components (`FolderModal`, `SubscriptionPlanCatalog`, `WhatsAppTemplateModal`, and `TemplatePickerModal`), with focused regression tests added to guard against render-time and synchronous effect-driven state mirroring.
> **Update Note (2026-03-27):** Cross-surface best-practices remediation now also hardens semantic navigation and shared runtime safety: Inbox attachment chips, Knowledge cards/table rows, and Skill cards use valid button/link structure; shared primitives plus calendar/settings/mobile-nav/auth flows avoid synchronous effect-driven state mirroring; route-prefetch inputs stay memoized; and channel webhook lookups now use dedicated JSONB indexes.
> **Update Note (2026-03-27):** Shared modal chrome now depends on `common.close` in both locale dictionaries. EN/TR common message bundles must keep that key present so client-rendered dialogs do not throw `MISSING_MESSAGE` during render and cascade into noisy Turbopack/React dev overlay errors.
> **Update Note (2026-03-26):** Inbox bot labels now distinguish default vs customized names: the stored default `Bot` should still render as the generic assistant label in mobile/desktop header and details, while user-defined names are shown verbatim.
> **Update Note (2026-03-26):** Inbox mobile composer now keeps template/send actions as icon-only buttons on phone widths so the reply input can occupy the remaining horizontal space without action-label overflow.
> **Update Note (2026-03-26):** Inbox selected-thread cache reuse is now idempotent: once a conversation is already loaded, sidebar/list rerenders no longer re-apply the cached payload and trigger React `Maximum update depth exceeded` loops through tag/media subtrees. Shared dropdowns also suppress duplicate open-state callbacks so the sidebar bot-mode menu does not churn on redundant close events.
> **Update Note (2026-03-26):** Required-intake repair now enforces category-compatible values before accepting AI-collected fields, then uses concept-linked sibling evidence for affirmative status inference only when the fields are actually related. This fixes duplicated wrong-value fills such as copying a due-date answer into a status field while still allowing the related status to normalize to `Evet`.
> **Update Note (2026-03-26):** Inbox thread selection now invalidates stale empty cache entries when the sidebar already has a non-seen preview message for that conversation. Warmed client caches can no longer win over visible preview evidence and open the thread as blank.
> **Update Note (2026-03-26):** Inbox thread cache writes are now guarded to the active loaded conversation. Switching conversations no longer lets the transient empty reset state wipe a previously warmed thread cache, which had caused list previews to remain visible while the opened thread rendered blank.
...
> **Last Updated:** 2026-06-20 (Implemented router-level early clarification with pending slot binding and documented the measured 2.22x OpenAI cost increase plus evaluation budget gates.)
...
> **Update Note (2026-03-26):** Inbox should inline-preview inbound media whenever the stored asset clearly resolves as an image (for example `image/*` MIME type or image-like stored URL/filename), even if the provider-level message type was persisted as `document` or `unknown`; only non-image assets should keep the explicit `Dosyayı aç / Open file` fallback card.
> **Update Note (2026-03-26):** Instagram `lookaside.fbsbx.com/ig_messaging_cdn` preview URLs must be treated as image-like remote assets during both webhook ingest and Inbox rendering so shared/story attachments with extensionless Meta CDN links still open inline instead of falling back to `Dosyayı aç / Open file`.
...
> **Update Note (2026-03-26):** Skills list loads now read existing skills first, avoid embedding-maintenance work on the request path, and no longer depend on first-visit default-skill seeding as the primary provisioning path. Localized default guardrail skills are provisioned at owner-membership time and backfilled for older workspaces, while lazy empty-workspace seeding remains only as a last-resort safety net. Knowledge folder counts now use an aggregate RPC path instead of scanning `knowledge_documents` rows on page load.
...
- [x] **WhatsApp (Meta Cloud API)**
  ...
  - [x] Inline-preview inbound image-like file messages in Inbox when the persisted asset resolves as an image via MIME type or stored URL/filename hints, even if the provider message type is `document`/`unknown`
  ...
  - [x] Keep WhatsApp disconnect local-only so `Settings > Channels > WhatsApp` removes the Qualy channel without forcing provider-side deregistration or WhatsApp Business app disconnect steps
  - [x] Clarify WhatsApp disconnect copy so operators are told explicitly that only the Qualy-side channel relationship is removed
  - [x] Let pending WhatsApp channels recover without disconnecting by retrying webhook subscription from Settings and auto-marking the channel verified on the first valid inbound webhook POST
  - [x] Rename channel-side template tooling to `WhatsApp templates`, keep WABA guidance inside the nested `How it works` guide, and ensure closing that guide does not dismiss the parent template modal
  - [x] Reorder WhatsApp onboarding so the Business app / coexistence path is first and default, with simplified copy and a recommended chip for the common SMB case
- [x] **Telegram (Sandbox)**
...
- [x] **Instagram (Meta Messaging)**
  ...
  - [x] Instagram channel card remains interactive for non-connected orgs and routes directly into its onboarding flow from the gallery
...
- [x] **Channels Discovery UX**
  - [x] Redesign Settings > Channels into a responsive business-messaging gallery with calmer card tones, direct grid layout, full-width page usage, and a maximum of 3 columns on wide desktop
  - [x] Promote WhatsApp as the primary CTA surface and keep gallery order fixed to `WhatsApp`, `Instagram`, `Facebook Messenger`, `Telegram`
  ...
  - [x] Persist Instagram reaction metadata (`action`, `emoji`, target provider message id) and render reaction events in Inbox as compact reaction rows instead of raw `react ❤️` message bubbles
  ...
  - [x] Suppress deleted-only Instagram Inbox noise so a first/only customer DM that is later deleted does not keep a visible thread, while established conversations still show deleted-state history with localized unresolved-contact fallback instead of raw numeric IG ids
  ...
  - [x] Refine onboarding detail readability and support CTA: smaller inner headings/body text, visible wizard step numbers in selected state, support banner `Ekiple konuşun` WhatsApp chat action, clarified `new API` copy for personal-account numbers, and temporary no-op migration info link
  - [x] Keep WhatsApp disconnect local-only: remove the Qualy channel record without deregistering Cloud API assets or requiring WhatsApp Business app / WABA disconnect steps
  - [x] Make the WhatsApp disconnect confirm copy state that the action only removes the Qualy-side connection
  - [x] Branch WhatsApp Embedded Signup completion by mode so `existing` / coexistence onboarding skips new-number phone registration and PIN persistence while `new` onboarding keeps the Cloud API registration step
  - [x] Add Meta coexistence launch flag to both WhatsApp embedded-signup client entry points so `existing` onboarding passes `extras.featureType = whatsapp_business_app_onboarding` instead of reopening the generic new-number popup
...
- [x] **Skill CRUD**
  ...
  - [x] Provision localized default guardrail skills at organization owner-membership time and backfill older workspaces that predate create-time seeding
  ...
  - [x] Temporarily hide the Skills action-button editor while preserving stored `skill_actions` data and runtime execution paths
  - [x] Add one optional uploaded image per skill with client-side `JPEG` normalization, loading/disabled editor state, and sequential text-then-image delivery across WhatsApp/Instagram/Telegram
  - [x] Prevent existing Skill image-only edits from triggering Organization Detail AI suggestions or embedding refreshes when title/response/triggers are unchanged
...
- [x] **Simulation Logic**
  ...
  - [x] Mirror skill image replies in the same text-then-image order used by live channel runtimes
...
- [x] **Inbox UI**
  ...
  - [x] Inbox page now bootstraps the first selected conversation from one combined thread payload (`messages + lead`) and keeps a per-conversation client cache so hot thread reopens avoid duplicate client-side fetch waterfalls
  ...
  - [x] Public Demo Chat tester turns now persist to the operator Inbox during POST even when the public API returns pending, and deterministic direct replies persist matching bot messages
  ...
  - [x] Inbox realtime now self-recovers after long idle tabs: browser resume events (`visibilitychange` / `focus` / `pageshow` / `online`) rebuild Supabase subscriptions and reconcile the first conversation page plus selected thread, and broken channel states trigger the same self-healing path
  - [x] Inbox browser-tab unread indicator now syncs from live unread-state events while the operator is already inside Inbox, so new inbound messages can light the tab without waiting for refresh or delayed unread queries
  - [x] Inbox selected thread now refreshes from stale-preview conversation updates, so the open timeline catches up when the list row advances before a message-insert event arrives
  ...
  - [x] Inbox mobile composer now keeps `Şablonlar` and `Gönder` as icon-only controls on phone widths so the reply field keeps the remaining row width
  - [x] Inbox bot labels now show the generic assistant name until the stored default `Bot` is actually customized, keeping mobile and desktop header/details consistent
  - [x] Inbox selected-thread header now includes a read/unread toggle, and manually unread threads stay unread across the normal switch-away auto-read flow until manual re-read or revisit
  - [x] Inbox header utility controls now use plain icon chrome: the list filter trigger and selected-thread read/unread toggle no longer render as bordered pill buttons
  ...
  - [x] Inbox template picker empty panes now keep a dedicated loading/skeleton state on first open so `no templates yet` warnings appear only after the initial fetch resolves
  - [x] Cross-surface best-practices hardening: Inbox attachment chips, Knowledge cards/table rows, and Skill cards now use valid semantic button/link structure; shared primitives plus calendar/settings/mobile-nav/auth flows avoid synchronous effect-driven state mirroring; route-prefetch inputs stay memoized; and active-channel webhook lookups now use dedicated JSONB indexes
...
- [x] **Navigation Shell**
  ...
  - [x] Settings-to-settings transitions now keep the shell visible: dashboard-wide fullscreen pending overlay skips settings routes, settings loading is detail-pane scoped, and billing-lock nav state hydrates client-side instead of blocking the server layout
  - [x] Heavy settings detail pages now stream behind local `Suspense` boundaries, and `Settings > Plans` no longer blocks GET renders on `billing_region` persistence
  - [x] Low-frequency settings tab trees and checkout legal-consent modals now lazy-load with `next/dynamic`
  - [x] Settings shell now reuses warmed in-session detail-pane content on subpage revisits while fresh server payloads stream in
  ...
  - [x] Top-level workspace routes now lazy-load the heavy `Inbox`, `Calendar`, `Leads`, `Skills`, and `Knowledge` client containers, and `/inbox` no longer blocks first paint on a server-side selected-thread bootstrap
  ...
  - [x] Root locale entry now redirects authenticated operators without waiting on onboarding state, and the dashboard root loading boundary now shows a branded localized Qualy progress surface for unavoidable waits
  ...
  - [x] Expanded desktop sidebar now uses Linear-style section headers with per-group collapse/expand state, larger inter-section spacing, softer active pills, and smaller nav typography
  - [x] Desktop dashboard content typography now runs one step denser than before, while the desktop sidebar is compensated upward by one step so navigation remains legible beside the tighter content scale
...
- [x] **KB Query Planning:** Add feature-flagged retrieval-only LLM planning that expands noisy, abbreviated, or policy-like questions into search variants while final answers remain grounded only in retrieved chunks
- [x] **KB Evidence-Enriched Chunking:** Preserve structural sections before token chunking, index markdown table rows and high-signal contact/address/code/value lines as standalone evidence chunks with reusable evidence metadata, and apply the same rule to app reprocessing plus crawler-corpus imports so future PDF uploads do not depend on demo-specific phrase fixes
- [x] **Public Demo RAG Diagnostics + Micro-Polish:** Try context-expanded focused evidence before broad contextual search, persist `rag_diagnostics` timing/search-strategy metadata on demo bot replies, and allow deterministic fast-path answers to expand terse course-code wording only from retrieved source-title initialism evidence
- [x] **Focused RAG Latency Tightening:** Keep deterministic focused fast paths planner-free, try subject-scoped high-signal required evidence before broad lecture-note scans, and skip broad health-policy OR scans when required remedy evidence already supports the answer
- [x] **KB Reindex Tooling:** Add a service-role organization reindex command for rebuilding existing ready documents with current chunk rules without tenant-auth coupling
...
- [x] **KB Mevzuat Retrieval Hardening:** Rank exact document codes, acronym/initialism matches, exact title phrases, and regulation/PDF sources above broad landing pages for large university corpus imports
- [x] **KB RAG Answer Repair:** Repair link-only/partial answers for abbreviation titles, full document numbers, and Senato meeting metadata when the retrieved PDF chunk already contains the evidence
- [x] **YİÜ Live Mevzuat QA:** Add and run a 62-question live inbound-pipeline QA suite against Yüksek İhtisas mevzuat/Tıp Fakültesi PDF sources, with source-title and answer-term checks
- [x] **KB Banner Styling:** Use an amber tone for pending AI suggestion banners
- [x] **KB AI Drafting Helper:** Freeform create page now shows a persistent `Qualy AI ile doldur` helper card above the title field with the filled AI icon, stacked CTA, reusable mobile-friendly modal, explicit loading state during draft generation, content replacement on each re-run, and title fill only when blank
...
- [x] **Bot Mode (Org-Level)**
  ...
  - [x] Runtime AI settings reads fail closed to `Kapalı` on shared WhatsApp/Instagram and Telegram webhook paths when settings cannot be loaded
- [x] **AI Settings:** Always-on flexible mode with a single threshold and structured AI assistant instructions for customer-facing replies
...
- [x] **Channel-Aware Text Formatting:** Normalize outgoing bot text for WhatsApp, Instagram, and Telegram by preserving raw URLs, repairing spaced/newline Markdown and raw URL artifacts, isolating repaired links on their own lines, cleaning unsupported Markdown, and converting inline dash-separated RAG lists into readable message bullets before send/persist
...
- [x] **Knowledge Base RAG Reliability:** Add a reusable clarification gate for under-specified price, contact, location, and generic information turns before no-information fallback, while concrete questions continue through evidence-backed retrieval.
- [x] **Knowledge Base RAG Reliability:** Add strict answer quality mode for the validated File Search demo with question normalization, structured program/faculty catalog answers, deterministic global critic repair/refusal, and sensitive-data/payment/credential safety handling before customer-facing replies.
- [x] **Knowledge Base RAG Reliability:** Add an optional LLM global evaluator for strict validated File Search answers, with pass/repair/clarify/refuse decisions, one bounded evaluator-driven retrieval retry, final deterministic critic validation, and metered evaluator usage.
- [x] **Knowledge Base RAG Reliability:** Add Codex-like research-plan/tool-router diagnostics and a strict claim ledger so validated File Search records route/tool/evidence expectations, rejects unsupported exact claims, and preserves generation/evaluator usage through repair/refusal paths.
- [x] **Knowledge Base RAG Reliability:** Harden strict validated File Search after the 508-question YİÜ customer audit by adding hospital/yurt/diploma/table guard facts and rerouting broad taban-puan/başarı-sırası questions to brochure tables; the original 106 low-score questions now retest at `6-9/10`.
- [x] **Knowledge Base RAG Reliability:** Extend the strict structured facts catalog for clinical/admissions ambiguity by guarding affiliated-hospital existence, clarifying broad staj/clinical-practice questions by program, clarifying personal admissions-chance questions by program plus score/rank, and rerunning the original 92 mid-score questions to `6-9/10`.
...
- [x] **Offering Profile (Catalog Optional)**
  ...
  - [x] Knowledge Base banner also shows a processing-state “suggestions are being prepared” helper while extraction is still running
  ...
  - [x] Organization settings now show a non-blocking AI extraction banner while Knowledge Base-derived business setup is still processing and auto-refresh when extraction completes
...
- [x] Offering profile batch/manual regeneration now uses real Skill + ready Knowledge Base body content, retries one stricter second repair when drafts still collapse into a paragraph, and fails loudly on suggestion insert errors so pending suggestion banners can appear reliably after AI-filled saves
...
- [x] **AI Extraction**
  ...
  - [x] Preserve caption/content from media-backed inbound turns in extraction context while excluding pure media placeholders
- [x] **Extraction Reliability**
  ...
  - [x] Use last 5 customer messages for extraction grounding, then recalibrate score/status from merged lead evidence before persistence
  ...
  - [x] Share one sector-agnostic runtime required-intake analyzer across live follow-up and response guards, and add deterministic exact-label repair plus constrained missing-field recovery during extraction so contextual answers can suppress re-asks and persist under the configured field labels
  - [x] Recover canonical `service_type` from current conversation + summary evidence when raw `service_type/services` are empty but an approved catalog match is still clear
  - [x] Required-intake repair now scans recent customer turns for category-compatible evidence and accepts relative timeline answers like `1 ay içinde` or suffixed month phrasing (`Temmuzda`) instead of relying only on the latest message
  - [x] Lead extraction now emits semantic `intent_stage` values (`none`, `informational_commercial`, `qualification`, `booking_ready`) so first-message commercial inquiries generalize across sectors/languages without static keyword-first scoring
  - [x] Media-backed first-contact prompts that ask for information/details about the shared attachment now promote to at least `informational_commercial` / `warm` even when the exact service text is still implicit (`bunun hakkında daha fazla bilgi alabilir miyim?`)
- [x] **Lead Scoring**
  - [x] Implement 0-10 scoring algorithm
  - [x] Auto-generate AI summary
  - [x] Semantic intent-stage scoring + score reasoning modal
- [x] **Lead Status**
...
- [x] **Skills Management**
  ...
  - [x] Skills list load now reads existing rows first, seeds defaults only for empty workspaces, and keeps embedding-maintenance work off the normal request path
  - [x] Simulator-first testing accepted for MVP (no additional per-skill playground)
- [x] **Knowledge Base UI**
  - [x] CRUD with categories
  - [x] Rich text editor
  - [x] Collection counts now come from an aggregate RPC path instead of scanning all document rows during page load
- [x] **Channels**
...
- [ ] **Pricing Strategy**
  ...
  - [x] Lock billing order: trial first; after premium activation, persistent extra-credit balance (including trial carry-over) is consumed before the monthly package
...
- [ ] **Plan Purchase (Online Payment)**
  ...
  - [x] Replace provider `NOW` full-price upgrades with hosted fixed-difference upgrade checkout, immediate delta-credit activation, and provider `NEXT_PERIOD` plan scheduling for the next renewal
...
- [ ] **Paywall Implementation (Execution)**
  ...
  - [x] Add trial-active dashboard banner with `Onboarding checklist` and `Upgrade now` CTAs, hiding the checklist CTA after onboarding completion while keeping the banner until trial end / upgrade
  - [x] Normalize trial countdown displays to calendar-day remaining values across sidebar, mobile usage card, and dashboard banner so partial-day remainder does not round up the visible end-date gap
  - [x] Add organization-scoped onboarding checklist route with first-visit auto-open, real setup-derived completion state, and shared cached shell snapshot for banner/sidebar/mobile consumers
  - [x] Keep desktop sidebar and mobile `Other` onboarding progress synchronized with fresh `/onboarding` page snapshots after same-layout route transitions
  - [x] Keep incomplete onboarding accessible from desktop sidebar and mobile `Other` menu even after upgrade, while hiding the entry once checklist completion reaches 100%
  - [x] Refine onboarding checklist presentation to a wrapper-light, left-aligned respond.io-style layout with localized `Bilgi Bankası` / `Yetenekler` copy and a channel preview that marks `Messenger` as coming soon
  - [x] Add one-time first-visit intro modals for `/calendar` and `/simulator`, using onboarding-style icon rows, localized TR/EN copy, mobile-friendly layout, and per-browser user/workspace dismissal state
  - [x] Lock bot mode to `Kapalı` for newly created workspaces until onboarding reaches `5/5`, including sidebar/settings lock UI, runtime enforcement, and a required completion modal that unlocks explicit `Dinleyici` / `Aktif` / `Kapalı kalsın` selection
  ...
  - [x] Add hosted-checkout timeout fallback to the stored secure Iyzico payment page so subscription, top-up, and upgrade checkouts do not strand users on a blank embedded surface when the provider script fails to mount
  ...
  - [x] Consume persistent extra-credit balance before monthly package credits after premium activation so carried-over trial credits are used first
  ...
  - [x] Make sidebar/mobile billing usage card locale-aware so renewal dates and credit counts always follow the active TR/EN app locale
  - [x] Show total remaining credits in `Settings > Plans` so the status surface matches the sidebar total
  - [x] Keep carried-over trial credits anchored to the original trial limit in `Settings > Plans`, so premium extra-credit status can render balances like `153.5 / 200`
  - [x] Hide `Settings > Plans` total and extra-credit status cards when premium extra-credit balance reaches `0`, leaving only the monthly package card visible
  - [x] Reorder `Settings > Plans` extra-credit actions so `Tek seferlik kredi ekle` stays primary and the custom-package contact CTA sits directly beneath it with quieter secondary styling
...
- [ ] **Trial Abuse Prevention**
  ...
  - [x] Exclude shared/disposable inbox domains such as Yopmail, Mailinator, 10MinuteMail, and GuerrillaMail from email-domain business fingerprints so test inboxes do not create false duplicate-trial blocks
...
- [x] Keep QA-Lab consistency normalization aligned with normalized summary text and cross-language intake-category cues: English `budget` phrasing now resolves to the budget category, stale did-not-ask/follow-up/missing-field claims clear correctly after consistency summaries, and low-information heuristics still distinguish abstract filler from grounded general-info offer copy
...
- [x] Playwright guard for settings shell persistence and warmed revisit navigation
...
- [x] Default new workspace booking settings to closed and surface the active/closed booking state as a `/calendar` header chip
...
- [x] Prevent mobile browser focus auto-zoom in reusable public Demo Chat by keeping the composer textarea at an iOS-safe 16px text baseline
...
- [x] Prevent cached public Demo Chat pages from reusing expired signed API tokens by forcing fresh server rendering for demo slugs
...
- [x] Complete Yüksek İhtisas RAG P1 by short-circuiting high-confidence lexical/focused evidence before broad vector search, making RAG completion compatible with GPT-5-family reasoning parameters, and recording live 33-question token/cost comparisons for `gpt-5-mini`, `gpt-4o`, and `gpt-4o-mini`
...
- [x] Make simulator and standalone fallback load organization AI behavior settings with the resolved response language while keeping deterministic Skill replies exact and unpolished
- [x] Add File Search admissions demo readiness/signoff gates to the brochure spec: vector-store ingestion status, usage/lifecycle tracking, citation mapping manifest, exact Skill template boundary, and customer answer/source review before preview
- [x] Implement the first PDF-independent brochure File Search infrastructure slice: vector-store readiness helper, approved citation mapping loader, ingest readiness/source-manifest output, and disabled-by-default provider config resolver
- [x] Run the OpenAI File Search RAG migration benchmark in an isolated branch/local flow before any agent-backbone migration or public demo provider switch
  ...
  - [x] Add the local File Search benchmark implementation plan (`docs/superpowers/plans/2026-06-03-openai-file-search-rag-spike-implementation-plan.md`)
  - [x] Add provider-neutral RAG eval types/scoring, approved-story manifest validation, File Search Responses API adapter with Qualy credit estimates, and local ingest/eval CLI scripts
  - [x] Build a 50-question YİÜ PDF-limited benchmark from the existing mevzuat QA cases filtered to the user-approved PDF links (`tmp/rag-evals/yiu-link-pdfs/cases.json`)
  - [x] Wait for user-approved story/PDF mappings before uploading PDFs to OpenAI vector stores; this run used only the two approved YİÜ source pages and downloaded 113 PDFs (`tmp/rag-evals/yiu-link-pdfs/manifest.json`)
  - [x] Run current RAG and OpenAI File Search side by side with answer/source/refusal/latency/cost reporting (`tmp/rag-evals/yiu-link-pdfs/rag-eval-compare-2026-06-03T13-13-50-146Z-replay.md`)
  - [x] Harden the brochure-first validated provider with intent/source-group routing, typed same-row table facts, evidence-gated approved facts, one narrowed retry, safe guardrails, and separate validated follow-ups
  - [x] Rerun the realistic 50-question approved-corpus benchmark to `50/50` with the current pair and both approved one-stage-at-a-time `gpt-5.4-mini` A/B variants; GPT-5.5 intentionally excluded
  - [x] Raise grounded follow-up coverage from `21/46` to `46/46` eligible cases with a domain-independent generic fallback while keeping refusal/source-only suppression
  - [x] Add claim-term verification v1, 7-band contextual no-info templates, current-demo campus-life guards, and `Mavi diploma` credential boundaries so strict RAG refuses unsupported exact claims without collapsing back to bare no-info copy
  - [x] Add strict research-plan diagnostics and a claim-ledger verification layer so the pipeline exposes route/tool/evidence decisions and blocks unsupported exact policy/value claims before final answers
  - [x] Add evidence-seeking retry and research blackboard diagnostics so strict File Search can re-query the missing answer facet before repair/refusal, and reject speculative LLM repair text when evidence is absent
  - [x] Add direct-answer quality diagnostics plus a source-of-truth customer-score report CLI so `7/8/9` bands are derived from rubric output and chronological retest artifacts instead of manual table edits
  - [ ] Add provider-stage model-aware actual-cost metering before customer billing or optional `gpt-5.4-mini` retrieval rollout
...
- [x] Validate the customer screenshot questions against the M1 crawl corpus and harden live demo RAG answers for exact campus addresses, contacts, TLT double-major evidence, medicine staj/final rules, lecture-note guidance, and canonical source links
- [x] Generalize policy-duration RAG retrieval and answer repair so PDF-backed duration/limit questions keep exact evidence in context beyond the single unpaid-leave example
- [x] Anchor public Demo Chat contextual follow-ups to the latest explicit user topic before RAG context construction, so short references like "bu program" cannot answer from older or unrelated chunks that only share generic terms
- [x] Expand shared-predicate compound knowledge questions into per-subject retrieval queries, so prompts like "SBF ve SHMYO kampüsleri nerede?" retrieve evidence for both subjects before answer generation
- [x] Repair multi-subject campus/location answers when the model omits one requested subject despite retrieved source evidence containing that subject's campus or address
- [x] Keep deterministic source-backed Public Demo Chat RAG repairs off the heavier grounded-generation path while still loading organization AI settings, running bounded behavior polish, persisting `rag_polish`, and metering polish usage when tokens are spent
- [x] Compact medicine training/staj repair output while preserving the exact source facts about six-year duration, clinical staj periods, intörnlük, and the lack of a separate `yaz stajı` claim
- [ ] Execute dashboard performance hardening before expanding pilot usage
  ...
  - [x] Reduce dashboard nav and Settings transition rework by reusing server shell billing/AI data, viewport-gating hidden nav prefetch batches, disabling automatic nav `Link` prefetch, using existence reads for boolean sidebar badges, and showing a local Settings detail skeleton on cold transitions
...
- [ ] Finish GTM readiness hardening before expanding beyond the first 5 pilots
  - [x] Harden signup confirmation/login handoff: `/login?code=...` exchanges through a server callback, stale auth codes are cleared on failure, post-auth redirect lookup failures fall back to `/inbox`, and Supabase RPC calls preserve client method context
