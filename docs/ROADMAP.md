...
> **Update Note (2026-05-27):** Public Demo Chat now renders standalone RAG source URL lines as a localized short action link (`Daha fazla oku` / `Read more`) in assistant replies. The stored bot text and channel/inbox rendering still keep raw URLs, so traceability and messenger compatibility remain unchanged while the customer-facing demo reads cleaner.
...
> **Last Updated:** 2026-05-27 (Yüksek İhtisas demo RAG now has funded retest capacity, zero broken URL text findings in the current corpus, URL line-break normalization for future chunking, high-signal keyword extraction for natural conversational questions, original-query-first retrieval-only LLM query planning with a bounded fail-fast planner timeout plus strong-evidence short-circuit for planned variants, standalone subject-cue questions preserved ahead of router/history drift, query-intent scoped focused evidence probes, required-evidence-first campus and medical-school policy scans before broad OR fallback, high-confidence current-campus evidence short-circuiting before broad address scans, acronym-to-expanded-title initialism matching for academic-unit retrieval and duration repair, title-initialism abbreviation recovery from retrieved document titles, final/bütünleme drift repair, in-year board-grade formula repair for missing `%96 + Hekimliğe Uyum / Kanıta Dayalı Tıp` details, compact course-table duration repair for staj/curriculum evidence, complete multi-bracket policy-duration repair for broad questions, bounded evidence-query timeouts, immediate pending Demo Chat POSTs, direct extractive address/location answers, polling-time deterministic extractive KB recovery before full pipeline fallback, stale-token-safe dynamic Demo Chat page rendering, expired-tab Demo Chat token refresh, Supabase trigram/scoped indexes for lexical fallback speed, recoverable vector-timeout warnings, topic-related RAG engagement guidance with generic closing-tail/source-link cleanup, public-demo canary/source-trace tooling, 33-question live screenshot-plus-variant QA coverage, and evidence-first answer repair for eligibility, unit contact, and factual policy answers.)
