...
> **Update Note (2026-06-11):** Customer-facing demo File Search replies now pass through the existing grounded RAG polish layer before they are returned to the browser, regardless of whether the answer came from retrieval, a table fact, strict catalog fast path, guardrail/refusal, or clarification flow. Provider-level direct/table/catalog paths also expose presentation-polish diagnostics when they polish early, and the demo bridge skips a duplicate call in that case; otherwise it runs one final polish pass using the organization's AI behavior settings (`prompt` and `bot_name`), then reattaches traceable source URL lines. This makes source mechanics such as brochure/table/row/field wording internal by default while still preserving citation traceability and exact factual values.
