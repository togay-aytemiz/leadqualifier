...
> **Update Note (2026-06-11):** Validated File Search direct answers now pass through the existing grounded RAG polish layer before reaching the customer. The direct/table path still validates against File Search citations and appends traceable source URLs, but the visible answer is rewritten by the LLM presentation layer using the organization's AI behavior settings (`prompt` and `bot_name`) so source mechanics such as brochure/table/row/field wording are not exposed unless the user explicitly asks for source provenance. Presentation polish usage and model metadata are recorded in provider diagnostics, and tests cover a direct program-fee answer where the raw brochure-table sentence becomes natural user-facing fee copy while preserving the exact amount.
