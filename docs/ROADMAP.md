...
> **Last Updated:** 2026-04-07 (Pending Meta channel surfaces now explicitly tell the operator to send a test message to the connected WhatsApp/Instagram account and explain that Qualy auto-verifies the connection after the first inbound message; pending gallery/onboarding surfaces now refresh when the operator returns focus to the app; onboarding completion still requires a neutral explicit bot-mode choice instead of auto-switching to `Dinleyici`; onboarding-complete workspaces with sparse legacy unlock metadata now still resolve to effective `Kapalı` until the operator chooses a mode; Skills now support one optional uploaded image per skill with `5 MB` max upload, client-side `WebP` normalization, loading/disabled editor state during processing, text-first-then-image delivery across WhatsApp/Instagram/Telegram, simulator parity, and non-blocking media failure handling that still completes `requires_human_handover` escalation after the image attempt; structured `Yapay zeka asistan talimatları` now replace the raw `AI Prompt` field inside `Settings > Qualy AI > Behavior and Logic`, with four optional instruction fields, locale-aware starter text, legacy custom-prompt carryover into `Diğer talimatlar`, and a mobile-friendly `Nasıl çalışır?` modal; customer-facing reply prompts now use compiled instructions while internal extraction/scoring/routing/summary prompts remain unchanged; default guardrail skills are now provisioned at organization owner-membership time with locale-aware create-time seeding plus backfill for older workspaces; dashboard chunk-mismatch recovery now retries the pending sidebar/mobile/settings destination instead of reloading the currently visible page, so chunk failures during route transition no longer look like the app stayed stuck on the same screen; trial countdown copy now uses calendar-day remaining values that match the displayed end date across the sidebar, mobile usage card, and dashboard banner, so partial-day remainder no longer rounds `2 Apr → 14 Apr` up to `13 days`; Organization Settings now shows a localized non-blocking AI extraction banner when Knowledge Base-derived business setup is still processing, keeps manual editing available, and auto-refreshes when service profile / required info / service list extraction completes; trial-active workspaces also use a cached global onboarding banner plus organization-scoped onboarding checklist with first-visit auto-open, real setup-derived completion, desktop/mobile navigation entry while incomplete, more breathing room between the banner and onboarding content, automatic expansion of the next incomplete step only on page entry, no forced reopen after manual collapse, and manual reopen support for completed steps; onboarding chrome is now localized as `Getting started` / `Başlangıç`, desktop sidebar surfaces it as a highlighted purple top card above `Workspace`, now with a plain purple rocket icon instead of a separate white icon wrapper, the onboarding page greets the user by name with simplified copy, the intro CTA is localized as `Devam et`, the progress bar and primary actions now use the same purple onboarding theme, the content layout is now a cleaner left-aligned respond.io-style accordion without extra inner wrappers, stacked single-column step bodies, no intro chips, more compact filled helper icon rows, and outline/filled step-state icons with better vertical centering; the intro copy now stays business-focused instead of team-focused and explicitly mentions appointment flow, the AI setup step now uses `Bilgi Bankası` / `Yetenekler` wording, the AI settings step now emphasizes bot-name personalization instead of bot-mode review, the final step previews connectable channels with direct larger icons and `Messenger` marked as coming soon, the banner copy/alignment stays trial-focused with bold remaining-days emphasis, and the final checklist step now completes when any ready live channel is connected rather than assuming WhatsApp only; intro acknowledgement now opens the next step immediately, missing onboarding-state schema no longer crashes the dashboard while the shell refreshes, the first-ever Knowledge Base document now opens a one-time educational modal that explains automatic AI extraction for business profile / required fields / service list before offering routes back to `Başlangıç` or directly into `Organization Details`, and brand-new workspaces now keep bot mode locked to `Kapalı` until onboarding reaches `5/5`, after which a mandatory modal unlocks an explicit `Dinleyici` / `Aktif` / `Kapalı kalsın` choice while sidebar/settings/runtime all honor that gate; bot-mode lock UI now also follows live onboarding progress, and the lock copy is now rendered as one bold violet banner without duplicate helper text or hover affordances while incomplete states remain visibly locked; live Iyzico `past_due` plans now support in-app recovery with hosted payment-method update and explicit retry payment actions while access stays locked until webhook confirmation, the settings shell now redirects missing-auth Iyzico return visits to login instead of rendering a blank white page, localhost Iyzico checkout returns now stay on the current local origin instead of being forced to production `NEXT_PUBLIC_APP_URL`, immediate Iyzico upgrades now explicitly preserve the existing billing anchor by sending `resetRecurrenceCount=false` with `upgradePeriod=NOW`, the immediate-upgrade popup now explicitly shows full delta charge, full delta credit unlock, and preserved next renewal date, `Settings > Plans` now softens the subscription-action hierarchy by moving cancel into the manage-plan modal and rendering payment recovery as lighter text actions, direct plan-change confirmation now uses a single explicit approval instead of repeated mandatory legal re-acceptance copy while still linking legal documents, `Settings > Plans` now also surfaces the next renewal date directly, a lightweight `Geçmiş / History` modal for successful purchases and renewals, a saved billing-profile form that pre-fills Iyzico checkout customer/address data, a lighter billing-info layout where only the next-renewal box stays wrapped while the history/update actions sit directly below it as underlined text actions, in-app subscription/top-up price surfaces now explicitly state that displayed amounts include VAT/KDV across both subscriber and non-subscriber pricing UI, top-up checkout now follows the same hosted in-app embed route as subscription checkout instead of opening a separate full-page provider flow, and the active monthly-package card now says `Yenileme / Renews at` instead of `Sıfırlanma / Resets at`; the hosted card-update page itself now includes localized guidance plus a direct return path back to Plans; WhatsApp disconnect now removes only Qualy's local channel relationship without provider-side deregistration, and the confirm copy now says that local-only scope explicitly; WhatsApp template UX clearly separates Meta WABA templates from normal saved messages, keeps WABA help inside the nested `How it works` guide, and prevents that guide from closing the parent modal; WhatsApp onboarding puts the Business app/WABA path first with simpler recommended copy; pending WhatsApp channels support retry-without-disconnect plus first-inbound self-verification; WhatsApp Embedded Signup recovers local channel completion from auth code with a short-grace fallback after missing Meta `FINISH`; legal credit wording now distinguishes monthly package credits from one-time extra credits, keeping monthly package credits non-rollover while extra credits stay active-subscription-only balance until consumed; Turkish login localizes invalid-credential errors; the collapsed desktop sidebar keeps its dark rail with immediate slim-mode tooltips; and `Settings > Channels` keeps the `WhatsApp`, `Instagram`, `Facebook Messenger`, `Telegram` order with a 3-column desktop cap.)
...
- [x] **Skill CRUD**
  ...
  - [x] Add one optional uploaded image per skill with client-side `WebP` normalization, loading/disabled editor state, and sequential text-then-image delivery across WhatsApp/Instagram/Telegram
