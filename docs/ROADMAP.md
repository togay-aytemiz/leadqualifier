...
> **Update Note (2026-03-31):** WhatsApp disconnect no longer throws expected coexistence/provider blockers as generic action-level 500s. Known Cloud API deregistration limits now stay in the page as localized inline guidance so operators can see the real next step.
...
> **Last Updated:** 2026-03-31 (WhatsApp template UX now clearly separates Meta WABA templates from normal saved messages, keeps WABA help inside the nested `How it works` guide, and prevents that guide from closing the parent modal; WhatsApp onboarding now puts the Business app/WABA path first with simpler recommended copy; pending WhatsApp channels now support retry-without-disconnect plus first-inbound self-verification; WhatsApp Embedded Signup now recovers local channel completion from auth code with a short-grace fallback after missing Meta `FINISH`; WhatsApp disconnect keeps coexistence blockers as localized inline guidance instead of action-level 500s; Turkish login localizes invalid-credential errors; the collapsed desktop sidebar keeps its dark rail with immediate slim-mode tooltips; and `Settings > Channels` keeps the `WhatsApp`, `Instagram`, `Facebook Messenger`, `Telegram` order with a 3-column desktop cap.)
...
- [x] **WhatsApp (Meta Cloud API)**
  ...
  - [x] Return structured disconnect errors for coexistence/provider blockers so `Settings > Channels > WhatsApp` shows inline guidance instead of a generic action-level 500
