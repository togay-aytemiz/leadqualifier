...
> **Last Updated:** 2026-06-11 (The validated File Search bridge now carries clarification context as explicit pending state and requires the contextual orchestrator to decide `state_decision` as `use`, `ignore`, `split`, or `clarify` before retrieval. Short, typo-heavy, long, or question-like student replies can still consume pending state when the LLM says they fill the slot; fresh independent questions bypass stale state; mixed follow-ups can split into clarification answer plus new facet; and still-vague replies ask one more clarification. The strict quality, evaluator/retry, research-plan, claim-ledger, actionable-boundary, and catalog layers remain configured.)
