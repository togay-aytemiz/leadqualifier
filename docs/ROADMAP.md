...
> **Last Updated:** 2026-05-25 (Public Demo Chat composer now avoids mobile focus auto-zoom with an iOS-safe 16px textarea baseline; Yüksek İhtisas demo RAG also has P1 evidence-first retrieval performance shortcuts, reasoning-model-compatible RAG completion parameters, answer repair, PDF type backfill, source-link cleanup, timeout extractive recovery, and 33/33 live screenshot-plus-variant QA coverage for PDF/crawl-backed answers across `gpt-5-mini`, `gpt-4o`, and `gpt-4o-mini`.)
