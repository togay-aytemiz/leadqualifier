...
> **Update Note (2026-03-28):** Inbox realtime now self-recovers after long idle tabs. Browser resume events (`visibilitychange`, `focus`, `pageshow`, `online`) trigger a first-page + selected-thread resync, and broken Supabase channel states (`CHANNEL_ERROR`, `TIMED_OUT`, `CLOSED`) trigger the same recovery path so operators no longer need a manual refresh to see queued messages.
...
- [x] **Inbox UI**
  ...
  - [x] Inbox realtime now self-recovers after long idle tabs: browser resume events (`visibilitychange` / `focus` / `pageshow` / `online`) resync the first conversation page plus selected thread, and broken Supabase channel states trigger the same recovery path
