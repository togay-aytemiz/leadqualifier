...
> **Last Updated:** 2026-06-11 (Customer-preview RAG now compiles tenant AI behavior text into a structured behavior policy, injects that policy into contextual orchestration, merges a policy-driven universal claim ledger into strict validation, and emits typed conversation state diagnostics for future turns. The YİÜ Tanıtım Günleri production demo organization also has structured AI behavior settings seeded for institution scope, brochure-first evidence priority, document-backed fact categories, safety/refusal boundaries, and student-friendly tone. Validated File Search uses a global contextual intake gate before retrieval for Public Demo Chat first turns and follow-ups. The gate records route/domain/safety/missing-slot diagnostics, asks one-slot clarifications for incomplete in-domain turns, refuses off-domain/unsafe/impossible turns, and works with a bounded zero-hop LLM research-planner boundary. Pending clarification state remains LLM-led but guarded against unrelated external-topic replies. The strict quality, evaluator/retry, research-plan, claim-ledger, actionable-boundary, and catalog layers remain configured.)
