...
> **Update Note (2026-05-26):** RAG demo polish now lets the LLM add one short engagement question only when it is topic-related and teaches or offers an adjacent grounded detail from the same source context without assuming the user is a student, applicant, personnel member, or admin. Generic assistant closing tails, role-assumptive department/status prompts, source-link preface tails, and vague "more info / contact the department" deferrals are stripped from otherwise grounded Knowledge Base answers. Policy-duration repair now extracts matching list/bullet evidence such as annual-leave brackets without letting generic eligibility repair overwrite correct duration answers, and standalone user questions are searched before drifted router rewrites. Vector-search timeout responses are treated as recoverable warnings while lexical/focused evidence continues. The latest stricter live YİÜ challenge, including generic/role-assumptive closing checks, passed 33/33 at `tmp/crawl-output/live-yiu-challenge-pipeline-qa-2026-05-26T18-28-49-968Z.md`.
...
> **Last Updated:** 2026-05-26 (Yüksek İhtisas demo RAG now has funded retest capacity, zero broken URL text findings in the current corpus, URL line-break normalization for future chunking, high-signal keyword extraction for natural conversational questions, retrieval-only LLM query planning for noisy/abbreviated/policy questions, Supabase trigram/scoped indexes for lexical fallback speed, recoverable vector-timeout warnings, topic-related RAG engagement guidance with generic closing-tail cleanup, public-demo canary/source-trace tooling, 33-question live screenshot-plus-variant QA coverage, and evidence-first answer repair for eligibility, unit contact, and factual policy answers.)
