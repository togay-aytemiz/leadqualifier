...
> **Update Note (2026-06-01):** Public Demo Chat source-link filtering now treats short learning-platform names such as UZEM/MEDU/ÖBS as required concrete evidence, not only ranking hints. If an answer says a material is available through a named platform, appended source URLs must come from chunks that actually contain that platform evidence, preventing broad learning-material PDFs from appearing as extra sources.
