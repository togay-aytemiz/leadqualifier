...
> **Last Updated:** 2026-06-12 (Live follow-up smoke hardening now lets pending clarification state override unnecessary LLM re-clarification for concise slot answers, preserves the original requested metric across follow-ups, recognizes `ing` as an English variant signal for table/direct/catalog routing, and fails open to the current provider on controller `plan_claim_mismatch`.)
