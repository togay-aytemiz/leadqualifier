...
> **Last Updated:** 2026-06-05 (The isolated YİÜ Tanıtım Test organization, `yiu-tanitim-gunleri-2026` demo channel, 10,000-credit premium billing state, approved 135-source Knowledge Base corpus, preview-only validated OpenAI File Search demo bridge, university icon, browser chat list rendering, and onboarding completion modal feedback are configured. The local validated File Search pipeline still passes `50/50` with `46/46` eligible follow-ups on the current model pair; `gpt-5.4-mini` retrieval remains optional until model-aware actual-cost metering exists. The existing `yiu-qualy-ai-demo` and global Supabase RAG default remain unchanged. Customer signoff and DB-backed provider-config generalization remain pending.)
