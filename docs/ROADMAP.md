...
> **Update Note (2026-05-26):** Public Demo Chat RAG focused evidence now prioritizes high-confidence current-campus/location evidence before wider address scans, and individual evidence queries are bounded so one slow Supabase lexical scan cannot keep a Netlify polling invocation open until the platform timeout. Acronym/academic-unit campus questions use the same subject-coverage and current-listing evidence rules instead of a customer-question phrase map; SHMYO-style campus retrieval now resolves from the current campus listing in under a second locally before answer repair/source-linking runs.
