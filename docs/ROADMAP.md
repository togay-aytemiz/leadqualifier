...
> **Update Note (2026-05-28):** Public Demo Chat and shared extractive RAG now run grounded fast-path answers through a bounded LLM polish step that can make wording warmer and add one engagement follow-up only when the model provides an exact evidence quote from retrieved Knowledge Base context. If the quote is missing or the polish drops factual values such as numbers, phones, e-mails, or other explicit values, the system falls back to the original extractive answer. Compound knowledge questions are split into independent retrieval queries so a multi-source answer is returned only when every clause has evidence instead of answering just the first clause; demo polling now also avoids running the heavier focused-evidence scan on the full compound sentence and aborts timed-out polish calls so slow LLM/network work cannot keep a Netlify invocation open until the platform timeout. Demo source links now render as `Kaynağı aç / Open source`, and multiple appended source URLs render as numbered `Kaynak 1 / Source 1` labels while raw stored URLs remain unchanged.
