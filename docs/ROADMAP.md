...
> **Update Note (2026-03-27):** `Settings > Plans` extra-credit balance now reconstructs its displayed total from the live topup pool, not only paid orders. Carried-over trial credits should keep their original pool size visible after premium activation, so a remaining balance like `153.5` is rendered against `200` instead of collapsing to `153.5 / 153.5`.
...
- [ ] **Paywall Implementation (Execution)**
  ...
  - [x] Reconstruct `Settings > Plans` extra-credit total from live topup balance + consumed topup usage so carried-over trial credits stay visible against their original pool size
