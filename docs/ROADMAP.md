...
> **Last Updated:** 2026-06-07 (The isolated YİÜ Tanıtım Test organization, `yiu-tanitim-gunleri-2026` demo channel, 10,000-credit premium billing state, approved 135-source Knowledge Base corpus, preview-only validated OpenAI File Search demo bridge, strict answer quality mode with optional LLM global evaluator/retry, research-plan/tool-router diagnostics, claim-ledger verification, facet-aware answer contract, evidence-seeking retry with research blackboard diagnostics, low-score and mid-score retest hardening, claim-term verification v1, current-demo campus-life/credential scope guards, structured clinical/admissions facts, finance/scholarship/fee direct facts, university icon, browser chat list rendering, onboarding completion modal feedback, and generalized clarification-gate behavior are configured. The local validated File Search pipeline still passes `50/50` with `46/46` eligible follow-ups on the current model pair; the 508-question audit currently has `0` questions below `7`, the v2 lab/facility facet smoke blocks program-existence false positives deterministically, and the focused service/transport smoke improved `4/4` from `3.25` to `7.00` while blocking unsupported speculative repair language. The supported-fact promotion smoke and actionable no-info boundary smoke were followed by a full current-7 rerun: `108` current-7 questions were retested, `58` moved to `8`, and `50` stayed at `7`; the follow-up finance/burs/fee direct-fact pass then moved `5` more current-7 answers to `8`, and the clinical/staj/lab direct-fact pass moved `18` more current-7 answers to `8`. The full 508-question effective distribution is now `27` x `7`, `421` x `8`, and `60` x `9`. Bot identity, unsupported WhatsApp advisory-line edge cases, unsupported finance/scholarship scope claims, and high-risk clinical/staj/lab prompts now route through deterministic catalog guards. `gpt-5.4-mini` retrieval remains optional until model-aware actual-cost metering exists. The existing `yiu-qualy-ai-demo` and global Supabase RAG default remain unchanged. Customer signoff, automated fact-catalog ingestion, larger post-v2 canary reruns, and DB-backed provider-config generalization remain pending.)
