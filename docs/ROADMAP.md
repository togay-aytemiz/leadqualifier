...
> **Last Updated:** 2026-03-05 (Iyzico provider integration moved from mock to live checkout flows: recurring subscription checkout now initializes with Iyzico subscription checkout form (hosted in-app checkout surface + callback finalization), one-time top-up purchases now initialize with Iyzico checkout form payment page + callback finalization, and callback processing now applies idempotent credit/package grants before redirecting back to Plans status; trial abuse policy progressed to one-trial-per-business: trial identity fingerprints now use company-name/email-domain signals at signup/initialization and extend with WhatsApp `business_account_id` + normalized phone on channel connect; duplicate fingerprints block additional free trials and force subscription-required state, while signup pre-check now returns a localized “trial already used” error; trial signup abuse guardrails improved: registration now runs signup velocity throttling (`email + IP`) with cooldown windows via RPC-backed guard functions, records successful/failed attempts for escalation, and supports optional Turnstile CAPTCHA verification in Sign Up when site/secret keys are configured; Sign Up success flow now routes unconfirmed accounts to a dedicated check-email checkpoint that shows submitted email, spam/junk guidance, quick change-email return, and sign-in shortcut; Inbox scroll-to-latest behavior improved: smooth animation restored for jump-to-latest and chat now stays pinned to true latest when composer height changes (e.g., WhatsApp 24-hour expired panel) or when lazy-loaded message images shift layout; Inbox conversation history now lazy-loads older messages while scrolling upward with preserved viewport position; QA Lab intake-coverage semantic fallback bug fixed: context-style service fields (e.g., `Talep bağlamı`) now allow broad semantic fulfillment fallback after explicit ask, so contextual customer replies are no longer falsely marked missing; QA Lab request-mode detection Unicode normalization bug fixed: Turkish uppercase dotted `İ` now normalizes safely (`NFKD` + combining-mark removal), preventing policy/procedure intent misses such as `İptal...` being misclassified as `general_information`; live intake blocked re-ask regression fixed: required-field matching now accepts partial-but-strong field cues in refusal/no-progress flows to prevent repeated pressure asks and guidance now explicitly says “do not insist”; Inbox composer now supports WhatsApp outbound attachments with multi-select (max 10), clipboard paste-to-attach for images (auto-takes first available slots up to max), dedicated pre-send attachment strip + preview/remove flow, signed-upload media pipeline, and optimistic async send status (`sending`/`failed`) for image/document messages with immediate local image preview; lead extraction conversation context now excludes WhatsApp media-placeholder messages and uses text-only turns; WhatsApp inbound media support now includes image/document parsing in webhook runtime, media download + storage persistence (`whatsapp-media` bucket), and Inbox rendering for inbound media (image preview/document card) with localized conversation-list media preview labels; consecutive image-only WhatsApp messages now render as grouped gallery bubbles (2/3/4+ grid) for cleaner multi-image threads; Settings > Usage now shows organization storage totals (Skills + Knowledge + WhatsApp media) and Admin organization list/detail now includes per-org storage visibility with WhatsApp media breakdown; AI Settings now includes a channel-wide bot disclaimer toggle with localized TR/EN editable text (default enabled); outbound bot replies in WhatsApp/Telegram/Instagram now append disclaimer text as a blockquote line (`\n\n> ...`) when enabled; Inbox bot-message rendering now hides trailing disclaimer quote lines from UI while preserving outbound channel payload (LF/CRLF tolerant parsing); matched skill bot replies now persist/display skill title metadata in Inbox footer area when available (object or JSON-string metadata payloads), and skill footer label never falls back to raw `skill_id`/UUID; runtime backfills title from `skills` row when matcher title is empty; matched skills are evaluated in ranked top-5 order and the first successful match is applied directly, and if that skill has `requires_human_handover=true` runtime escalates to operator without an extra intent guard; AI Settings `Sensitivity` control is now grouped under `Behavior and Logic` (not `General`); `Lead extraction during operator` is now grouped under `Escalation`; Settings navigation/page title label is now `Qualy AI`; AI Settings keeps reusable animated tabs (`General`, `Behavior and Logic`, `Escalation`) with escalation content split into two primary sections (`Automatic Escalation` + `Skill Based Handover`) and no section subtitle text; Organization Settings now uses reusable tabs (`General`, `Organization Details`, `Security & Data`); AI/Organization settings content now starts directly with tabs (intro description text removed); Inbox template picker remains mobile-optimized with underline tabs, WhatsApp-only refresh action, inset chevrons, and smooth tab resize animation; Inbox message-day badges now render from actual message dates (`Today`/`Yesterday`/localized full date) instead of a static `Today` label; lead status model is simplified to `cold/warm/hot` only and legacy `ignored/undetermined` values are normalized to `cold`; Inbox queue now supports `All / Unassigned / Me` tabs with a single circular badge model (attention count in red), and conversation rows use compact red `!` attention indicator only on `All` tab (hidden on filtered `Unassigned`/`Me` views) to avoid redundancy while preserving human-attention state (`human_attention_*`); main sidebar bot status now uses a filled quick-switch dropdown with inline mode explanations and a settings shortcut; collapsed sidebar hover UX now shows immediate custom tooltips across bot status/nav items and the bot dropdown open/close animates; mobile Inbox header now includes a bot-status pill that opens a quick-switch bottom sheet with mode explanations; mobile bottom navigation now groups Skills + Knowledge under an `AI` tab sheet; shadow-mode runtime paths now have explicit regression coverage to guarantee reply suppression while lead extraction continues; skills now support up to 3 WhatsApp quick-action buttons (`trigger skill` or `send URL`) and inbound button replies execute deterministically; WhatsApp onboarding direction is now clarified as existing-number-first, concierge-assisted, and preflight-gated before Meta connect.)  
...
- [x] **WhatsApp (Meta Cloud API)**
  ...
  - [ ] Replace blind WhatsApp Meta redirect with an existing-number-first preflight wizard
  - [ ] Persist WhatsApp onboarding requests and classify them into `direct_oauth` / `needs_concierge` / `blocked`
  - [ ] Add a lightweight admin onboarding queue for WhatsApp setup requests
  - [ ] Gate direct Meta OAuth behind explicit onboarding eligibility instead of opening it for every customer
- [x] **Telegram (Sandbox)**
