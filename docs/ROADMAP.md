...
- [x] **Operator Takeover Control**
  ...
  - [x] Compact Details operator tools: tags/private note now render as plain sections below lead extraction, tag input is collapsed by default, empty tag state stays silent, private-note conflict checks ignore unrelated conversation updates, and note metadata is reduced to two compact lines (`who`, `when`)
