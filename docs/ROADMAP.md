...
> **Update Note (2026-05-28):** RAG answer generation now uses a reusable grounded-answer layer before deterministic answer repair in both Public Demo Chat recovery and the shared inbound pipeline. The model may write in the organization's configured voice only when it returns exact support quotes from retrieved Knowledge Base context; generated answers are rejected if support quotes are missing or if critical values such as numbers, phone numbers, e-mails, document codes, or URLs are not present in the retrieved evidence. Deterministic repair remains as a fallback safety net instead of the primary answer-writing mechanism.
