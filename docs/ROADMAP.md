...
> **Update Note (2026-06-01):** Public Demo Chat focused RAG latency now avoids hidden LLM planner work on deterministic context-expanded fast-path searches; broad fallback still owns planned semantic variants when focused evidence cannot answer. Focused lecture-note retrieval tries one high-signal required evidence group before wider OR scans, and health-report/mazeret-exam retrieval returns from required remedy evidence instead of paying the broad health-policy OR scan when the remedy rows already ground the answer. Production-style profiling against the YİÜ corpus showed the target local focused searches drop from multi-second waits to sub-second evidence retrieval without adding customer-specific phrases.
...
- [x] **Focused RAG Latency Tightening:** Keep deterministic focused fast paths planner-free, try high-signal required evidence before broad lecture-note scans, and skip broad health-policy OR scans when required remedy evidence already supports the answer
