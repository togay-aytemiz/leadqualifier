...
> **Last Updated:** 2026-06-10 (The validated File Search bridge now receives recent ordered user/bot history before strict understanding, catalog, retrieval, and evaluator decisions. A generic contextual orchestrator can keep a message standalone, rewrite it from history, ask clarification, or refuse before retrieval, preventing short continuation turns such as `olur et` from being searched literally. History lookup is fail-soft, contextual decisions are metered/diagnosed, and variant-existence questions such as `burslu program var mı` route to primary brochure table evidence before broader scholarship or website/PDF sources. The strict quality, evaluator/retry, research-plan, claim-ledger, actionable-boundary, and catalog layers remain configured.)
