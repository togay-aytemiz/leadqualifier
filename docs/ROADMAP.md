...
> **Update Note (2026-04-07):** Pending Meta channel UX must now tell the operator exactly what to do: after connecting WhatsApp or Instagram, send a test message to the connected account and Qualy will automatically verify the connection when that first inbound message arrives. Pending gallery/onboarding surfaces should refresh when the operator returns focus to the app, and bot mode must still remain an explicit, neutral operator choice after verification instead of auto-switching to `Dinleyici`.
