...
> **Update Note (2026-05-26):** Public Demo Chat polling now has a deterministic Knowledge Base fast path for grounded extractive answers before it falls back to the full inbound AI pipeline. If existing retrieved chunks can repair a no-information seed into a factual answer with a canonical source link, the polling `GET` persists that bot reply directly against the demo message id; if not, the shared pipeline remains the fallback. This keeps the improvement general to RAG evidence/repair behavior rather than tied to a specific customer phrase, and avoids Netlify 30s polling failures for clear PDF/crawl-backed facts.
