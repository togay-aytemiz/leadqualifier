...
> **Update Note (2026-05-27):** Public Demo Chat RAG duration repair now compacts long course-table evidence rows before replying. When a staj/duration question is answered from a dense curriculum row, the repair step extracts the matching `Yaz Stajı` course label plus the explicit duration such as `20 iş günü` instead of exposing unrelated neighboring course-table text; this keeps acronym/program duration answers grounded and readable without adding customer-phrase-specific exceptions.
