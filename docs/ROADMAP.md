...
> **Implementation Update (2026-06-13):** Public Demo Chat now has a small history-aware Skill query rewrite before non-Skill fallback. The immediate path still tries raw exact and semantic Skill matching first. If that misses, it reads recent demo conversation history and asks a compact `gpt-4.1-mini` rewriter to decide whether the latest user turn is a standalone request, an acceptance of the assistant's previous offer, or unresolved. Accepted offers such as `evet göster`, `olur hadi bana göster`, or longer polite confirmations are rewritten into a concrete Skill search query and passed through the existing exact-then-semantic Skill matcher. When a rewritten Skill match succeeds, the original user message is still persisted, while the existing inbound Skill pipeline is re-run with the rewritten query so normal Skill behavior is preserved.
...
> **Last Updated:** 2026-06-13 (Added reproducible YİÜ Public Demo route-attribution and Skill follow-up eval runs over the 508-question pool.)
