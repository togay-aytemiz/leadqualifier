...
> **Last Updated:** 2026-05-28 (RAG now prefers conversation-aware retrieval and source-quote-validated grounded generation before deterministic repair, allows contextual deterministic repair only when evidence contains the prior explicit topic anchor, then can apply bounded source-grounded polish for organization voice/engagement; demo polling budgets are capped below platform timeout and source URL ranking filters weak extra links that lack concrete answer values.)
