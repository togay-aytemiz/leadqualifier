...
> **Update Note (2026-06-02):** Public Demo Chat now has an operator-controlled maintenance mode via `DEMO_MAINTENANCE_MODE=1`. When enabled, demo pages render a localized Qualy-branded maintenance screen before Supabase/channel lookup, and demo chat `POST`/polling `GET` requests return `503` with `Retry-After` before any service-role, Inbox, RAG, or AI pipeline work can run.
...
- [x] Add `DEMO_MAINTENANCE_MODE=1` maintenance mode for Public Demo Chat so Netlify can hide demo pages behind a localized maintenance screen and block demo API calls with `503` during unstable deploys or live fixes
