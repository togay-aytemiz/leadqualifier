...
> **Last Updated:** 2026-02-15 (Updated top-up eligibility policy: extra credits are now purchasable whenever premium subscription is active; package-credit exhaustion is no longer required. Applied across billing policy helpers, Plans action gating/copy, and SQL mock checkout + entitlement functions. Added a compatibility fallback in app checkout logic so premium organizations can still buy extra credits even if the database is still on the legacy top-up rule. Desktop/mobile usage progress now visualizes remaining package and extra credits as separate bar segments (two colors) while still reflecting total remaining credits. Extra-credit segment color is now aligned to inbox AI purple (replacing green), and expanded sidebar breakdown rows now include matching color-dot indicators (package dark + extra-credit purple) for immediate legend clarity. Plans > extra-credit purchase flow now uses a dedicated modal step (`Kredi ekle/Add credits` → `Sonraki/Next`) and copy explicitly states extra credits are one-time (non-monthly). Modal title is now generalized (`Daha fazla kredi al / Get more credits`), modal surface follows existing light product style (non-dark), and overlay is rendered at app-root level (portal to body) so dimming covers the full application including sidebars. Actions section microcopy is simplified, premium-active helper text is reduced, and `Kullanımı gör / View usage` link is moved under the section description with persistent underline affordance. The extra-credit card now hides fixed amount/credit bundle values; those details are shown in the popup step to support future multi-package options. Billing > Usage page now prioritizes `Kredi kullanımı` above ledger, moves `Kredi geçmişi` to the bottom with first-3 preview + `Daha fazla göster` expansion, and localizes common system-generated reason strings in the `Neden` column. Sidebar usage card keeps the chevron inline with the progress row and expands package/top-up/renewal details with smooth animation. Billing > Usage page no longer shows the separate account-status section to keep the page focused and shorter. Plans status cards now use the same decreasing remaining-credit progress logic as the sidebar so package/trial bars represent remaining credits instead of consumed credits. Desktop sidebar navigation now places `DİĞER / Other` directly under AI tools in the same section rhythm (no separate footer block), preserving consistent gap and styling. Sidebar and Plans now show a low-credit warning badge when remaining credits drop below 10%. Plans > Current plan now includes a dedicated extra-credit depletion bar (purple) and replaces the `Ek kredi kullanılabilir` status copy with remaining/total credit text, aligned with monthly package card semantics. Collapsed desktop sidebar now keeps credit visibility via a compact circular progress chip with hover tooltip details and direct link to Plans. Mobile More menu usage card now also surfaces the same low-credit warning badge under the shared `<10%` threshold.)  
