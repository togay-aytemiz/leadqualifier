...
> **Update Note (2026-03-31):** WhatsApp template UX now distinguishes Meta WABA templates from Qualy's regular saved messages. The channel-manage action is labeled `WhatsApp templates`, the modal warns that a connected WABA is required, and the in-product guide links to Meta's WhatsApp Business overview so first-time operators understand where these templates come from.
...
- [x] **WhatsApp (Meta Cloud API)**
  ...
  - [x] Rename channel-side template tooling to `WhatsApp templates` and add in-product WABA guidance so operators do not confuse Meta templates with Qualy saved messages
- [x] **Telegram (Sandbox)**
