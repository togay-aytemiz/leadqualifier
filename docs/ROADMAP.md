...
> **Update Note (2026-03-26):** Skills list loads now read existing skills first and seed defaults only for empty workspaces, removing hidden embedding-maintenance work from normal navigation. Knowledge folder counts now use an aggregate RPC path instead of scanning `knowledge_documents` rows on page load.
