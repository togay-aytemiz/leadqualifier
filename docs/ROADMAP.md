...
> **Update Note (2026-05-28):** Grounded RAG generation now gets the same source-grounded polish safety net as extractive answers when the first generated reply is factually valid but terse. The polish pass may warm the organization voice and add one engagement offer only if its evidence quote exists in retrieved Knowledge Base context, and its usage/metadata is recorded separately. Public Demo Chat polling budgets are capped below production serverless timeout limits even when environment values are misconfigured, and timed-out polling GETs no longer attach recovery work through Next `after()` because Netlify can hold that response open; they return `202 pending` and let the next poll retry recovery instead of surfacing a generic 502. Demo/outbound formatting also keeps canonical source URL lines separate after e-mail sentences, preventing raw PDF URLs from appearing inline inside the answer body.

> **Update Note (2026-05-28):** RAG answer generation now uses the real conversation-aware pipeline before deterministic/extractive fallback. Public Demo Chat polling first attempts the shared inbound AI pipeline instead of the separate fast extractive path, shared Knowledge Base search passes recent conversation turns into the query planner for follow-up resolution, and grounded RAG answer generation receives recent history so references such as "this program" can be resolved without guessing. The model may write in the organization's configured voice only when it returns exact support quotes from retrieved Knowledge Base context; generated answers are rejected if support quotes are missing or if critical values such as numbers, phone numbers, e-mails, document codes, or URLs are not present in the retrieved evidence. Deterministic repair remains as a fallback safety net instead of the primary answer-writing mechanism.
