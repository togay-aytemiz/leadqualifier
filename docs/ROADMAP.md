...
> **Last Updated:** 2026-06-03 (GTM LinkedIn sprint now has the first Qualy AI launch visual pack plus two follow-up 4:5 visuals for scattered DMs and AI request summaries, all with editable/review metadata and product-boundary provenance. Public Demo Chat maintenance mode is controlled at runtime through `demo_chat_channels.maintenance_enabled`, with `DEMO_MAINTENANCE_MODE=1` retained as an emergency global override and `DEMO_MAINTENANCE_BYPASS_TOKEN` retained for hashed-cookie admin testing while customers stay on the maintenance screen. Shared inbound replies now check Skills before scheduling, required-intake loading, RAG routing, fallback generation, or other token-consuming work so matched Skill turns can answer quickly; Public Demo Chat checks exact Skill triggers during POST before pending/RAG recovery; ASCII-written Turkish help/scope messages classify as Turkish so Turkish user turns keep Turkish replies. Knowledge Base RAG now uses typed Evidence Pack selection, evidence-id grounded answer generation, selected source chunks for demo/shared source links, and hybrid RRF retrieval across vector/keyword/title-source/focused-evidence channels; Public Demo Chat scope-help meta questions now return a deterministic demo-topic answer before RAG/shared AI recovery so existing pending polls can escape repeated `202` loops; focused RAG skips hidden query-planner latency on deterministic context-expanded fast paths, lecture-note focused retrieval tries one high-signal required evidence group before broad OR fallback and scopes it to named academic units when present, health-report/mazeret-exam focused retrieval skips the expensive broad OR scan when required remedy evidence already grounds the answer, contextual follow-ups try context-expanded focused evidence before broad search and use the expanded question during deterministic repair, persisted demo bot replies include `rag_diagnostics` timing/search-strategy metadata for latency triage, deterministic fast-path answers may receive source-title-supported micro-polish, source-link ranking treats Turkish word-number durations as concrete evidence for compound answers, source-backed repairs short-circuit settings/generation/polish work, generated answers still run through deterministic repair before source-linking, broad duration policies preserve grouped source brackets, contact repair/source links avoid generic/footer-phone program conflation, engagement tails are stripped unless source-grounded, shared-predicate compound questions expand into per-subject retrieval queries, and campus repair completes omitted requested subjects from evidence.)
...
- [x] Short-circuit deterministic source-backed Public Demo Chat RAG repairs before organization-settings lookup, grounded generation, polish, and usage metering to reduce latency for clear KB facts
