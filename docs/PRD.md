...
> **Last Updated:** 2026-05-29 (Public Demo Chat follow-up retrieval tries context-expanded focused evidence before broad search, demo bot replies persist `rag_diagnostics` timing/search-strategy metadata, deterministic micro-polish is limited to source-safe presentation cleanup such as title-supported course-code expansion, fast RAG may return deterministic source-backed repairs without settings/generation/polish work, grounded RAG generation remains quote-validated, generated replies pass through deterministic intent repair before source-linking, grouped policy-duration evidence must remain grouped, focused contact answers and source links avoid generic/footer phone conflation, engagement tails are rejected unless specific, source-grounded adjacent-detail offers, contextual follow-ups filter candidate chunks to the latest explicit topic anchor, shared-predicate compound questions expand into per-subject retrieval queries, and campus answer repair completes omitted requested subjects when evidence exists.)
