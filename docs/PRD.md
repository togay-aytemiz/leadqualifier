...
> **Last Updated:** 2026-03-06 (Iyzico billing provider integration remains live for Plans and is now hardened for repeated hosted-checkout attempts in the same SPA session by resetting stale checkout globals/scripts on mount-unmount with descriptor-safe cleanup for non-configurable `window.iyzi*` globals; billing checkout/callback redirects now respect next-intl `localePrefix: as-needed`, so Turkish default routes no longer hardcode `/tr`; a merchant-controlled pre-payment legal consent gate is now required before redirecting users to Iyzico for both subscription and top-up purchases, with mandatory acceptance narrowed to `pre-information + distance-sales-agreement + terms` and a short immediate-start acknowledgment while `subscription-trial` is not repeated at checkout and informational links are rendered below a subtle divider; checkout server actions now require both consent fields and resolve subscription/top-up selection from server-side catalog IDs instead of trusting client-posted credit/price amounts; register disclosure now surfaces the separate KVKK disclosure link without mixing it into mandatory contractual acceptance; Plans CTA hover feedback now uses clearer color shifts instead of motion/shadow; one-trial-per-business policy, signup throttling/CAPTCHA, media/inbox/runtime improvements, and other recent UX refinements remain as previously documented.)  
