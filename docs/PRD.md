...
> **Last Updated:** 2026-04-14 (dashboard performance refactor planning now sets measurable route/action timing, shell critical-path slimming, settings message-scope reduction, and calendar window caching targets, with the first route/action timing harness, dashboard shell data loader, and narrowed Settings route message payloads in place; calendar capacity-aware booking also ships the simple org-level `max_concurrent_bookings` model.)
