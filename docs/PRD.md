...
> **Update Note (2026-05-26):** RAG answer polish should be general, role-neutral, and source-grounded: when a Knowledge Base answer already contains the requested fact, customer-facing output may include one LLM-chosen engagement question only when it is specific to the user's topic and points to an adjacent grounded detail such as requirements, deadlines, exceptions, or the next rule. It must not assume the user is a student, applicant, personnel member, or admin; if clarification is needed, ask for the topic/program/unit/document rather than the user's identity. Generic assistant closing tails, role-assumptive department/status prompts, source-link preface tails, and vague "more info / contact the department" deferrals are stripped because they sound evasive after a correct answer. Policy-duration repair should extract matching list/bullet evidence, protect already-correct duration answers from unrelated eligibility repair, and standalone user questions should be searched before drifted router rewrites. Vector-search timeout responses from Supabase remain recoverable retrieval events and should be logged as warnings while lexical/focused evidence continues. The latest stricter YİÜ live challenge, including generic/role-assumptive closing checks, passed 33/33 at `tmp/crawl-output/live-yiu-challenge-pipeline-qa-2026-05-26T18-28-49-968Z.md`.
