...
> **Update Note (2026-03-28):** Inbox realtime should self-recover after long idle tabs. Browser resume events (`visibilitychange`, `focus`, `pageshow`, `online`) must reconcile the first conversation page plus the selected thread, and broken Supabase channel states (`CHANNEL_ERROR`, `TIMED_OUT`, `CLOSED`) must trigger the same recovery path so operators do not need a manual refresh to see new messages.
...
> **Last Updated:** 2026-03-28 (Inbox realtime now self-recovers after long idle tabs by reconciling the first conversation page plus the selected thread on browser resume events and broken Supabase channel states, so operators no longer need a manual refresh to see new messages; Instagram Inbox reaction events now persist structured reaction metadata, parse legacy raw `[Instagram reaction] ...` rows, and render as compact reaction rows with target-message-aware wording when the reacted provider message id resolves to a known outbound turn; lead extraction now classifies buying intent with a sector-agnostic semantic `intent_stage` contract (`none`, `informational_commercial`, `qualification`, `booking_ready`) instead of relying on static language-specific pricing heuristics, so first-message commercial inquiries can rise to `warm` before service confirmation while `hot` still requires stronger qualification evidence; lead extraction also continues to recover the approved canonical service from current conversation plus summary evidence when raw `service_type/services` are empty, required-intake repair scans recent customer turns for category-compatible answers including relative timeline phrasing such as `1 ay içinde`, and persisted lead scoring/status are recalibrated from merged extraction evidence so later acknowledgement-only turns cannot overwrite previously captured date/intent signals with `cold/0`; `Settings > Applications` now treats Google Calendar as a passive coming-soon integration on the management surface, using localized placeholder status copy such as `Google Takvim / Yakında` in Turkish and no clickable connect CTA yet; `Settings > Calendar` and `Settings > Applications` save actions now stay disabled until the active tab/surface actually has unsaved changes; `Settings > Plans` now keeps the one-time extra-credit purchase as the primary expansion action by moving the custom-package contact card directly beneath it and softening that CTA to a quieter secondary treatment; `Settings > Plans` extra-credit balance now keeps the original trial limit visible when premium extra credits come from trial carryover, so balances like `153.5` render against `200` instead of collapsing to `153.5 / 153.5`; settings navigation now preserves the shell on settings-to-settings transitions by skipping the global dashboard pending overlay for settings routes, scoping loading to the detail pane, and hydrating billing-lock nav state on the client so the settings layout no longer waits on billing snapshot reads; heavy settings detail routes now stream behind local `Suspense` boundaries so `Settings > Billing`, `Settings > Plans`, and `Settings > Organization` can show page chrome before the slowest server payload resolves, while `Settings > Plans` treats `billing_region` persistence as post-response `after()` work instead of a blocking GET-render mutation; settings bundle trimming now lazy-loads hidden organization-details sections and checkout legal-consent modals with `next/dynamic` so first-load JS stays focused on immediately visible UI; top-level workspace routes now lazy-load the heavy `Inbox`, `Calendar`, `Leads`, `Skills`, and `Knowledge` containers behind `next/dynamic`, and `Inbox` no longer waits for the initial selected-thread bootstrap before the route shell can paint; Supabase migration version prefixes are globally unique again, and the March 27 premium debit-order rollout now ships only through `00102_reassert_premium_usage_debits_topup_first.sql` to avoid duplicate-version push failures against remote `schema_migrations`; desktop dashboard typography now uses a slightly smaller shared content scale across operator surfaces while the desktop sidebar is nudged one step larger so navigation remains comfortably readable; desktop main sidebar now uses calmer Linear-style section headers with per-group collapse/expand state, more spacing between groups, softer active pills, and slightly smaller desktop nav typography while the collapsed rail keeps the full icon set visible; `Settings > Plans` now hides the extra-credit balance and combined-total cards once extra credits are exhausted so premium status falls back to the monthly package card only, while premium debit priority is still `extra credits first, then monthly package`; Inbox header utility controls now use plain icon chrome instead of bordered pill buttons, Inbox template picker now keeps first-open empty panes in a dedicated loading state with skeleton/copy feedback until template fetches resolve, Inbox selected-thread header now includes a manual read/unread toggle that preserves manually unread threads until manual re-read or a later revisit, cross-surface best-practices hardening now requires semantic Inbox/Knowledge/Skills interactions, effect-safe state handling in shared primitives plus calendar/settings/mobile-nav/auth flows, stable prefetch derivations, and indexed channel webhook lookups, Inbox operator-facing bot labels now keep the generic assistant name when the stored `bot_name` is still the default assigned value, Inbox mobile composer now keeps template/send actions icon-only on phone widths so the reply field holds the remaining width, and Calendar / scheduling / booking now includes a cleaner full-width operator workspace, a dedicated `Takvim ayarları` redirect from `/calendar`, top-level `Settings > Calendar` and `Settings > Applications` surfaces, compact business-hour editing, inline timing-rule help, simpler Google connection cards, a single calendar-settings entry in the workspace header, knowledge-aligned calendar header action sizing with a dark `Yeni randevu` CTA, modal-localized manual booking validation for past-date and slot-availability failures, client-side buffered calendar navigation for faster day/week/month switching, mutation-time cache invalidation, AI scheduling continuity across follow-up availability questions, stricter booking-intent gating, deterministic human handoff for booking-change requests and scheduling failures, backend booking-disable enforcement, minimum-notice enforcement across all scheduling paths, safe Google disconnect cleanup for mirrored future events, atomic availability replacement, per-booking duration override plus customer-email capture in the booking modal, shared select-field visual cleanup, full-width `Settings > Channels` gallery usage, Turkish-first UI copy cleanup, Instagram webhook/page-id hardening with pending-vs-verified readiness, state-consistent channel readiness messaging, production-safe Instagram Login webhook provisioning, direct-Instagram outbound DM sync into Inbox with business identity + avatar hydration, persisted Instagram share/story preview URLs for inline Inbox rendering when Meta includes a previewable asset link, limited unsupported-payload debug snapshots for Instagram attachment diagnosis, unsupported Instagram attachment fallback copy, image-aware inline Inbox preview fallback for image-like inbound file messages, caption/text-aware lead extraction for media-backed inbound turns, direction-aware outbound media preview labels, stricter Instagram request-badge classification, hidden Instagram secondary ids in details, aligned billing plan CTA rows, conservative generic required-intake fallback matching plus semantic custom-field resolution, inbox refresh latency hardening with parallel Instagram contact hydration + duplicate lead-refresh removal on thread switch + targeted query indexes, a title-row Inbox filter trigger for unread + customer-score views with reset support, colored score chips, read-on-leave behavior, and paginated Inbox filter backfill before empty-state rendering, direct post-auth/app-shell latency hardening with selective prefetch rebalancing, non-blocking Settings shell pending-suggestion hydration with stale-response guards, lighter shell refresh IO via unread existence checks, desktop-only sidebar subscriptions, reduced settings-link prefetch pressure, route-scoped dashboard i18n payloads, self-healing inbox sidebar preview hydration when realtime message inserts are missed, deploy-time chunk self-recovery, dashboard-wide intent-based route warming with immediate skeleton bridging, optimistic sidebar/mobile/settings active-state feedback, lightweight post-auth route resolution without admin org validation, a root dashboard loading boundary, slim active-org dashboard refreshes, non-blocking inbox thread-open hydration with parallel message/lead fetch, server-bootstrapped Inbox thread payload caching, client-cached Leads sort/search/page interactions, a pure-read Skills list path that seeds defaults only for empty workspaces, and aggregate Knowledge collection counts via RPC instead of document-row scans.)
...
> **Update Note (2026-03-18):** `Settings > Channels` now follows the same wide dashboard rhythm as `/calendar` and admin surfaces: the centered max-width shell is gone, and the gallery can expand to four columns on ultra-wide desktops.
...
- Inbox realtime must self-heal after long-lived tab inactivity. When the browser returns to the app (`visibilitychange`, `focus`, `pageshow`, `online`) or a Supabase channel reports a broken state (`CHANNEL_ERROR`, `TIMED_OUT`, `CLOSED`), the client must reconcile the first conversation page plus the selected thread so operators do not need a hard page refresh to see new messages.
...
- **Inbox Realtime Recovery Strategy (Implementation v1.55):** Treat websocket delivery as primary but not sufficient. Inbox must reconcile the first conversation page plus the selected thread on browser resume events (`visibilitychange`, `focus`, `pageshow`, `online`) and when Supabase channels enter broken states (`CHANNEL_ERROR`, `TIMED_OUT`, `CLOSED`) so long-lived operator tabs self-heal without manual reload.
...
- **WhatsApp Embedded Signup Post-Auth Provisioning (Implementation v1.33):** After Embedded Signup returns `auth code + waba_id + phone_number_id`, the server must exchange the code, call `/{phone_number_id}/register` for Cloud API use, then call `/{waba_id}/subscribed_apps` before the channel is marked active. When app URL + verify token are available, the WABA subscription should override the callback URL to the app’s `/api/webhooks/whatsapp` endpoint.
- **WhatsApp Embedded Signup PIN Management (Implementation v1.33):** Embedded Signup completion derives a deterministic 6-digit two-step verification PIN from server-side inputs, uses it during Cloud API registration, and persists it in channel config so reconnect/retry flows remain deterministic without adding a new operator-facing setup field.
...
- **WhatsApp Guided Wizard IA (Implementation v1.27):** The dedicated WhatsApp onboarding page should open with three clear choices (`new API account`, `existing API account`, `WhatsApp Business app number migration`), run a phone-number eligibility check before launching Meta for new-account setup, require an explicit `Meta Business Manager access` confirmation screen before the final `Connect with Facebook` launch step, and split `existing API account` into `Meta assets already in Meta` vs `another BSP migration` before launch.
- **WhatsApp Support CTA:** The onboarding support banner should provide a direct human-contact CTA (`Ekiple konuş` / `Talk to support`) that opens an email draft (`mailto:askqualy@gmail.com`) instead of static helper text only.
