...
- **Social Contact Avatars (Implementation v1.30):** Persist a nullable `conversations.contact_avatar_url` and render it through the shared Inbox avatar surface. Instagram and Telegram hydrate best-effort customer photos from channel APIs; WhatsApp currently remains initials-only because webhook/customer APIs do not expose a supported avatar source.
