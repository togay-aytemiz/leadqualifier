...
> **Update Note (2026-03-31):** WhatsApp Business app / coexistence Embedded Signup completion must recover from long-running Meta flows where the popup returns an auth code but the client misses or times out waiting for the final `FINISH` postMessage. In that case, the server must still discover the granted WABA/phone assets from the exchanged token, persist the local channel row, and let Inbox/webhook provisioning proceed instead of leaving Qualy disconnected while the phone already shows Business Platform linked.
...
> **Last Updated:** 2026-03-31 (WhatsApp Embedded Signup now requires auth-code-based recovery when long-running Business app onboarding misses the final Meta `FINISH` event, alongside localized sign-in credential errors, the dark collapsed desktop sidebar rail with immediate slim-mode tooltips, and the `Settings > Channels` `WhatsApp`, `Instagram`, `Facebook Messenger`, `Telegram` order with a maximum of 3 columns on wide desktop.)
