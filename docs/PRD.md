...
> **Update Note (2026-05-27):** RAG duration repair should keep extracted answers customer-readable even when the supporting PDF chunk is a dense curriculum/course table. For staj/duration questions, a long row containing many course names, credits, and percentages may be compacted to the matching `Yaz Stajı` label plus the explicit duration value before replying, while still requiring subject and duration evidence from the retrieved chunk. This is a general table-evidence cleanup rule, not a YİÜ-only phrase map.
