...
> **Last Updated:** 2026-04-24 (runtime AI-settings reads now fail closed for customer-facing webhook paths so settings load drift cannot make `Dinleyici` mode send a bot reply; dashboard performance hardening still keeps current-route/click-time manual prefetch races closed, and calendar capacity-aware booking ships the simple org-level `max_concurrent_bookings` model.)
