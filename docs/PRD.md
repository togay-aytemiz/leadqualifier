...
- **Focused RAG Latency Budget (Update 2026-06-01):** A deterministic focused evidence pass should stay planner-free when the caller already supplies an expanded contextual query; LLM query planning remains available in broad fallback where its semantic variants justify the latency. Focused evidence helpers should prefer high-signal required-term probes and skip broad OR scans when those probes already ground the answer, so latency improvements stay general retrieval behavior rather than organization- or phrase-specific shortcuts.
