...
> **Update Note (2026-03-30):** WhatsApp disconnect must no longer behave like a silent local-only delete. For Cloud API numbers, Qualy should only remove the local channel after provider-side phone deregistration succeeds; when Meta returns the official coexistence limitation, the UI must keep the channel intact and instruct the operator to disconnect Cloud API inside WhatsApp Business app `Settings > Account > Business Platform` first.
...
- **WhatsApp Disconnect Contract (Implementation v1.47):** `Disconnect` must perform a real provider-side shutdown for WhatsApp instead of deleting only the local row. Qualy should call Cloud API phone deregistration before deleting the channel record; if Meta rejects deregistration because the number is in coexistence with WhatsApp Business app, the channel must remain connected locally and the UI must tell the operator to disconnect Cloud API from WhatsApp Business app `Settings > Account > Business Platform` first.
