...
> **Update Note (2026-05-27):** Public Demo Chat should keep source links traceable without showing long PDF URLs as the visible answer text. Assistant replies may render standalone raw source URL lines with a localized short label (`Daha fazla oku` / `Read more`) on the demo surface only, while persisted text and non-demo channel renderers continue to preserve raw URLs.
...
> **Last Updated:** 2026-05-27 (Yüksek İhtisas demo RAG now uses generic evidence-first retrieval, original-query-first retrieval-only LLM query planning with a bounded planner timeout and strong-evidence short-circuit for planned variants, standalone subject-cue original-query preservation against router/history drift, query-intent scoped focused evidence probes, required-evidence-first campus/location and medical-school policy scans before broad OR fallback, high-confidence current-campus evidence short-circuiting before broad address scans, acronym-to-expanded-title initialism matching for academic-unit retrieval and duration repair, title-initialism abbreviation recovery from retrieved document titles, final/bütünleme adjacent-policy drift repair, in-year board-grade formula repair, compact course-table duration repair for staj/curriculum evidence, complete multi-bracket policy-duration repair for broad questions, bounded evidence-query timeouts, immediate pending Demo Chat POSTs, polling-time deterministic extractive KB recovery before full pipeline fallback, stale-token-safe dynamic Demo Chat page rendering, expired-tab Demo Chat token refresh, academic-unit scoped internship evidence, deterministic answer repair, source-link cleanup, topic-related engagement guidance with generic closing-tail cleanup, recoverable vector-timeout warning logs, direct extractive address/location answers, timeout extractive recovery, high-signal keyword extraction for conversational questions, URL line-break normalization for chunking/corpus hygiene, indexed lexical fallback search, evidence-first answer repair for eligibility/unit contact/factual policy questions, and live screenshot-plus-variant QA checks so uploaded PDF/crawl evidence answers customer-style questions instead of falling back.)
