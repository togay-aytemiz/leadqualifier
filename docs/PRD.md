...
> **Last Updated:** 2026-03-05 (Iyzico billing provider integration moved from mock to live checkout flows for Plans: recurring subscription checkout now uses Iyzico subscription checkout form with in-app hosted checkout surface + callback finalization, and one-time top-up purchases now use Iyzico checkout form payment page + callback finalization with idempotent credit/package grant application; trial abuse controls advanced: one-trial-per-business policy is now enforced using business fingerprint signals (`company_name`, `email_domain`, WhatsApp `business_account_id`, normalized phone) with duplicate-trial blocking at signup/initialization and trial-lock enforcement on WhatsApp connect; Sign Up also enforces `email + IP` velocity throttling with cooldown windows backed by RPC guard functions, records success/failure attempts for escalation, and supports optional Turnstile CAPTCHA verification when configured; Sign Up completion now routes unconfirmed accounts to a dedicated check-email checkpoint page that shows submitted address, spam/junk guidance, quick edit-email return, and sign-in shortcut; Inbox scrolling refined: jump-to-latest smooth animation restored, thread now remains pinned to actual latest after dynamic composer-height changes (e.g., WhatsApp 24-hour expired panel) and lazy-loaded image reflow, and long conversation history now lazy-loads while scrolling upward with stable viewport restore; QA Lab intake-coverage semantic fallback bug fixed: context-style service fields (e.g., `Talep bağlamı`) now accept broad semantic fulfillment fallback after explicit ask, so contextual customer replies are not falsely marked missing; QA Lab request-mode detection Unicode normalization bug fixed: Turkish dotted-uppercase inputs (e.g., `İptal ...`) now normalize without token splitting, so explicit policy/procedure intent is not misclassified as `general_information`; live intake blocked re-ask regression fixed: refusal/no-progress flow now accepts partial-but-strong field cues for previously asked fields to prevent repeated pressure asks, and follow-up guidance explicitly enforces “do not insist”; Inbox composer now supports WhatsApp outbound image/document attachments with multi-select (`max 10`), clipboard image paste-to-attach (up to remaining slot count), pre-send attachment preview/remove flow, signed-upload media transfer, and optimistic send status (`sending`/`failed`) in chat timeline with immediate local image preview in outbound bubbles; lead extraction now uses text-only conversation turns by excluding WhatsApp media-placeholder messages from extraction context; WhatsApp inbound media support now includes `image`/`document` parsing in webhook runtime, media download + storage persistence, and Inbox media rendering with localized conversation-list media previews; consecutive image-only WhatsApp messages now render as grouped gallery bubbles (2/3/4+ layout) for cleaner multi-image threads; Settings > Usage now includes storage totals with `Skills + Knowledge + WhatsApp media` breakdown, and Admin organization list/detail now shows per-org storage with WhatsApp media breakdown; AI Settings now includes a channel-wide bot disclaimer toggle with localized TR/EN editable text (default enabled); outbound bot replies in WhatsApp/Telegram/Instagram now append disclaimer text as a blockquote line (`\n\n> ...`) when enabled; Inbox bot messages now hide this trailing disclaimer from UI while keeping outbound payload unchanged (LF/CRLF tolerant parsing); skill-matched bot replies now carry/display matched `skill_title` metadata in Inbox footer area (object or JSON-string metadata payloads), and raw `skill_id`/UUID values are never shown as footer labels; runtime backfills title from `skills.title` when matcher title is missing; matched skills are evaluated in ranked top-5 order and first successful match is applied directly, and `requires_human_handover=true` always triggers operator escalation for that matched skill (no extra handover-intent guard); AI Settings `Sensitivity` control is grouped under `Behavior and Logic` (not `General`); `Lead extraction during operator` is grouped under `Escalation`; Settings nav/page label is standardized as `Qualy AI`; AI Settings keeps reusable animated tabs with 3 grouped areas: `General`, `Behavior and Logic`, and `Escalation`, where Escalation is split into primary `Automatic Escalation` + `Skill Based Handover` title-only sections; Organization Settings now uses 3 tabs (`General`, `Organization Details`, `Security & Data`) with grouped content; AI/Organization settings content starts directly with tabs (no top intro description); Inbox template picker remains mobile-optimized with underline tabs, WhatsApp-only refresh action, inset chevrons, and smooth tab resize animation; Inbox message-day badges now render from message timestamps (`Today`/`Yesterday`/localized full date) instead of a static `Today` chip; lead status model is simplified to `cold/warm/hot` only and legacy `ignored/undetermined` values normalize to `cold`; Inbox queue now uses `All / Unassigned / Me` tabs with single circular badge presentation (red when attention exists), and conversation rows show compact red `!` attention markers only on `All` tab (hidden on `Unassigned`/`Me`) with tooltip reason details; main sidebar bot status now opens a filled quick-switch dropdown with inline mode explanations and a settings shortcut; collapsed sidebar hover UX now shows immediate custom tooltips for bot/nav controls and the bot dropdown open/close is animated; mobile Inbox header now exposes bot status as a pill that opens a quick-switch bottom sheet with mode explanations; mobile bottom navigation now groups Skills + Knowledge Base under an `AI` tab sheet; skills now support up to 3 WhatsApp quick-action buttons (`trigger_skill` or `open_url`) and inbound button replies resolve deterministically; WhatsApp onboarding strategy is now clarified as existing-number-first, concierge-assisted by default, and must not assume prior `business.facebook.com` usage.)  
...
- Instagram MVP supports text messages only and sends replies reactively to inbound customer messages (no proactive/template-initiated flow in MVP)
- WhatsApp MVP supports reactive inbound text plus media (`image`/`document`) capture. Media-only inbound events are persisted to Inbox without automatic AI reply; captioned media continues through normal AI flow.
...
- **Plan Purchase (Online Payment):**
  ...
  - Mock simulation flow remains as a fallback when `BILLING_PROVIDER=mock` (local/dev safety and rollback path).
  - Add payment webhook lifecycle handling (success, failure, renewal, cancellation, retry) as a follow-up hardening step.
...
- **Iyzico Checkout Finalization Strategy (Implementation v1):** Initial rollout finalizes both subscription and top-up purchases from checkout callback token retrieval (`retrieve` APIs) and applies idempotent ledger/account updates server-side before user redirect; webhook lifecycle sync remains a follow-up hardening phase.
...
- **WhatsApp Onboarding Entry (MVP):** Do not send every user directly into Meta OAuth. Start with an in-app preflight that assumes most SMBs already use WhatsApp Business on an existing number, then route only explicitly eligible cases into direct Meta connect.
...
- **WhatsApp Existing-Number Onboarding (MVP):** Optimize for businesses that already use WhatsApp Business App on their current number. Do not assume prior `business.facebook.com` usage; onboarding must support users who need to create or connect a Meta business portfolio during assisted setup, and existing-number cases must be treated as migration/coexistence-style onboarding rather than generic new-number registration.
...
| Onboarding | **Concierge-first (MVP)** | Existing-number-first assisted WhatsApp setup for 5 pilots; direct Meta connect only after preflight/eligibility |
