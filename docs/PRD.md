...
> **Last Updated:** 2026-05-25 (Public Demo Chat composer now keeps mobile input text at an iOS-safe 16px baseline; Yüksek İhtisas demo RAG also uses generic evidence-first retrieval, deterministic answer repair, PDF type backfill, canonical source-link cleanup, timeout extractive recovery, reasoning-model-compatible RAG completion, and M1/live 33-question screenshot-plus-variant QA checks across `gpt-5-mini`, `gpt-4o`, and `gpt-4o-mini` so uploaded PDF/crawl evidence answers customer-style questions instead of falling back.)
