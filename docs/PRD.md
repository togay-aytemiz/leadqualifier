...
> **Last Updated:** 2026-04-07 (Billing history now filters directly to purchase ledger entry types so usage debits cannot hide successful purchases; Iyzico direct plan-change calls now use the documented signed REST upgrade body including `resetRecurrenceCount=false`, and upgrade charge displays no longer invent local delta amounts without provider order evidence.)
