...
> **Update Note (2026-03-31):** WhatsApp template tooling must be unmistakably separate from Qualy's generic saved-message templates. `Settings > Channels > WhatsApp` should label this surface as `WhatsApp Templates`, explain that it uses Meta-approved WABA templates only, and expose a guide link so first-time operators understand that a connected WhatsApp Business Account (WABA) is required before templates appear here.
...
> **Update Note (2026-03-31):** WhatsApp disconnect must treat coexistence/provider limitations as expected operator-facing outcomes, not uncaught server-action failures. If Cloud API deregistration is blocked because the number is still attached to WhatsApp Business app, the UI should stay on the page and render the localized guidance without surfacing a generic `500 Internal Server Error`.
...
> **Last Updated:** 2026-03-31 (WhatsApp template UX now explicitly separates Meta WABA templates from Qualy's normal saved messages and links first-time operators to WABA guidance, pending WhatsApp channels now support webhook retry without disconnect plus first-inbound self-verification, WhatsApp Embedded Signup now requires auth-code-based recovery plus short-grace fallback after missing Meta `FINISH`, WhatsApp disconnect now keeps coexistence failures as localized in-page guidance instead of raw 500s, alongside localized sign-in credential errors, the dark collapsed desktop sidebar rail with immediate slim-mode tooltips, and the `Settings > Channels` `WhatsApp`, `Instagram`, `Facebook Messenger`, `Telegram` order with a maximum of 3 columns on wide desktop.)
...
- **WhatsApp Disconnect Error Surface (Implementation v1.49.2):** Known disconnect blockers such as coexistence and provider-side deregistration failure are expected application outcomes. Server actions should return structured error results for these cases so the settings page can keep rendering localized inline guidance without emitting a generic action-level `500` response.
