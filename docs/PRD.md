...
> **Last Updated:** 2026-03-31 (WhatsApp Embedded Signup now requires auth-code-based recovery plus short-grace fallback after missing Meta `FINISH`, WhatsApp disconnect now keeps coexistence failures as localized in-page guidance instead of raw 500s, alongside localized sign-in credential errors, the dark collapsed desktop sidebar rail with immediate slim-mode tooltips, and the `Settings > Channels` `WhatsApp`, `Instagram`, `Facebook Messenger`, `Telegram` order with a maximum of 3 columns on wide desktop.)
