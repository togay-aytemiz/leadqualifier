...
> **Update Note (2026-05-15):** Customer-facing bot replies must be formatted for the destination messenger before send/persist. WhatsApp converts double-star Markdown emphasis to WhatsApp-friendly single-star emphasis, Instagram strips Markdown markers, Telegram sends plain text unless a parse mode is explicitly configured, inline ` - ` RAG list artifacts are converted to line-broken bullets, and raw URLs are preserved for all channels.
