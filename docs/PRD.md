...
> **Update Note (2026-05-15):** Customer-facing chat-channel replies must defensively normalize malformed link output before send/persist. Markdown-style links with spaced URLs and raw URLs with accidental spaces inside the domain must be converted into visible raw URLs without whitespace, and RAG prompts must instruct models to copy source URLs exactly without inserting spaces.
...
> **Last Updated:** 2026-05-15 (Admin manual billing now displays remaining credits consistently, validates premium package grants, and disables repeated billing submits while processing.)
