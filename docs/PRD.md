...
> **Last Updated:** 2026-05-25 (Yüksek İhtisas demo RAG now uses generic evidence-first retrieval, deterministic answer repair, PDF type backfill, canonical source-link cleanup, timeout extractive recovery, and M1/live 33-question screenshot-plus-variant QA checks so uploaded PDF/crawl evidence answers customer-style questions instead of falling back.)
...
- **Demo Chat Composer UX (Update 2026-05-21):** Public Demo Chat uses an autosizing textarea rather than a fixed text input so longer demo questions can expand naturally on mobile. The composer keeps Enter-to-send, Shift+Enter newline behavior, hidden native scrollbars for capped long text, and one bottom-right send action only, with no attachment button or instant-action dropdown in the public demo surface.
