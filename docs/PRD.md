...
> **Update Note (2026-05-28):** Knowledge Base RAG should use model reasoning for answer wording without trusting unsupported output. Public Demo Chat recovery and the shared inbound RAG pipeline now try a reusable grounded answer generator before deterministic repair/free-form completion. The generator must return exact support quotes from retrieved context, apply organization AI voice instructions, reject missing support or invented critical values, and allow one role-neutral engagement question only when its evidence quote exists in the same context. Deterministic repair remains a fallback for bounded recovery, not the default answer author.
