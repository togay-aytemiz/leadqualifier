...
| RAG Query Planning | **Retrieval-only LLM planner in auto mode** | For noisy, abbreviated, policy-like, or conversational KB questions, the planner may create search variants and must-have evidence terms, but it must not generate answer facts; final answers still come only from retrieved chunks. Planner calls fail fast after a default 2.5s budget (`KNOWLEDGE_QUERY_PLANNER_TIMEOUT_MS`) and can be disabled with `KNOWLEDGE_QUERY_PLANNER_ENABLED=false`. |
