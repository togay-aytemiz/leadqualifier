...
> **Last Updated:** 2026-05-26 (Yüksek İhtisas demo RAG now uses generic evidence-first retrieval, feature-flagged retrieval-only LLM query planning with a bounded planner timeout and strong-evidence short-circuit for planned variants, academic-unit scoped internship evidence, deterministic answer repair, source-link cleanup, topic-related engagement guidance with generic closing-tail cleanup, recoverable vector-timeout warning logs, timeout extractive recovery, high-signal keyword extraction for conversational questions, URL line-break normalization for chunking/corpus hygiene, indexed lexical fallback search, evidence-first answer repair for eligibility/unit contact/factual policy questions, and live screenshot-plus-variant QA checks so uploaded PDF/crawl evidence answers customer-style questions instead of falling back.)
...
| RAG Query Planning | **Retrieval-only LLM planner in auto mode** | For noisy, abbreviated, policy-like, or conversational KB questions, the planner may create search variants and must-have evidence terms, but it must not generate answer facts; final answers still come only from retrieved chunks. Planner calls fail fast after a default 2.5s budget (`KNOWLEDGE_QUERY_PLANNER_TIMEOUT_MS`), planned variants are skipped when the original query already returns enough strong grounded evidence, and planning can be disabled with `KNOWLEDGE_QUERY_PLANNER_ENABLED=false`. |
