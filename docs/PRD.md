...
> **Last Updated:** 2026-04-07 (Pending Meta channel surfaces now explicitly tell the operator to send a test message to the connected WhatsApp/Instagram account and explain that Qualy auto-verifies the connection after the first inbound message; returning focus to the app refreshes pending channel surfaces; onboarding completion still requires a neutral explicit bot-mode choice instead of auto-switching to `Dinleyici`; onboarding-complete workspaces with sparse legacy unlock metadata now still resolve to effective `Kapalı` until the operator chooses a mode; Skills now support one optional uploaded image per skill with a `5 MB` original-file limit, client-side `WebP` conversion, explicit loading/disabled editor state during processing, text-first-then-image delivery across WhatsApp/Instagram/Telegram, simulator parity, and non-blocking image-failure handling that still preserves operator-visible errors plus `requires_human_handover` escalation after the media attempt; structured `Yapay zeka asistan talimatları` now replace the raw `AI Prompt` field inside `Settings > Qualy AI > Behavior and Logic`, giving operators four optional instruction blocks plus a `Nasıl çalışır?` modal while preserving old custom prompts via `Diğer talimatlar`; customer-facing AI replies now consume a compiled instruction model, but internal extraction/scoring/routing/summary prompts remain unchanged; default guardrail skills are now provisioned at organization owner-membership time with locale-aware create-time seeding plus backfill for older workspaces; dashboard chunk recovery now preserves the pending navigation target, so sidebar/mobile/settings transitions that hit a real Next chunk failure retry the intended route instead of reloading the currently visible page and making navigation appear stuck; trial countdown UI now uses calendar-day remaining values that match the displayed end date across sidebar, plans, and dashboard banner surfaces, so partial-day remainder no longer rounds `2 Apr → 14 Apr` up to `13 days`; Organization Settings now surfaces a localized non-blocking AI extraction banner whenever Knowledge Base-derived business setup is still processing, keeps manual edits available, and auto-refreshes when service profile / required fields / service list extraction finishes; trial-active workspaces also use a global onboarding banner plus an organization-scoped onboarding checklist with first-visit auto-open, desktop/mobile navigation entry while incomplete, real setup-derived completion, a little extra spacing between the banner and onboarding content, and automatic expansion of the next incomplete step only on first entry; onboarding chrome is now localized as `Getting started` / `Başlangıç`, the desktop sidebar surfaces it as a highlighted purple top card above `Workspace` with a plain purple rocket icon, the onboarding page greets the user by name with simplified copy, the intro CTA is localized as `Devam et`, the progress bar and primary actions now use the same purple onboarding theme, the onboarding content now follows a cleaner respond.io-style left-aligned accordion layout without extra inner wrappers, stacked single-column step bodies, compact filled icon rows, outline/filled state icons with better vertical centering, and no intro chips, Turkish copy uses product terminology such as `Bilgi Bankası` and `Yetenekler`, the intro copy is now business-focused and explicitly mentions appointment flow, completed checklist steps remain reopenable and manually collapsed steps stay closed, the AI settings step now emphasizes bot-name personalization plus reply/handoff behavior instead of prematurely asking for bot-mode review, the final checklist step previews connectable channels with direct larger icons and marks `Messenger` as coming soon, the banner copy/alignment stays trial-focused with bold remaining-days emphasis, and the final checklist step completes when any ready live channel is connected rather than assuming WhatsApp only; intro acknowledgement now opens the next step immediately and no longer crashes the UI if the onboarding-state relation is temporarily missing while shell state is refreshing, the first successful Knowledge Base document create now pauses on a one-time educational modal that explains automatic AI extraction for business profile / required fields / starter service list before routing back to `Başlangıç` or into `Organization Details`, and newly created workspaces now keep bot mode effectively `Kapalı` until onboarding reaches `5/5`, with sidebar/settings locks plus a required unlock modal that explicitly chooses `Dinleyici`, `Aktif`, or `Kapalı`; bot-mode lock UX now also follows live onboarding progress, and its message is now rendered once as a bold violet banner with fully disabled option cards and no duplicate helper copy while the workspace is still incomplete; live Iyzico `past_due` subscriptions now expose self-serve payment-method update and payment-retry recovery actions from `Settings > Plans`, while access remains locked until webhook confirmation, the shared settings layout now redirects missing-auth Iyzico return visits to login instead of rendering a blank white page, localhost billing returns now stay on the active local origin instead of being forced to production `NEXT_PUBLIC_APP_URL`, immediate Iyzico upgrades now preserve the existing renewal anchor by sending `resetRecurrenceCount=false` with `upgradePeriod=NOW`, the upgrade confirmation popup now explicitly shows full delta charge, full delta credits, and unchanged next renewal date, `Settings > Plans` now keeps one dominant subscription CTA by moving cancel into the manage-plan modal and rendering payment recovery as lighter text actions, direct plan changes now use a lighter one-confirmation modal with linked legal references instead of repeated mandatory legal re-acceptance copy, `Settings > Plans` now also surfaces the next renewal date directly, a lightweight `Geçmiş / History` modal for successful purchases and renewals, a minimal billing-profile form that pre-fills Iyzico checkout customer/address fields, a lighter billing-info layout where only the next-renewal box stays wrapped while history/update actions sit directly below it as underlined text actions, in-app subscription plus one-time extra-credit price surfaces now explicitly state that displayed amounts include VAT/KDV across both subscriber and non-subscriber pricing UI, top-up checkout now follows the same hosted in-app embed route as subscription checkout instead of opening a separate full-page provider flow, and the active monthly-package card now uses `Yenileme / Renews at` wording instead of `Sıfırlanma / Resets at`; the hosted card-update page now includes localized guidance plus a direct return path back to Plans; WhatsApp disconnect now removes only Qualy's local channel relationship without requiring provider-side deregistration, and the operator-facing confirm copy explicitly says it does not disconnect the customer's WhatsApp Business app/Meta setup; WhatsApp template UX explicitly separates Meta WABA templates from Qualy's normal saved messages and keeps WABA setup/help inside the `How to use` guide; nested guide close no longer dismisses the parent template modal; WhatsApp onboarding defaults to the recommended WhatsApp Business app/WABA path with simpler copy; pending WhatsApp channels support webhook retry without disconnect plus first-inbound self-verification; WhatsApp Embedded Signup requires auth-code-based recovery plus short-grace fallback after missing Meta `FINISH`; legal credit wording now distinguishes monthly package credits from one-time extra credits, keeping monthly package credits non-rollover while extra credits remain active-subscription-only balance until consumed; alongside localized sign-in credential errors, the dark collapsed desktop sidebar rail with immediate slim-mode tooltips, and the `Settings > Channels` `WhatsApp`, `Instagram`, `Facebook Messenger`, `Telegram` order with a maximum of 3 columns on wide desktop.)
...
- **Skill Image Delivery Contract (Implementation 2026-04-06):** Store one optional image metadata payload directly on `skills`, upload only through Qualy’s in-app flow, enforce `5 MB` max original size, normalize to `WebP` client-side before storage, derive the active organization server-side before any storage write/delete, use unique object keys per upload, and deliver matched skill replies as `text -> image -> optional human handover`. Image failures must not cancel the text reply, but they must remain visible to operators and in simulator/runtime parity.
...
- **Skill Image Delivery (Implementation 2026-04-06):** Each skill may include one optional uploaded image. Enforce a `5 MB` original-file limit, normalize to `WebP` on the client before upload, scope storage upload/delete to the active organization server-side, generate unique upload object keys, and send matched skill replies as two sequential outbound turns: text first, then image. The Skills editor must show a visible loading/disabled state while the asset is processing or uploading, and Simulator should mirror the same order.
