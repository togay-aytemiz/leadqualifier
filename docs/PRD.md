...
> **Update Note (2026-05-26):** Public Demo Chat polling must not depend on long-running completion-model recovery when RAG evidence can already produce a deterministic answer. Polling `GET` may first run a bounded extractive Knowledge Base recovery path that searches the tenant corpus, repairs a no-information seed from retrieved chunks, appends canonical source links, and persists the bot row with the originating demo message id. If this extractive path cannot produce a grounded answer, the normal shared inbound AI pipeline remains the fallback; no customer/demo-specific phrase rules should be introduced.
