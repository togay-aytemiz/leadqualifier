...
> **Last Updated:** 2026-03-23 (Calendar / scheduling / booking now includes a cleaner full-width operator workspace, a dedicated `Takvim ayarları` redirect from `/calendar`, top-level `Settings > Calendar` and `Settings > Applications` surfaces, compact business-hour editing, inline timing-rule help, simpler Google connection cards, a single calendar-settings entry in the workspace header, client-side buffered calendar navigation for faster day/week/month switching, mutation-time cache invalidation, AI scheduling continuity across follow-up availability questions, stricter booking-intent gating, deterministic human handoff for booking-change requests and scheduling failures, backend booking-disable enforcement, minimum-notice enforcement across all scheduling paths, safe Google disconnect cleanup for mirrored future events, atomic availability replacement, per-booking duration override plus customer-email capture in the booking modal, shared select-field visual cleanup, full-width `Settings > Channels` gallery usage, Turkish-first UI copy cleanup, Instagram webhook/page-id hardening with pending-vs-verified readiness, state-consistent channel readiness messaging, production-safe Instagram Login webhook provisioning, direct-Instagram outbound DM sync into Inbox with business identity + avatar hydration, unsupported Instagram attachment fallback copy, direction-aware outbound media preview labels, stricter Instagram request-badge classification, hidden Instagram secondary ids in details, aligned billing plan CTA rows, conservative generic required-intake fallback matching plus semantic custom-field resolution, inbox refresh latency hardening with parallel Instagram contact hydration + duplicate lead-refresh removal on thread switch + targeted query indexes, direct post-auth/app-shell latency hardening with selective prefetch rebalancing, non-blocking Settings shell pending-suggestion hydration with stale-response guards, lighter shell refresh IO via unread existence checks, desktop-only sidebar subscriptions, reduced settings-link prefetch pressure, route-scoped dashboard i18n payloads, self-healing inbox sidebar preview hydration when realtime message inserts are missed, and intent-based Inbox/Leads route warming with immediate dashboard skeleton bridging.)
> **Update Note (2026-03-23):** Inbox and Leads navigation must feel immediate in production. Desktop sidebar and mobile bottom-nav interactions now warm only these hot routes on explicit user intent (`hover/focus/touch/click`), and the dashboard shell renders the destination skeleton immediately while the App Router is still resolving the next payload.
...
- **Hot Route Transition Strategy (Implementation v1.43):** Keep global shell prefetch conservative, but treat `/inbox` and `/leads` as explicit hot routes. Warm them only on strong user intent (`hover/focus/touch/click`) and bridge slow server responses with a dashboard-level route skeleton overlay instead of reintroducing broad mount-time prefetch floods.
