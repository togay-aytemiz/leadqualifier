...
> **Update Note (2026-05-21):** University-pilot RAG must prefer the exact governing policy sentence over broad semantically related staff pages for duration questions. For YİÜ-style `personelin ücretsiz izin süresi ne kadar` turns, retrieval should keep the `İzin Kullanımı Yönergesi` clause `Ücretsiz izin süresi en fazla 1 (bir) yıldır` inside the RAG context, demote Erasmus/personel-hareketliliği and annual-paid-leave distractions, and repair model no-information wording from that explicit retrieved evidence.
...
> **Last Updated:** 2026-05-21 (YİÜ staff unpaid-leave duration answers now require the exact `1 (bir) yıl` policy evidence to remain in RAG context and can be repaired from that retrieved sentence.)
