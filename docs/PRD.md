...
> **Last Updated:** 2026-03-25 (Calendar / scheduling / booking now includes a cleaner full-width operator workspace, a dedicated `Takvim ayarları` redirect from `/calendar`, top-level `Settings > Calendar` and `Settings > Applications` surfaces, compact business-hour editing, inline timing-rule help, simpler Google connection cards, a single calendar-settings entry in the workspace header, client-side buffered calendar navigation for faster day/week/month switching, mutation-time cache invalidation, AI scheduling continuity across follow-up availability questions, stricter booking-intent gating, deterministic human handoff for booking-change requests and scheduling failures, backend booking-disable enforcement, minimum-notice enforcement across all scheduling paths, safe Google disconnect cleanup for mirrored future events, atomic availability replacement, per-booking duration override plus customer-email capture in the booking modal, shared select-field visual cleanup, full-width `Settings > Channels` gallery usage, Turkish-first UI copy cleanup, Instagram webhook/page-id hardening with pending-vs-verified readiness, state-consistent channel readiness messaging, production-safe Instagram Login webhook provisioning, direct-Instagram outbound DM sync into Inbox with business identity + avatar hydration, persisted Instagram share/story preview URLs for inline Inbox rendering when Meta includes a previewable asset link, unsupported Instagram attachment fallback copy, direction-aware outbound media preview labels, stricter Instagram request-badge classification, hidden Instagram secondary ids in details, aligned billing plan CTA rows, conservative generic required-intake fallback matching plus semantic custom-field resolution, inbox refresh latency hardening with parallel Instagram contact hydration + duplicate lead-refresh removal on thread switch + targeted query indexes, a title-row Inbox filter trigger for unread + customer-score views with reset support, colored score chips, and read-on-leave behavior, direct post-auth/app-shell latency hardening with selective prefetch rebalancing, non-blocking Settings shell pending-suggestion hydration with stale-response guards, lighter shell refresh IO via unread existence checks, desktop-only sidebar subscriptions, reduced settings-link prefetch pressure, route-scoped dashboard i18n payloads, self-healing inbox sidebar preview hydration when realtime message inserts are missed, deploy-time chunk self-recovery, dashboard-wide intent-based route warming with immediate skeleton bridging, slim active-org dashboard refreshes, and non-blocking inbox thread-open hydration with parallel message/lead fetch.)
...
- **Inbox Header Filters:** Inbox title row exposes a single filter trigger for `All / Unread` plus `All / Hot / Warm / Cold`; the menu must stay mobile-safe, include one inline reset action, and filter against live client state so matching conversations can appear immediately after realtime updates.
...
> Finalized: 2026-01-31 (updated with implementation decisions through 2026-03-22)
