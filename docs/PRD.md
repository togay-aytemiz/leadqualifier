...
> **Update Note (2026-06-01):** Public Demo Chat meta questions about what the tester can ask should be answered deterministically from the demo scope before starting RAG or shared inbound AI recovery. This keeps scope-help turns responsive and lets already-pending polls recover instead of remaining in repeated `202 pending` status.

> **Update Note (2026-06-01):** Knowledge Base RAG answer writing should be separated from evidence selection. Retrieved chunks should first become a typed Evidence Pack with source-backed facts, and the LLM answerer should cite selected evidence ids rather than writing from broad raw context. Customer-visible source links should come from evidence actually used in the answer. The next retrieval iteration should use hybrid Reciprocal Rank Fusion across vector, lexical, title/source, and focused-evidence channels, with local/test UI smoke required before production deployment.

> **Update Note (2026-06-01):** Public Demo Chat source links must support the concrete platform wording in the answer. When a grounded answer names a short learning platform such as UZEM, MEDU, or ÖBS, the canonical source-link filter treats that platform name as concrete evidence and should not append secondary sources whose chunks omit the named platform, even if those chunks match the broader course-material topic. When a final reply contains canonical source URLs, persisted RAG `metadata.sources` should mirror the document ids behind those visible URLs instead of every retrieved chunk.

> **Update Note (2026-06-01):** Focused contact-answer repair should score requested subject acronyms independently from inflected generic role words. Turkish forms such as `sorumlusunun` must be treated as contact-role stopwords before acronym construction, preventing a requested program acronym like `TLT` from being blended into an unrelated acronym and selecting the next contact-table row's e-mail.

> **Update Note (2026-06-01):** Public Demo Chat focused RAG fast paths should not invoke the LLM query planner after the route has already built a deterministic context-expanded query; planner latency belongs to broad fallback when focused evidence cannot answer. Focused evidence retrieval should prefer selective required-term queries for high-signal lecture/course-material access questions before broad OR scans, scope those probes to named academic units when the question supplies one, and health-report/mazeret-exam retrieval should return from required remedy evidence without running the broad health-policy OR scan when the required evidence already grounds the answer.

> **Update Note (2026-05-29):** Public Demo Chat RAG follow-up retrieval should spend the first retry on context-expanded focused evidence before falling back to broad/planned search, so ambiguous turns can stay anchored to the latest explicit topic without unnecessary latency. Demo bot replies should persist `rag_diagnostics` metadata with search strategy, query count, retrieved chunk count, deterministic fast-path usage, micro-polish usage, and timing buckets. Deterministic micro-polish is allowed only for source-safe presentation cleanup, such as expanding a terse course-code answer from retrieved document-title initialism evidence; it must not add facts, unsupported engagement, or role assumptions.

> **Update Note (2026-05-29):** Contextual deterministic repair should use the context-expanded retrieval question for evidence extraction while preserving the user's original message for conversation/generation. The first contextual focused pass should try only the strongest expanded query before broad fallback, instead of waiting on weaker bare follow-up probes. Source-link ranking must also recognize Turkish word-number durations such as `altı yıldır` as concrete answer evidence, so compound answers that combine facts from separate documents cite each supporting source instead of collapsing to the source that contains only a numeric value.

> **Update Note (2026-05-29):** Public Demo Chat fast RAG should avoid LLM work when retrieved chunks already support a deterministic repaired answer. In that case the route may return the repaired, source-linked answer without loading organization AI settings, generating, polishing, or recording LLM usage; source-quote-validated generation remains the next step only when deterministic repair cannot answer safely. Dense regulation repairs should be compact but still fact-preserving, such as medicine training answers that keep six-year duration, Dönem I-III / IV-V / VI structure, and avoid implying a separate summer internship unless the source says so.

> **Update Note (2026-05-29):** Grounded RAG generation must not bypass answer repair just because the model returned exact support quotes. Generated KB answers should be post-processed with the same intent/contradiction repair used by extractive fallback, then optionally polished with organization AI voice when no safe engagement is present or when repair changed the answer. Policy-duration responses must retain meaningful groups from the source, especially when a policy lists general brackets and exception brackets separately. Focused contact replies must not merge generic institution or footer phone numbers into program-specific contact answers unless the focused evidence itself supports that phone on the same contact line/block. Source links for contact answers should only include chunks that support the concrete e-mail or phone shown in the reply. Engagement questions must teach or offer a source-grounded adjacent detail, not ask the visitor’s department/program/status as a personal-profile prompt. Context-dependent follow-ups must narrow candidate chunks to the latest explicit user topic anchor before RAG context construction, so generic words such as internship, campus, or exam cannot pull stale or unrelated evidence from older conversation topics. Compound questions where multiple subjects share one predicate, such as `A ve B kampüsleri nerede?`, should expand into per-subject retrieval queries before answer generation, and answer repair should fill omitted campus/location subjects when retrieved evidence supports them.

> **Update Note (2026-05-28):** RAG answer quality should favor grounded naturalness over brittle deterministic prose. Public Demo Chat polling must start with bounded fast RAG recovery before the heavier shared inbound pipeline, but answer authoring inside that fast path should still prefer source-quote-validated LLM generation with organization voice and short demo-specific AI timeouts. Deterministic repair is only a fallback when grounded generation cannot safely answer a standalone question; context-dependent follow-ups may use deterministic repair only when the retrieved chunks also contain the prior explicit topic anchor from recent user history. For follow-ups such as "this program" or "that exam", demo fast RAG must read recent demo history, skip prior ambiguous follow-ups as anchors, search first with recent explicit user-question context, and run history-aware broad/planned retrieval instead of generic focused evidence fallback so generic terms do not pull unrelated sources. Source-quote-validated generated RAG answers can run through the same bounded polish layer used by extractive answers when the answer is safe but lacks a grounded engagement offer; the polish layer must preserve every critical value, reject raw URLs, and require exact context evidence for follow-up suggestions. Demo polling recovery must cap sync waits below serverless platform timeout even if env configuration is too high and return pending on timeout. Source URL rendering must keep appended canonical URLs on standalone lines after e-mail/contact answers, and secondary source links for numeric/duration answers should only be appended when that source also contains the concrete value.

> **Update Note (2026-05-28):** Knowledge Base RAG should use model reasoning and conversation context for answer wording without trusting unsupported output. Shared Knowledge Base retrieval passes recent conversation turns into the query planner, and grounded answer generation receives recent history only to resolve references while still requiring exact support quotes from retrieved context. Public Demo Chat applies that grounded answer authoring in the focused-evidence recovery path first, then falls back to the shared inbound pipeline only when fast RAG cannot safely answer. The generator must apply organization AI voice instructions, reject missing support or invented critical values, and allow one role-neutral engagement question only when its evidence quote exists in the same context. Deterministic repair remains a fallback for bounded recovery, not the default answer author.

> **Update Note (2026-05-28):** Contact-style RAG answers must be subject-aligned before deterministic repair can replace the model output. When the user asks for a named program, department, or administrative unit, repair should inspect all retrieved contact candidates and choose only an e-mail/phone whose local evidence window overlaps the requested subject or compact subject form; generic institution contact details remain valid for general institution questions but must not override specific unit/program evidence. Phone repair should prefer values labeled `Telefon/Tel` over nearby `Fax/Faks` values.

> **Update Note (2026-05-28):** Knowledge Base RAG must preserve the user's actual language and intent before applying answer repair. ASCII Turkish university/policy wording such as `programinda`, `butunleme`, `kac is gunu`, or `telefon numarasi` should resolve to Turkish so organization AI behavior settings are applied in the correct language; contact-number intent must win over document-code extraction unless the user explicitly asks for a document/form/regulation code or number. Grounded Turkish answers should not expose English retrieval boilerplate, referential final/bütünleme policy clauses that say `bu sınav` should be rewritten into a self-contained answer, and renderer autolinking should avoid turning internal document codes into fake source domains.

> **Update Note (2026-05-28):** Organization AI behavior settings must be observable in RAG answers, not just loaded implicitly. Grounded RAG polish should follow explicit organization tone/personality/style instructions more strongly than terse extractive wording while keeping all factual values and source constraints intact, and persisted extractive RAG bot messages should record `rag_polish.usedPolish`, `rag_polish.addedEngagement`, and `rag_polish.model` for live QA.

> **Update Note (2026-05-28):** RAG source validation must confirm that appended sources support the answer's actual wording, not merely that the URL is reachable. If evidence supports a broader content-access term such as course-content sharing but does not mention lecture notes, answer repair should keep the supported access platforms and use broader course-content wording instead of over-claiming a narrower artifact.

> **Update Note (2026-05-28):** RAG answers should feel less mechanical without weakening grounding. Extractive Knowledge Base answers may be polished by an LLM using the organization AI voice, and the polish layer must treat those organization AI settings as the active tone/behavior contract in the detected response language. Any engagement follow-up must be backed by an exact quote from retrieved KB context and must stay role-neutral. If the polish removes explicit factual values or the engagement evidence is not present in retrieved context, the original grounded answer is used. When a user asks multiple knowledge questions in one message, retrieval should search the full question and each supported clause, then answer only when the relevant clauses have evidence; the public demo polling path should run focused evidence on the independent clauses instead of the entire compound sentence, and timed-out polish calls must be aborted before fallback. Demo source links should emphasize source traceability with compact inline `Kaynağı aç / Open source` and numbered labels when multiple sources are appended. Source-link ranking should prefer direct evidence URLs over generic listing/index pages, address/campus answers must preserve all distinct campus entries supported by the retrieved source rather than over-compressing them into one address, named learning-platform claims such as UZEM/MEDU/ÖBS must be pruned when retrieved evidence supports only a subset, and those short platform names should count as concrete source-ranking evidence even though they are shorter than normal keyword thresholds.

> **Update Note (2026-05-27):** Public Demo Chat should keep source links traceable without showing long PDF URLs as the visible answer text. Assistant replies may render standalone raw source URL lines with a localized short label on the demo surface only, while persisted text and non-demo channel renderers continue to preserve raw URLs.

> **Update Note (2026-05-27):** RAG retrieval should stay general by turning strong document signals into reusable search behavior, not by special-casing customer screenshots. For standalone subject-cue questions, the original user text remains the first retrieval query even if conversation history or the router rewrite drifts; named campus/location and medical-school policy questions try required evidence groups before broad OR scans; abbreviation questions get stronger title-initialism scoring and can be repaired from retrieved document titles when the model says it has no information. Customer-facing answers should also remove generic engagement prefaces, role-assumptive clarification tails, and adjacent-policy drift such as final/bütünleme answers sliding into final-exemption conditions; source-link appending applies the same trailing-sentence cleanup. Grade-calculation repair must include the in-year board-grade `%96 + Hekimliğe Uyum / Kanıta Dayalı Tıp` formula when the question asks about `dönem içi kurul notu` and retrieved evidence contains it. The latest full live YİÜ challenge passed 33/33 at `tmp/crawl-output/live-yiu-challenge-pipeline-qa-2026-05-27T18-20-44-717Z.md`.

> **Update Note (2026-05-27):** Public Demo Chat should handle long-open browser tabs where the embedded signed access token expires after the page was rendered. The browser client now checks token expiry before sending, shows localized refresh copy, and reloads the dynamic page to mint a fresh token instead of making a guaranteed `401 Demo access denied` request.

> **Update Note (2026-05-27):** Knowledge Base ingestion must enrich evidence during chunk creation, not compensate later with customer-specific prompts. For future PDF uploads and crawler imports, section-aware chunking should run before token chunking, single-heading documents should still keep `Section:` metadata, markdown table rows and high-signal contact/address/course-code/value lines should be embedded as standalone evidence chunks, and chunk rows should carry evidence metadata so retrieval can find exact rows without overfitting to one demo question. Existing organizations can be rebuilt with the service-role reindex/import tooling, and the Yüksek İhtisas test organization was refreshed from the M1 crawl/PDF corpora into 6,153 ready documents and 49,863 chunks before passing the 33-question live challenge plus 12-question public demo canary.

> **Update Note (2026-05-27):** Public Demo Chat pages must not be cached with embedded signed API tokens. Because the browser client receives a short-lived token from the server-rendered page, demo slug pages should force a fresh server render with zero revalidation so stale HTML cannot cause `401 Demo access denied` on subsequent chat requests.

> **Update Note (2026-05-27):** RAG recovery and source-linking should validate against every retrieved candidate available to the inbound pipeline, not only the compact prompt context sent to the completion model. This lets timeout/extractive repair see supporting chunks that were trimmed for prompt size while preserving the same groundedness rules. Policy repairs should normalize PDF line breaks, separate similar intents such as final exemption versus bütünleme, prefer evidence rows whose durations and subjects match the question, and strip generic/role-assumptive helper tails before source-link canonicalization so answers stay direct, role-neutral, and source-backed.

> **Update Note (2026-05-27):** RAG answer recovery should handle referential evidence inside policy text, such as a clause that names `bütünleme sınavı` and a following clause that says eligible students enter `bu sınav`, without waiting for the full completion pipeline when the answer is already grounded. Longer-term RAG quality work should follow the modernization plan in `docs/plans/2026-05-27-rag-modernization-plan.md`: richer chunk metadata, hybrid vector/full-text retrieval, evidence validation, optional reranking for ambiguous cases, and tenant-agnostic evals should reduce reliance on deterministic customer-specific repairs.

> **Update Note (2026-05-27):** RAG duration repair should keep extracted answers customer-readable even when the supporting PDF chunk is a dense curriculum/course table or a multi-row policy bracket list. For staj/duration questions, a long row containing many course names, credits, and percentages may be compacted to the matching `Yaz Stajı` label plus the explicit duration value before replying; for broad policy-duration questions without a user-specified threshold, all matching duration brackets should be preserved together instead of returning only the first row. Both behaviors still require subject and duration evidence from the retrieved chunk and remain general cleanup rules, not YİÜ-only phrase maps.

> **Update Note (2026-05-26):** Focused Knowledge Base retrieval should return high-confidence current-campus/location evidence before spending the polling budget on broad address scans. Academic-unit acronyms and expanded unit names are matched through subject-coverage and initialism rules against current listing or program evidence, and extractive duration repair uses the same acronym-to-expanded-name matching so no-information seeds can be repaired from retrieved evidence without waiting for the full completion pipeline. Each evidence query has a bounded timeout so one slow lexical scan cannot stall public Demo Chat polling in Netlify.

> **Update Note (2026-05-26):** Public Demo Chat polling must not depend on long-running completion-model recovery when RAG evidence can already produce a deterministic answer. Polling `GET` may first run a bounded extractive Knowledge Base recovery path that tries focused evidence retrieval before broad corpus search, repairs a no-information seed from retrieved chunks, appends canonical source links, and persists the bot row with the originating demo message id plus duplicate-reply protection. If this extractive path cannot produce a grounded answer, the normal shared inbound AI pipeline remains the fallback; no customer/demo-specific phrase rules should be introduced.

> **Update Note (2026-05-26):** RAG answer polish should be general, role-neutral, and source-grounded: when a Knowledge Base answer already contains the requested fact, customer-facing output may include one LLM-chosen engagement question only when it is specific to the user's topic and points to an adjacent grounded detail such as requirements, deadlines, exceptions, or the next rule. It must not assume the user is a student, applicant, personnel member, or admin; if clarification is needed, ask for the topic/program/unit/document rather than the user's identity. Generic assistant closing tails, role-assumptive department/status prompts, source-link preface tails, and vague "more info / contact the department" deferrals are stripped because they sound evasive after a correct answer. Policy-duration repair should extract matching list/bullet evidence, protect already-correct duration answers from unrelated eligibility repair, and standalone user questions should be searched before drifted router rewrites. Vector-search timeout responses from Supabase remain recoverable retrieval events and should be logged as warnings while lexical/focused evidence continues. Retrieval-only query planning is also bounded by a short fail-fast deadline; the original query is searched before planner latency is spent, planned semantic variants are skipped when the original query already returns enough high-scoring grounded evidence, focused evidence probes are scoped by query intent, public Demo Chat POSTs return a pending id immediately, and clear address/location answers may be produced by grounded extractive repair before the completion model. The latest stricter YİÜ live challenge, including generic/role-assumptive closing checks, passed 33/33 at `tmp/crawl-output/live-yiu-challenge-pipeline-qa-2026-05-26T18-28-49-968Z.md`.

> **Update Note (2026-05-26):** Knowledge Base RAG should answer from concrete evidence before redirecting users to links or departments. Retrieval and repair now cover informal eligibility phrasing, exact unit contact-table values, and factual policy answers with unnecessary "contact the unit / more info" tails, so source URLs remain supporting context rather than a substitute for the answer unless the user explicitly asks for a link.

> **Update Note (2026-05-26):** Public demo RAG readiness must prove both answer correctness and source traceability before customers are invited to retest. Demo QA now includes a public Demo Chat canary that records the persisted conversation id, source document ids/titles/URLs, a corpus-health scan for missing chunks/source URLs/oversized chunks/broken URL text, and a 33-question live challenge over screenshot questions plus natural variants. General RAG behavior should prefer remedy/outcome evidence for missed-exam health-report queries, repair self-contradictory final/bütünleme eligibility answers, recover elective-count answers from board-determination evidence, scope internship/staj evidence to the academic unit or program named in the question, rank appended source URLs by concrete answer evidence such as e-mail/phone values, fail soft if usage metering writes fail after a response has enough grounding to continue, preserve late high-signal subject terms in natural conversational questions, normalize URL line-break artifacts before indexing or reusing source text, keep lexical fallback searches indexed for large crawled/PDF corpora, and use a feature-flagged LLM query planner only to create retrieval variants/must-have terms before grounded chunk search. The latest YİÜ live challenge report passed 33/33 at `tmp/crawl-output/live-yiu-challenge-pipeline-qa-2026-05-26T11-06-11-285Z.md`.

> **Update Note (2026-05-25):** RAG model selection should be validated with the same live customer-style challenge before changing defaults. The Yüksek İhtisas 33-question run now records per-conversation token usage and compares `gpt-5-mini`, `gpt-4o`, and `gpt-4o-mini`; GPT-5-family RAG completion is supported through `OPENAI_RAG_MODEL` with reasoning-compatible parameters, but the default remains `gpt-4o-mini` because retrieval quality, grounding repair, and source validation produced the quality gain without the broad `gpt-4o` cost increase.

> **Update Note (2026-05-25):** Public Demo Chat composer text fields must keep an iOS-safe 16px baseline so focusing the reusable institution demo textarea does not trigger mobile browser auto-zoom. This applies beyond the Yüksek İhtisas demo to any future public demo slug.

> **Update Note (2026-05-25):** Knowledge Base RAG should improve generally through evidence-first retrieval, not customer-specific question patches. Runtime retrieval now rescues address, lecture-note, elective, exam, grade-calculation, contact, double-major, and policy evidence with subject-aware reranking; answer repair removes contradictory no-information openings, preserves required evidence terms such as `final/bütünleme`, prefers official source terminology, and recovers from RAG completion timeouts by synthesizing from selected chunks before generic fallback.

> **Update Note (2026-05-22):** Payment-provider integration code should avoid vulnerable SDK dependency chains when the required Iyzico endpoints can be called with first-party signed REST requests. The billing client now signs Iyzico v1/v2 requests with Node crypto and `fetch`, and the repository dependency audit currently reports zero vulnerabilities.

> **Update Note (2026-05-22):** WhatsApp webhook POST responses should be bounded to provider acknowledgement work. Signature verification, channel lookup, and channel-verification state remain synchronous; AI pipeline execution and media downloads run in a deferred `after()` task so Meta does not wait for RAG/LLM response generation.

> **Update Note (2026-05-22):** Billing entitlement checks may run multiple times before token-consuming AI stages, but manual-admin subscription renewal should run once per inbound request and then be skipped by downstream deferred lead-extraction/fallback checks that already sit behind a fresh gate.

> **Update Note (2026-05-22):** Server-side media proxying for Instagram skill images must validate the Supabase bucket path, source MIME type, and source size before buffering or converting an image. The current public proxy accepts only raster images from the `skill-images` bucket and caps source payloads at 5 MB.

> **Update Note (2026-05-22):** OAuth, callback, and signout redirect origins must be derived from configured app/site origins or the concrete request origin, not arbitrary forwarded-host headers. Forwarded origins are trusted only when they match configured or request origins.

> **Update Note (2026-05-22):** Hosted Iyzico card-update callbacks must be bound to the active recovery attempt. The app stores the provider card-update token when starting the hosted flow, requires the same token and matching conversation/subscription record id on the callback, and clears the active token after recording a valid callback payload.

> **Update Note (2026-05-22):** Admin dashboard usage metrics should be aggregated by the database for large tenants and all-time views. AI usage token/credit totals may come from an authenticated organization-scoped aggregate RPC, with row-paging retained only as a backward-compatible migration fallback.

> **Update Note (2026-05-22):** Large Knowledge Base RAG fallback should spend latency on the slowest useful fallback query, not the sum of every fallback query. After bounded vector search, independent lexical fallbacks for keyword, document code, abbreviation, focused keywords, exact title, title, and source path may run concurrently and then merge through the existing scoring/reranking layer.

> **Update Note (2026-05-22):** Public Demo Chat API calls must be authorized before service-role database or AI pipeline work can run. A demo channel needs a configured `shared_secret_hash`; the server-rendered demo page uses it to mint a short-lived signed access token, the browser sends that token on `POST` and polling `GET`, and the API rejects missing/invalid tokens plus rate-limits repeated AI/recovery work per session/IP.

> **Update Note (2026-05-21):** Public Demo Chat must present one operator-visible bot answer per tester message even when pending polling or recovery re-runs the shared AI pipeline. Persisted demo bot replies should be idempotent by original demo message id plus reply kind (`text` or `image`), and Inbox should hide historical duplicate demo bot rows without deleting customer/audit data.
...
> **Update Note (2026-05-21):** Specific policy duration/limit questions should not always pay the full embedding + vector-search cost. Runtime may first run token-bound policy evidence retrieval, and when that evidence contains the question's distinctive subject plus an explicit duration value, it should return that grounded context immediately.
...
> **Update Note (2026-05-21):** Public Demo Chat pending replies must be resilient even when serverless deferred work stalls after a fast `202` response. Polling should be able to recover the original question through the existing stored inbound row or the browser-provided message text, run the shared inbound AI pipeline again against the same demo message id without creating duplicate contact messages, and return the completed answer to the browser. The demo layout should also behave like a chat app shell: header and composer remain visible, only the middle transcript scrolls.

> **Update Note (2026-05-21):** University-pilot RAG must treat PDF policy duration/limit questions as a general evidence pattern. For questions asking how long, how many days/months/years, maximum duration, or application windows, retrieval should favor chunks where the question's subject terms and an explicit duration value appear together in governing policy text, demote broad but weakly covered semantic matches, and repair model no-information wording from that retrieved sentence.

> **Update Note (2026-05-21):** Live QA cleanup must include every deterministic Codex-generated pilot test marker, including the newer `Codex YIU Mevzuat QA` contact-name prefix, `codex-live-yiu-mevzuat-qa-` phone prefix, `codex_yiu_mevzuat_qa` tag, and ad-hoc demo-chat sessions whose generated session id starts with `codex-`. Cleanup must still avoid normal public demo sessions such as generic `Demo ziyaretçi` conversations unless they carry explicit Codex QA/debug metadata.

> **Update Note (2026-05-21):** Public Demo Chat must not block the browser request for the entire AI/RAG answer when pilot university PDFs make retrieval slow. Demo chat submissions should return a fast pending state before platform timeout, continue the shared inbound pipeline in the background, persist the bot reply against the originating demo message id, and let the browser poll for the completed answer while keeping the normal thinking state visible.

> **Update Note (2026-05-21):** University-pilot RAG must recover facts that exist only in uploaded PDFs or crawled contact tables even when vector similarity alone sits below the organization threshold. Runtime retrieval should combine token-bound lexical matching, focused multi-keyword fallback, and narrow policy-intent reranking so YİÜ-style staff leave, acronym/contact-table, and mazeret-exam eligibility questions answer from the governing corpus instead of drifting to generic FAQ or schedule notices.

> **Update Note (2026-05-21):** Live QA cleanup tooling must cover both newer YİÜ demo QA markers and earlier live RAG QA conversation names. Safe cleanup should match only explicit Codex QA tags or deterministic `Codex YIU Demo QA` / `Codex Live QA` contact prefixes and their generated phone prefixes before deleting demo-test conversations from pilot Inbox views.

> **Update Note (2026-05-21):** Public Demo Chat composers should use a mobile-friendly autosizing textarea instead of a fixed single-line input. Long questions should expand the composer up to a controlled height, support Enter-to-send plus Shift+Enter line breaks, keep internal scrolling available without showing a native scrollbar, keep the disclosure near the composer but concise, start loading feedback with a neutral thinking message before source-specific copy, and use only one bottom-right send button without attachment or instant-action dropdown affordances.

> **Update Note (2026-05-20):** Public demo-chat surfaces for university stakeholders should avoid persona-specific labels when the same demo is used by candidates, current students, and administrative reviewers. The demo header should use the institution logo when available, explain demo/account context in a static notice instead of an assistant message, promote `www.askqualy.com`, render links and WhatsApp-style bold text cleanly, support a light/dark mode toggle, stay mobile-friendly with dark-mode contrast checks, and present assistant answers as unframed plain text with a quick reveal animation instead of a second chat bubble.
...
> **Update Note (2026-05-19):** Customer-facing RAG answers from crawl-imported Knowledge Base corpora must keep source links and chat formatting deterministic for pilot demos. Runtime should carry canonical source URLs with selected chunks, append raw URLs for link/page/document requests without relying only on the model, keep exact-field answers grounded to matching title/source context, prefer root contact pages for rectorate-wide contact questions, and preserve hyphenated item labels such as `Tanıtım Videosu - 1` when converting inline lists for messenger channels.

> **Update Note (2026-05-19):** Website-crawl RAG quality must be regression-tested with mixed PDF and non-PDF questions before pilot demos. For page/link-style questions, retrieval should use bounded source-URL slug fallback in addition to vector, keyword, and title signals so canonical stable pages outrank old announcements or adjacent program pages while PDF/regulation queries avoid unnecessary source-path searches.

> **Update Note (2026-05-19):** Knowledge Base RAG retrieval must blend vector search with bounded keyword and title-document fallback for large crawl/PDF corpora. Exact regulation, directive, and PDF metadata questions such as Senato decision, scope, AKTS, and document number should retrieve the matching early document chunks even when broad semantic results point to nearby pages.

> **Update Note (2026-05-19):** Sidebar remaining credits and `Settings > Usage` totals must stay financially consistent after correcting text-embedding pricing. Historical current-period embedding debits charged with the old chat-token formula should be reconciled by returning overcharged credits to the billing account and writing a localized credit-history adjustment, not merely by changing the display math.

> **Update Note (2026-05-18):** Billing credit consumption must distinguish chat-completion token usage from text-embedding usage. Knowledge Base indexing with `text-embedding-3-small` uses embedding-token pricing, admin token totals expose input/output/text-embedding detail for cost review, customer Usage summaries/history should recalculate linked debits from token rows when available, user-facing ledger and usage-breakdown labels should call KB embedding imports `Bilgi Bankası indeksleme` rather than generic AI usage, and Knowledge storage counts must not cap at Supabase's default 1000-row page.

> **Update Note (2026-05-15):** Admin manual billing must show remaining package/trial credits as the primary operator value, with used/limit as supporting context, so `0 / 2000 used` cannot be mistaken for `0 credits left`. Manual premium assignment must reject zero-credit package grants before calling Supabase, reassert the package grant and renewal metadata in the `admin_assign_premium` RPC, and all admin billing forms must enter a disabled `Processing... / İşleniyor...` pending state after submit.

> **Update Note (2026-05-15):** Customer-facing chat-channel replies must defensively normalize malformed link output before send/persist. Markdown-style links with spaced or newline-wrapped URLs and raw URLs with accidental domain/path whitespace must be converted into visible raw URLs without whitespace, risky URL punctuation should be kept out of the clickable token, and RAG prompts must instruct models to put copied source URLs on their own final line without extra words after the URL. When RAG output already contains a malformed model-emitted URL and selected chunks carry canonical source URLs, runtime should remove the broken model URL and append the canonical source URL as a separate final link line.

> **Update Note (2026-05-15):** Default workspace Skills now include an editable active `Greeting and First Message / Karşılama ve İlk Mesaj` skill in addition to the four guardrail skills. It covers first-contact greetings and Telegram `/start` with localized trigger examples, does not require human handover, and uses exact trigger matching before embedding search so slash commands and simple greetings can resolve deterministically without hardcoded channel replies.

> **Update Note (2026-05-15):** Customer-facing bot replies must be formatted for the destination messenger before send/persist. WhatsApp converts double-star Markdown emphasis to WhatsApp-friendly single-star emphasis, Instagram strips unsupported Markdown markers while preserving the standardized quoted AI-disclaimer footer, Telegram sends plain text unless a parse mode is explicitly configured, inline ` - ` RAG list artifacts are converted to line-broken bullets, and raw URLs are preserved for all channels.

> **Update Note (2026-05-15):** Customer-facing bot disclaimer text is app-owned, not model-owned. Before appending the configured footer, runtime must strip any trailing AI-disclaimer artifact that an LLM, Skill, KB, or fallback response accidentally produced, including inline `> ...` variants, so outbound WhatsApp/Telegram/Instagram messages contain the disclaimer at most once. If the org disables the app disclaimer, model-supplied disclaimer artifacts must still be removed.

> **Update Note (2026-05-15):** When onboarding reaches `5/5`, the required bot-mode choice modal must make the safest user decision obvious without defaulting for them: show `Active / Aktif` first, `Shadow / Dinleyici` second, and `Keep it off / Şimdilik kapalı kalsın` last, with the same green, amber, and rose status tones used by the sidebar bot-mode switch.

> **Update Note (2026-05-15):** Dashboard chrome onboarding progress must reflect the freshest checklist state from `/onboarding`, not only the parent layout's initial shell snapshot. When the onboarding page shows four completed steps, desktop sidebar and mobile `Other` navigation must show the same `4/5` progress without requiring a full page reload.

> **Update Note (2026-05-15):** Large website-crawl corpora can be imported directly into an organization's Knowledge Base. The import flow must use service-role Supabase writes, create a crawl-specific folder, protect against duplicate imports by default, support explicit replacement, persist source URL metadata in document/chunk content, generate embeddings for chunks, and leave all imported documents in `ready` status only after chunks are stored.

> **Update Note (2026-05-15):** Supabase email-confirmation links that land on `/login?code=...` must be exchanged through a server-side auth callback before rendering the login form. Successful exchanges redirect through the normal post-auth workspace/onboarding route, failed exchanges remove the stale code and show localized login guidance. After a successful password login or callback exchange, post-auth workspace/onboarding resolution must fail soft to `/inbox` instead of turning a valid session into a 500. Supabase client methods such as `rpc` must be invoked on the client object, not detached into standalone function references, because the Supabase client relies on internal method context.

> **Update Note (2026-05-14):** Customer-facing RAG replies on WhatsApp/Instagram must include source links as visible raw URLs instead of Markdown-style `[label](url)` hyperlinks. `NO_ANSWER` is an internal refusal sentinel for routing to fallback/handoff behavior, not a literal customer-facing answer; unknown answers should be phrased as “I do not have a clear answer” / “Bu konuda elimde net bilgi yok.”

> **Update Note (2026-05-14):** Website-crawl RAG quality must be validated with realistic customer/navigation questions, not only import success. Retrieval should rank the exact source page first for generic academic-calendar, dormitory, academic-staff, coordinator-contact, department, fee, scholarship, Erasmus, and student-services questions where the crawled corpus contains that page.

> **Update Note (2026-05-14):** RAG answers for large crawled websites must be grounded by more than embedding similarity alone. Knowledge search should hybrid-rank vector, normalized keyword, title, and source-URL signals; stored chunks should preserve document-title metadata; and the answer prompt should use matching source URLs for navigation/link requests while continuing to return no-answer for unsupported facts.

> **Update Note (2026-05-14):** University website RAG must favor stable informational pages over old announcements for generic admission/student questions, understand common Turkish suffix variants in page slugs, rerank a wider keyword candidate pool, and never expose the internal `NO_ANSWER` token even when the model wraps it in explanatory text.

> **Update Note (2026-05-12):** University admissions outreach is treated as a GTM/package motion on top of the existing multi-channel Qualy AI inbox, not a core MVP pivot into a generic university web assistant. For rector/IT-facing pilots, the product promise should stay focused on WhatsApp/Instagram candidate-message operations, approved-source answers, AI request summaries, priority/handoff workflow, and pilot reporting while university IT retains content and approval control.

> **Update Note (2026-05-06):** Sales-led manual billing must behave like a real recurring package after admin activation. Admin-assigned `manual_admin` premium subscriptions auto-renew by default on each monthly boundary, reset monthly package usage, add a package-grant ledger row, and remain premium until an admin cancels or changes the package.

> **Update Note (2026-05-05):** Billing launch mode can be sales-led instead of fully self-service. Plan pricing remains visible in `Settings > Plans`, but plan purchases, package changes, and top-up requests may create an internal billing request, show the operator a sent-to-team confirmation, and optionally email the admin through Resend. Admins can manually activate catalog packages and adjust credits while existing Iyzico checkout code remains preserved for later self-service rollout.

> **Update Note (2026-05-01):** Telegram webhook handling must fail predictably on malformed or incomplete payloads before service-role database access, and production Telegram webhook authentication must use the provider secret header instead of URL query secrets. Simulator server actions must not write customer message content, organization ids, or raw match payloads to production logs.

> **Update Note (2026-04-30):** App entry should prioritize fast workspace handoff over advisory onboarding routing. The root locale route must avoid DB-backed onboarding reads before redirecting authenticated operators, while the dashboard root loading boundary should show a localized, branded Qualy opening state with logo motion and a slim progress bar whenever server-render work is still unavoidable.

> **Update Note (2026-04-29):** Runtime safety must fail closed on billing, webhook authentication, and AI grounding failures. Billing entitlement lookup errors or unexpectedly missing billing rows must lock usage in production; Iyzico success RPCs must not be executable by normal authenticated users; Meta webhook POST handling must verify signatures before any channel verification or no-event success path; AI skill matching technical errors must route to human attention instead of KB/fallback, and RAG/fallback paths must not send `NO_ANSWER` or ungrounded LLM-generated business claims.

> **Update Note (2026-04-29):** Frontend operator surfaces must keep keyboard and screen-reader access aligned with visual interactions. Shared modals need dialog semantics, Escape close, focus trapping, and focus restore; clickable table/conversation rows need keyboard activation and accessible names, Leads search/sort/table controls need explicit labels and scoped headers, coming-soon Knowledge sources must be disabled rather than inert clickable items, WhatsApp connection setup must localize SDK/timeout failures, and production builds must not emit debug `console.log` calls from frontend Inbox or Telegram webhook logging paths.
...
> **Update Note (2026-04-24):** Runtime bot-mode enforcement must fail closed for customer-facing inbound channels. If live webhook processing cannot load organization AI settings or the settings row is unexpectedly missing, WhatsApp/Instagram shared inbound processing and Telegram must resolve the effective bot mode to `Kapalı` instead of falling back to default `Aktif`; this preserves the `Dinleyici` no-reply guarantee under transient settings-read failures.

> **Update Note (2026-04-15):** Dashboard navigation prefetch must not duplicate the same page chunk while a route is loading. Manual app-shell prefetch should skip the current route, click transitions should not also prefetch their destination, and touch-start prefetch should be avoided because it races with the actual tap navigation on mobile.

> **Update Note (2026-04-14):** Dashboard entry should avoid duplicate client reads after the server payload is available. Inbox current-user and profile hydration must dedupe auth, assignee, and message-sender lookups through shared client caches with in-flight request reuse, while Knowledge should use the server-loaded pending-suggestion count on initial render and refresh it only from realtime or explicit local suggestion events.

> **Update Note (2026-04-14):** Dashboard client interactions should adopt the Lumiso-style cache discipline without migrating away from Next App Router. Hot client-side data paths should use shared TTL caches, in-flight request dedupe, stale-while-revalidate reuse, and bounded eviction. Calendar windows and Leads pagination/search/sort now use this shared model, while route auth, billing enforcement, webhooks, and server actions remain server-owned.

> **Update Note (2026-04-14):** Dashboard performance is now a product requirement, not only an implementation cleanup. Calendar, Settings, and workspace navigation must be measured with route/action timing first, then optimized by slimming dashboard shell critical-path reads, deduping org/billing/onboarding data, narrowing settings i18n work, and caching calendar data windows. Target local `next start` first-useful-content budgets are under two seconds for small workspaces and sub-500ms for cached calendar next/back interactions after adjacent-window prefetch.

> **Update Note (2026-04-14):** Calendar capacity-aware booking now uses the simple organization-level model in implementation: each workspace has one `max_concurrent_bookings` setting, new/missing settings default it to `1`, and the booking engine allows overlapping internal appointments only while peak simultaneous pending/confirmed occupied ranges stay below that limit. Capacity is evaluated as point-in-time occupancy over buffered ranges, Google busy overlay remains a hard block, nearest AI suggestions return two slot-grid-aligned options across the next 30 local days when available, and staff/resource-group scheduling remains out of scope for this iteration.

> **Update Note (2026-04-12):** Launch and social visual generation now has a durable product-marketing context and prompt guardrail contract. Qualy social assets should position the product as `Qualy AI` managing an AI-powered unified inbox, keep the Qualy logo/wordmark visible, represent the AI actor as `Qualy AI` instead of a generic human-like assistant, and show only the approved channel set (`WhatsApp`, `Instagram`, `Telegram`, `Messenger`) when channel logos are part of the visual.

> **Update Note (2026-04-10):** Hosted Iyzico checkout must fail soft when the embedded provider script never renders. `Settings > Plans` subscription, top-up, and fixed-difference upgrade checkout routes should show a visible loading state first and then expose a fallback CTA to the stored secure `checkout_page_url`, so the operator can continue payment instead of being stranded on a blank in-app checkout page while the order remains `pending`. The fixed-difference upgrade confirmation copy must also match the real provider flow: after confirmation the user continues to the secure Iyzico payment screen, and card details may be requested again there.

> **Update Note (2026-04-08):** `Settings > Plans > Geçmiş / History` upgrade amounts must prefer Iyzico payment-detail `paidPrice` when a same-cycle upgrade order includes a successful `paymentAttempts[].paymentId`. Iyzico subscription-operation `orders[].price` can differ from the actually collected transaction amount shown in the provider transaction list, so Qualy should backfill upgrade ledger metadata from `/payment/detail` first in both direct saved-card upgrade completion and webhook settlement, keep the provider order/payment references for auditability, and continue avoiding local plan-delta estimates as confirmed charges.

> **Update Note (2026-04-08):** `Settings > Plans` checkout, direct plan-change, renewal, and payment-recovery feedback must survive transient query cleanup. After an Iyzico success return or in-app plan change, the newly refreshed subscriber Plans state should remain on screen first, then the green success feedback should stay visible until the operator dismisses it; removing callback query params must not trigger a second App Router navigation that immediately unmounts the banner.

> **Update Note (2026-04-07):** Billing history must be purchase-event based, not “latest ledger row” based. `Settings > Plans > Geçmiş / History` should load `package_grant` and `purchase_credit` rows directly so normal AI usage debits cannot make successful purchases disappear. Iyzico direct plan changes must use the documented subscription upgrade REST endpoint with merchant `conversationId`, `upgradePeriod=NOW`, `useTrial=false`, and `resetRecurrenceCount=false` actually present in the signed request body; the UI must not present locally calculated plan deltas such as `₺300` as confirmed charges unless a provider order reference/amount is reconciled from Iyzico.

> **Update Note (2026-04-07):** Structured AI assistant instruction starter text must follow the active UI locale even if an older row already saved the English starter defaults into the new structured columns. Exact known EN/TR starter sets are treated as untouched defaults and re-resolved for the current locale, while genuinely custom operator-written instructions are preserved as-is.

> **Update Note (2026-04-07):** Skill matching must be robust at the general routing layer rather than through skill-specific exceptions. Runtime now keeps embedding similarity as the primary signal but gates ambiguous skill matches with sector-independent semantic confidence, candidate-margin, and fuzzy non-generic trigger/title anchoring before sending a deterministic skill reply; rejected matches continue through KB/RAG/fallback.

> **Update Note (2026-04-07):** New workspaces should start with calendar booking disabled. The runtime fallback and Supabase default for `booking_settings.booking_enabled` now resolve to `false` until the operator explicitly enables appointments in `Settings > Calendar`, and the operational `/calendar` header must show a visible `Randevu aktif / Randevu kapalı` status chip while disabling the new-booking CTA when booking is closed.

> **Update Note (2026-04-07):** Instagram bot image replies must dedupe webhook echoes even when Meta does not return the same provider id shape as expected. Outbound Instagram send callbacks should normalize provider ids from known response variants, and Inbox should hide legacy image echo rows near bot skill-image placeholders so the operator does not see a second `askqualy` image bubble after the `Yapay Zeka Asistanı` image bubble.

> **Update Note (2026-04-07):** Existing Skill image-only edits must not create Organization Detail AI suggestions. New Skill creation can still feed offering-profile/service-list/required-field generation, and existing Skill text/title/trigger changes may still refresh those AI signals, but adding/replacing only the optional skill image should be treated as media metadata maintenance, not a service-scope change.

> **Update Note (2026-04-07):** Instagram skill-image delivery and Inbox echo rendering must now behave as one coherent bot-send flow. AI-triggered Instagram replies should persist the provider `instagram_message_id` for both text and image turns so webhook echoes can dedupe instead of creating a second `user` row, new skill uploads should be stored as JPEG, older WebP skill images should still be converted through a public JPEG proxy before calling Meta send-image APIs, and Inbox should hide duplicated `instagram_is_echo` records both when a bot message with the same provider id already exists and when legacy rows repeat the same AI-disclaimer content inside the same short reply window.

> **Update Note (2026-04-07):** `Knowledge > Yeni İçerik > Serbest Metin` now includes a persistent `Qualy AI ile doldur` helper banner above the title field. The helper uses the filled sidebar AI icon inside a softer colorful card, keeps the CTA stacked under the copy instead of pushing it to the far edge, and opens a reusable mobile-friendly bottom-sheet modal for short structured business/process/assistant notes. When draft generation starts, the modal switches into an explicit loading state and closes only after the new content is ready to appear in the editor. Each re-run replaces the content body, fills the title only when blank, uses `Yapay zeka asistanının dikkat etmesi gerekenler` wording for the assistant-guidance field, and rolls usage into the existing `İçerik işleme / Content processing` billing breakdown instead of a new visible bucket.

> **Update Note (2026-04-07):** Knowledge Base driven offering-profile suggestions must stay reliable after AI-assisted saves. Automatic/manual suggestion generation now needs the actual ready Knowledge Base content and recent Skill guidance text instead of title-only summaries, offering-profile repair gets one stricter second pass when the first AI draft still collapses into a single paragraph, and suggestion inserts must fail loudly instead of being dropped silently so pending-suggestion banners remain trustworthy in both `Knowledge` and `Settings > Organization`.

> **Update Note (2026-04-07):** `Knowledge` must keep the service-profile helper visible during extraction, not only after pending suggestions exist. If any organization Knowledge Base document is still `processing`, the page should show a visible “Hizmet profili önerileri hazırlanıyor / Service profile suggestions are being prepared” banner with wait-state copy; once processing ends, that helper may switch to the existing ready-state review banner.

> **Update Note (2026-04-07):** Pending Meta channel surfaces must now explain the operator action explicitly: after connecting WhatsApp or Instagram, send a test message to the connected account and Qualy will automatically verify the connection when the first inbound message arrives. Returning focus to the app should refresh pending channel surfaces, onboarding setup pages should show the connected account explicitly, keep the lead paragraph concise to avoid duplicated instructions, and reuse the same pending helper copy inside a full-width `Aksiyon gerekli / Action required` helper. Post-verification onboarding must still require a neutral explicit bot-mode choice instead of auto-switching to `Dinleyici`.

> **Update Note (2026-04-05):** The Skills action-button editor is temporarily hidden from the operator UI until editor-side regression coverage is restored. Existing `skill_actions` records and WhatsApp runtime behavior remain supported; only the editing controls are not exposed for now.

> **Update Note (2026-04-06):** Skills now support one optional uploaded image per skill. Operators upload the asset inside Qualy, the client enforces a `5 MB` original-file limit, converts the image to flattened `JPEG` before storage, shows a visible loading/disabled state while the asset is being processed/uploaded, and live skill replies now send text first and image second across WhatsApp, Instagram, Telegram, and Simulator. If image delivery fails, the text reply still stands, operators can see the failure, and `requires_human_handover` still escalates after the image attempt.

> **Update Note (2026-04-06):** Skill-image delivery is hardened around tenant safety and operator visibility. Storage upload/delete actions now derive the active organization server-side before touching service-role storage, upload object keys are unique even for same-second edits, and failed skill-image sends fall back to visible failure text in Inbox instead of rendering as a normal successful image bubble.

> **Update Note (2026-04-06):** `Settings > Qualy AI > Behavior and Logic` now uses structured `Yapay zeka asistan talimatları` instead of a raw prompt textarea. Operators guide customer-facing AI replies through four optional fields (`Asistanın görevi`, `Eksik bilgi toplama kuralı`, `Asla yapma`, `Diğer talimatlar`), old custom prompts are carried into `Diğer talimatlar`, locale-aware starter copy stays sector-agnostic, and a mobile-friendly `Nasıl çalışır?` modal explains what the instructions do and what they do not override with an inner scroll area, bullet-based examples, explicit dialog semantics, Turkish product copy that avoids English pipeline jargon, and a low-emphasis text-link help action placed directly under the section description. Internal extraction, scoring, routing, and summary prompts remain unchanged.

> **Update Note (2026-04-04):** Default guardrail skills must be created when a workspace is actually provisioned, not only when someone first opens `Skills`. New organizations now seed localized default skills from signup locale when available, fall back to billing-region language when it is not, and backfill older organizations that predate create-time seeding.

> **Update Note (2026-04-04):** Dashboard chunk recovery must preserve the operator’s intended destination. If a sidebar/mobile/settings navigation hits a real Next chunk failure mid-transition, Qualy should retry the pending target route instead of cache-busting the currently visible page and making navigation look like it silently reloaded the same screen.

> **Update Note (2026-04-04):** `Settings > Plans > Tek Seferlik Ek Kredi` kartı, fiyat/KDV ve konuşma-aralığı notunu kart gövdesinde tekrar etmemeli. Bu açıklama yalnızca satın alma modalı içinde kalmalı, böylece outer card kısa ve karar odaklı görünür.

> **Update Note (2026-04-03):** `/calendar` and `/simulator` should each show a one-time, mobile-friendly intro modal on first entry for that browser/user/workspace surface. The modal should reuse the onboarding visual language with compact icon rows, explain what to do in that area without a separate hero-style description icon, offer one relevant next-step CTA, use direct `sen` tone in Turkish copy, and never reappear after dismissal unless the intro content version is intentionally bumped.

> **Update Note (2026-04-03):** In `Settings > Plans`, the active monthly-package card should describe the next cycle as `Yenileme / Renews at` instead of `Sıfırlanma / Resets at`, so the label matches subscription renewal language.

> **Update Note (2026-04-03):** One-time extra-credit checkout in `Settings > Plans` must use the same in-app hosted Iyzico embed pattern as subscription checkout. Operators should stay inside the app on a local checkout route rather than being sent to a separate full-page provider URL, reducing auth-context loss after purchase and keeping billing UX consistent.

> **Update Note (2026-04-03):** In-app `Settings > Plans` price surfaces must explicitly say that displayed subscription and one-time extra-credit amounts include VAT/KDV, then immediately show the conversation-range variability note used on the landing pricing section. This applies to unsubscribed package cards, active-subscriber package management UI, and extra-credit purchase surfaces.

> **Update Note (2026-04-03):** `Settings > Plans > Geçmiş / History` purchase rows should now render localized time together with the purchase date so same-day package starts/upgrades are easier to audit without ambiguity.

> **Update Note (2026-04-03):** `Settings > Plans > Geçmiş / History` must not invent same-day upgrade charges from local package price differences. When exact provider charge metadata and an order reference are absent, the history layer should show the amount as unavailable instead of deriving a `plan difference`; repeated pre-effective downgrades should still replace the previously scheduled target plan cleanly instead of stacking stale pending metadata.
...
> **Update Note (2026-04-03):** Legal credit wording must distinguish monthly package credits from one-time extra credits. Monthly package credits remain non-transferable, non-cash-value, active-subscription-only, and non-rollover; one-time extra credits also remain non-transferable, non-cash-value, and active-subscription-only, but they persist across billing periods until consumed while the paid subscription stays active.
...
>
> **Update Note (2026-04-02):** `Settings > Plans` should not duplicate the active-package story across separate sections. If the workspace already has a managed monthly package, the old top `Plan durumu` membership card should disappear, the current package summary and plan-management actions belong inside `Mevcut plan`, and `Yükselt veya değiştir / Ödeme yöntemini güncelle` should render as side-by-side text actions inside that card. The standalone `Aylık paketler` section stays only for workspaces without an active monthly package. `Fatura bilgileri` also stays at the bottom of the page and should not repeat its title/description inside the inner card wrapper.
>
...
>
> **Update Note (2026-04-02):** `Settings > Plans` should keep one dominant subscription CTA. `Aboneliği iptal et` must not compete as a sibling button next to the main upgrade/switch action; it should live as low-emphasis helper text inside the manage-plan modal, and payment-recovery actions such as `Ödeme yöntemini güncelle` should use lighter text-style affordances instead of stacked secondary buttons.
>
> **Update Note (2026-04-02):** Downgrade plan cards inside the manage-plan modal should not overload the primary CTA with timing copy. The button should keep the same `Select this plan and continue` wording as other selectable plans, while the `period end` warning remains below it as a warmer amber helper line.
>
> **Update Note (2026-04-02):** The downgrade timing helper under plan-selection CTAs should stay short and neutral. Use `Plan change applies at period end` style wording rather than a heavier `lower-tier change` warning sentence.
>
> **Update Note (2026-04-02):** Existing-subscriber plan changes should use a lighter confirmation model than first purchase checkout. `Settings > Plans` upgrades/downgrades should show one concise plan summary, one explicit apply-confirmation, legal-document links as references, and upgrade wording that makes Iyzico responsible for the actual saved-card charge calculation instead of quoting a local plan delta as the payment amount.
>
> **Update Note (2026-04-02):** Immediate Iyzico plan upgrades should preserve the original billing anchor by sending `resetRecurrenceCount=false` together with `upgradePeriod=NOW` in the actual signed provider request body. The entitlement can expand right away, but the next full recurring charge should stay on the existing renewal date unless product policy intentionally resets it.
>
...
>
> **Update Note (2026-04-02):** Immediate upgrade confirmation must show the plan effect explicitly without overclaiming the provider charge: the higher entitlement applies immediately, the credit-delta unlocks today, the next renewal date stays unchanged, and the today-charge row must say that Iyzico calculates and collects the saved-card amount during the change.
>
...
>
> **Update Note (2026-04-02):** Direct-change confirmation should only visually prioritize an exact charged amount when that amount is provider-confirmed. Saved-card upgrade confirmations should keep a neutral apply CTA and avoid repeating local estimates such as `Pay ₺300`.
>
> **Update Note (2026-04-02):** The `today's charge` row in direct plan-change confirmation should use provider-calculated copy for upgrades unless an exact Iyzico order charge is already reconciled; do not render local plan-difference labels such as `₺300 plan farkı` as confirmed payments.
>
> **Update Note (2026-04-02):** The next-renewal, purchase-history, and billing-profile surface now belongs inside `Settings > Plans`, not `Settings > Billing`. Operators should see the next renewal date there, open a lightweight `Geçmiş / History` modal for successful purchases/renewals, and save a minimal billing profile whose company/address fields are reused to prefill Iyzico checkout payloads.
>
...
>
> **Update Note (2026-04-01):** `Settings > Organization` must show a localized non-blocking AI extraction loading state whenever Knowledge Base document-derived business setup is still running. If any organization Knowledge Base document is still `processing`, the page should immediately explain that the service profile, required customer fields, and service list are still being prepared from uploaded content, keep manual editing available, and auto-refresh once extraction completes.
>
> **Update Note (2026-04-01):** Trial-active workspaces must show a global dashboard banner with localized `Getting started` / `Başlangıç` and `Upgrade now` CTAs. The getting-started CTA disappears after onboarding completion, but the banner itself stays visible until the workspace upgrades or the trial window ends.
>
> **Update Note (2026-04-01):** Workspace onboarding is now an organization-scoped checklist with first-dashboard auto-open, a highlighted top purple `Getting started` / `Başlangıç` desktop-sidebar card above `Workspace`, mobile `Other` access while incomplete, a purple page-level progress bar, user-name greeting with a simple wave emoji and a smaller title size, a cleaner respond.io-style left-aligned accordion layout, outline/filled step-state icons instead of wrapped to-do circles, stacked single-column step bodies with compact filled icon rows, no intro chips, localized `Bilgi Bankası` / `Yetenekler` terminology in Turkish, business-focused intro copy that now also mentions appointment flow, `Yetenekler` copy that emphasizes low-credit deterministic replies defined by the operator, the same `Bilgi Bankası` / `Yetenekler` iconography already used in sidebar navigation, explicit business-review copy that says service profile / required info / service list are extracted automatically by AI from Knowledge Base documents but can still be entered or edited manually, a dedicated persisted `Qualy AI ayarlarını gözden geçirin` review step, a business-review CTA that opens the `Organization Details` tab directly, a final-step channel preview with direct larger icons and `Messenger` shown as coming soon, no duplicated ratio badge inside the sidebar card, a little extra breathing room below the trial banner, and hybrid completion derived from real setup state (`Knowledge Base` or at least one non-default custom skill, business profile, one manual AI-settings review acknowledgement, any ready live channel) plus one manual intro acknowledgement step while the next incomplete step stays expanded by default only on first entry and completed steps remain reopenable.
>
> **Update Note (2026-04-01):** Onboarding intro acknowledgement must not crash the UI when the onboarding-state relation is temporarily missing from the exposed schema. The CTA should open the next step immediately and let the shell refresh reconcile progress/sidebar/banner state once persistence succeeds.
>
> **Update Note (2026-04-01):** The first successful Knowledge Base document create should pause on a one-time educational modal instead of redirecting immediately. That modal must clearly say AI will automatically extract the business profile, required customer fields, and starter service-list structure from the uploaded content, while offering direct routes to `Başlangıç` and `Settings > Organization > Organization Details` without falsely completing the business-review onboarding step.
>
> **Update Note (2026-04-01):** Using the `Qualy AI ayarlarını gözden geçirin` CTA must complete that onboarding step immediately. The completed state should survive route changes/back navigation, and deployments missing the latest onboarding column should still preserve the step in the current browser via cookie fallback instead of silently reverting to incomplete.
>
> **Update Note (2026-04-01):** Newly created workspaces must default bot mode to `Kapalı` and keep it onboarding-locked until `Başlangıç` is fully complete (`5/5`). While locked, both the desktop sidebar quick switch and `Settings > Qualy AI > General` must render bot mode as disabled with onboarding-required helper copy, and runtime must still enforce effective `Kapalı` even if a stale client attempts `Dinleyici`/`Aktif`. Once onboarding reaches `5/5`, the shell must show a required completion modal that asks the operator to explicitly choose `Dinleyici`, `Aktif`, or keep the bot `Kapalı`; existing workspaces are not force-switched off.
>
> **Update Note (2026-04-01):** Bot-mode lock UX must key off real onboarding progress, not only the stored unlock flag. If onboarding is still incomplete (for example `4/5`), sidebar quick switch and `Settings > Qualy AI` must stay disabled, render the effective mode as `Kapalı`, and explicitly tell the operator that the control opens at `5/5`.
>
> **Update Note (2026-04-01):** Bot-mode lock copy should be rendered once as a bold colored onboarding-required banner in both sidebar quick switch and `Settings > Qualy AI > General`. While locked, option cards must stay visibly disabled with no hover affordance, and the sidebar popup must not repeat the same helper sentence in two places.
>
> **Update Note (2026-04-01):** The `Qualy AI ayarlarını gözden geçirin` onboarding step should not instruct operators to review bot mode before onboarding is complete. This step’s copy should instead focus on personalization and behavior settings that are still meaningful while bot mode remains locked.
>
...
> **Update Note (2026-04-10):** Same-cycle plan upgrades now use a fixed-difference billing model. Because Iyzico `upgradePeriod=NOW` charges the full target-plan amount, Qualy must collect only the fixed package-price difference through a hosted one-time checkout, unlock the target entitlement and only the credit delta immediately after that payment succeeds, and schedule the subscription’s target pricing plan with `upgradePeriod=NEXT_PERIOD` so the full new recurring amount is collected on the next renewal. Upgrade callback finalization must be recoverable and idempotent: if payment succeeds but local apply hits a transient failure, retries should reuse the stored provider schedule response and complete the local entitlement apply without double-scheduling or showing a false success banner.
> **Update Note (2026-04-02):** Live Iyzico billing now includes self-serve `past_due` recovery. `Settings > Plans` exposes `Update payment method` and `Retry payment`, card updates run through a hosted Iyzico card-update form embedded in-app, failure metadata stays persisted on the active subscription row for retry, and access remains locked until a subsequent provider success webhook restores the subscription to `premium_active`.
...
> **Last Updated:** 2026-06-01 (Public Demo Chat scope-help meta turns now answer deterministically before RAG/shared AI recovery and existing pending polls can recover from repeated `202` status; focused RAG fast paths skip LLM query-planner work once the route has already built a deterministic context-expanded query, lecture/course-material access retrieval prefers selective required-term evidence before broad OR scans and scopes those probes to named academic units when present, health-report/mazeret-exam retrieval skips broad health-policy OR scans when required remedy evidence grounds the answer, follow-up retrieval tries context-expanded focused evidence before broad search and deterministic repair uses that expanded question for evidence extraction, demo bot replies persist `rag_diagnostics` timing/search-strategy metadata, deterministic micro-polish is limited to source-safe presentation cleanup such as title-supported course-code expansion, source-link ranking recognizes Turkish word-number durations for compound-answer citation coverage, fast RAG may return deterministic source-backed repairs without settings/generation/polish work, grounded RAG generation remains quote-validated, generated replies pass through deterministic intent repair before source-linking, grouped policy-duration evidence must remain grouped, focused contact answers and source links avoid generic/footer phone conflation, engagement tails are rejected unless specific, source-grounded adjacent-detail offers, shared-predicate compound questions expand into per-subject retrieval queries, and campus answer repair completes omitted requested subjects when evidence exists.)
...
- Org-level AI settings control customer-facing reply behavior through a single sensitivity threshold plus structured AI assistant instructions
...
- When disclaimer is enabled, outbound bot replies append a disclaimer footer after one empty line. WhatsApp, Instagram, and Telegram use the same customer-facing footer shape: `\n\n> ...`.
- Runtime owns the disclaimer footer: before appending it, outbound formatting strips trailing model-supplied/default/custom disclaimer artifacts so LLM output cannot duplicate the footer. When the disclaimer toggle is off, the same stripping still prevents a model-generated disclaimer from leaking to customers.
- Runtime formats the final customer-facing bot message for the target messenger before send/persist: WhatsApp receives single-star emphasis instead of `**...**`, Instagram receives plain text without unsupported Markdown markers while preserving the standardized quoted disclaimer footer, Telegram defaults to plain text when no parse mode is configured, full raw URLs are preserved, malformed Markdown links with spaced URLs are converted into visible raw URLs, accidental spaces inside raw URL domains are removed, and inline dash-separated RAG list fragments become readable bullet lines.
...
3. Apply threshold plus a general sector-independent intent gate: ambiguous matches must have strong semantic confidence, a clear candidate margin, or fuzzy non-generic trigger/title anchoring with the customer message
4. If no skill candidate passes the gate → fallback to KB/RAG or topic-guided fallback response
...
- Large uploaded/crawled corpora use token-bound lexical matching plus focused multi-keyword fallback so exact PDF/contact-table evidence can outrank broad semantic neighbors; narrow policy-intent rerankers may demote schedule/announcement notices when the user asks an eligibility/rule question.
...
- Default guardrail skills are seeded automatically when an organization receives its owner membership.
- Provisioning uses signup locale when available and falls back to billing-region language (`TR -> tr`, `INTL -> en`) when locale metadata is unavailable.
- Older organizations that predate create-time provisioning are backfilled by migration so they receive the same default pack without waiting for a `Skills` page visit.
...
- Availability windows ending at `24:00` must be treated consistently across generated suggestions and direct booking writes, so an appointment ending exactly at local midnight remains inside the starting day's rule.
...
- Booking capacity protection is enforced both in application logic and at the database layer through an atomic guard over buffered occupied ranges.
- Capacity-aware booking keeps the model simple: each organization has one `max_concurrent_bookings` setting that controls how many pending/confirmed internal bookings may overlap at the same time.
- New organizations must default `max_concurrent_bookings` to `1`, preserving current no-overlap behavior until the operator explicitly raises the limit.
- Capacity must be evaluated as peak point-in-time occupancy over buffered booking ranges, not the total number of bookings that touch the requested interval.
- Before/after buffers expand both existing booking ranges and the candidate booking range, so reverse-order creation cannot bypass cleanup/prep time.
- Nearest AI availability suggestions are generated from the configured slot grid and working-hours windows, not from the arbitrary minute when the customer asked.
- Staff, room, equipment, or per-service resource-group scheduling is intentionally deferred; this iteration should not add a staff dispatch board or a second resource-management IA.
...
- Pending WhatsApp and Instagram states must explain the operator action explicitly instead of showing only a vague waiting badge: after connection, the operator should send a test message to the connected account, and Qualy auto-verifies the channel on the first inbound webhook delivery.
- Pending WhatsApp and Instagram setup surfaces should opportunistically refresh on browser `focus` / `visibilitychange` so verification state can update quickly after the operator returns from sending that test message. The setup page should show the connected account explicitly, keep the lead paragraph short, and repeat the exact same pending action copy used on channel cards only inside a full-width warning helper with an explicit `Aksiyon gerekli / Action required` title.
...
- Structured `Yapay zeka asistan talimatları` replace the raw prompt textarea with four optional fields: `Asistanın görevi`, `Eksik bilgi toplama kuralı`, `Asla yapma`, and `Diğer talimatlar`
...
- AI Settings information architecture is grouped into three tabs:
  ...
  - `Behavior and Logic`: sensitivity, structured AI assistant instructions
...
- Structured assistant instructions are locale-aware by default, preserve older custom prompts in `Diğer talimatlar`, and include a mobile-friendly `Nasıl çalışır?` explainer modal
- Structured assistant instructions repair exact saved EN/TR starter sets into the active UI locale while preserving custom operator-written instructions
- Knowledge Base freeform authoring now includes a persistent AI drafting helper card above the title field with the filled AI icon, stacked CTA, and a reusable mobile-friendly modal; draft generation shows an explicit loading state before the modal closes, repeated runs replace only the content body, leave non-empty titles untouched, use `Yapay zeka asistanının dikkat etmesi gerekenler` for the assistant-guidance field, and never auto-save the draft
...
- Credit history defaults to current-month ledger rows, can switch to previous month or all time, and lazy-loads more rows instead of expanding a fixed small slice.
- Credit history includes package grants and extra-credit purchases in the same feed as usage debits, so package/top-up loads remain visible even when many AI usage rows exist.
- Credit history supports record-level plus daily/weekly/monthly aggregate views over the loaded rows, with compact type/pool/reason labels and detailed package/top-up reason text preserved on hover/title.
...
- **Plan Purchase (Online Payment):**
  ...
  - Active premium tier upgrades now collect only the fixed plan-price difference through a hosted Iyzico one-time checkout, unlock the target entitlement immediately without stacking old+new monthly credits, add only the credit delta to ledger history, and then schedule the target recurring pricing plan through the documented subscription upgrade REST API with `upgradePeriod=NEXT_PERIOD` so the next renewal charges the full new package amount.
  ...
  - Existing-subscriber direct plan changes are lighter than first purchase checkout: the confirmation modal shows current plan, target plan, effective timing, and charge messaging, keeps legal documents as low-emphasis references, and requires only one explicit plan-change approval when the original legal acceptance remains valid.
  ...
  - `past_due` recovery is self-serve in `Settings > Plans`: users can update the saved card through Iyzico hosted card-update checkout and then request a retry of the failed renewal. Card update alone does not reactivate access; local entitlement unlocks only after a provider success webhook.
...
- **Membership Lifecycle (Trial / Premium):**
  ...
  - Trial-active workspaces show a global dashboard banner with two actions: `Onboarding checklist` and `Upgrade now`.
  - The banner stays visible only while the membership state is still `trial_active`; upgrading or crossing out of the active trial window removes the banner entirely.
  - If onboarding finishes before upgrade, the banner remains but hides the checklist CTA so only the upgrade path stays visible during the rest of the trial.
  - Workspace onboarding itself is organization-scoped, not user-scoped. Completion state belongs to the workspace setup, not an individual seat.
  - Incomplete onboarding remains reachable from desktop sidebar and mobile `Other` navigation even after upgrade; completed onboarding should disappear from those navigation entry points.
  - The onboarding page itself should stay wrapper-light and left-aligned like respond.io: greeting plus progress strip at the top, no extra page hero card, localized Turkish product terms such as `Bilgi Bankası` / `Yetenekler`, and a final-step channel preview that shows `Messenger` as coming soon instead of assuming WhatsApp-only setup.
  - When the final onboarding step depends on a pending Meta channel, the UI must explain that the operator should send a test message to the connected account and that Qualy verifies the channel automatically after the first inbound message.
  - Completing Meta channel verification must never auto-switch bot mode. After verification, Qualy should still require one explicit operator choice between `Dinleyici`, `Aktif`, or `Kapalı`, and the choice modal should present those modes neutrally without recommending one by default.
...
- **Trial Policy Decision (Required Gate):**
  ...
  - Upgrade UX now starts from a static trial banner in the dashboard shell plus deep links from the onboarding checklist into `Settings > Plans`.
...
> Finalized: 2026-01-31 (updated with implementation decisions through 2026-06-01)

- **University Demo RAG Evidence Grounding (Update 2026-05-25):** Customer-facing university demo answers may use focused evidence retrieval and deterministic repair before broad vector fallback when the user's intent is known to require exact evidence, such as campus addresses, program contacts, TLT double-major facts, medicine staj/final/exam policies, lecture-note guidance, or policy durations. Crawler-imported PDF URLs are normalized as PDF documents, canonical source links replace malformed model-emitted URL fragments, and release readiness for the Yüksek İhtisas screenshot set requires both an M1 crawl retrieval comparison and a live inbound-pipeline QA pass.
- **Focused RAG Latency Budget (Update 2026-06-01):** A deterministic focused evidence pass should stay planner-free when the caller already supplies an expanded contextual query; LLM query planning remains available in broad fallback where its semantic variants justify the latency. Focused evidence helpers should prefer high-signal required-term probes, scope them to named academic units when the user provides one, and skip broad OR scans when those probes already ground the answer, so latency improvements stay general retrieval behavior rather than organization- or phrase-specific shortcuts.
- **Demo Chat RAG Diagnostics and Source-Safe Micro-Polish (Update 2026-05-29):** Public Demo Chat follow-up retrieval should try context-expanded focused evidence before broad contextual search and persist `rag_diagnostics` metadata on direct RAG bot replies so production smoke tests can compare latency by strategy. Deterministic micro-polish may improve presentation only when the supporting retrieved source text/title contains the needed evidence, such as expanding a course-code acronym from document-title initials; it cannot add unsupported facts, engagement prompts, or role-specific assumptions.
- **Compound RAG Source Coverage (Update 2026-05-29):** Source-link ranking should keep separate citations for separate supported facts in a compound answer. Concrete value detection covers both numeric durations (`20 iş günü`) and Turkish word-number durations (`altı yıldır`) so a document that supports a worded duration is not filtered out just because another clause contains a digit.
- **Knowledge Chunk Evidence Enrichment (Update 2026-05-27):** Chunk creation should expose retrieval-ready evidence rows as a general ingestion contract. PDF/manual document reprocessing and crawler imports both preserve detected sections, split markdown table rows into standalone `table-row` chunks, split high-signal contact/address/course-code/value lines into `evidence-row` chunks, and prepend evidence metadata before embedding. Importers must use bounded chunk-insert batches and resilient bisection so large vector writes do not make corpus refreshes brittle; existing tenant corpora may be rebuilt through service-role reindex/import scripts.
- **Demo Chat Channel (Implementation 2026-05-20):** Public university demo links use a slug under `app.askqualy.com` instead of exposing organization ids. The slug maps server-side to a demo organization/channel, each tester session persists as a separate `demo_chat` Inbox conversation, AI replies use the shared inbound AI pipeline, and operator outbound replies are disabled until a later human-takeover iteration.
- **Demo Chat API Access (Security Update 2026-05-22):** Public demo pages may remain unauthenticated, but the API must not run service-role AI work from arbitrary callers. Each enabled demo channel requires `shared_secret_hash`; the server page mints a signed short-lived browser token from that server-only value, API `POST`/polling `GET` verify the token before pipeline/recovery work, and repeated AI work is rate-limited by channel, browser session, and client IP.
- **Demo Chat Page Caching (Security/UX Update 2026-05-27):** Public demo slug pages must force dynamic server rendering with zero revalidation because the signed API token is embedded into the browser client. Cached HTML with an expired token would make otherwise valid tester questions fail with `401 Demo access denied`.
- **Demo Chat Session UX (Update 2026-05-20):** Public Demo Chat keeps one browser-local session id and local message history per slug so refreshes continue the same conversation, while the reset action clears browser history and creates a new session/conversation for a fresh test. Demo Chat removes per-message bot disclaimer footers in favor of one composer-level demo disclosure, keeps input placeholders vertically centered, slows and expands loading copy, pulses the loading status dot, and preserves markdown emphasis/raw links for browser and operator rendering.
- **Demo Chat Composer UX (Update 2026-05-21, mobile zoom fix 2026-05-25):** Public Demo Chat uses an autosizing textarea rather than a fixed text input so longer demo questions can expand naturally on mobile. The composer keeps Enter-to-send, Shift+Enter newline behavior, hidden native scrollbars for capped long text, an iOS-safe 16px text baseline to prevent focus auto-zoom, and one bottom-right send action only, with no attachment button or instant-action dropdown in the public demo surface.
- **Demo Chat Pending Reply Contract (Update 2026-05-21):** Demo Chat `POST` requests wait only for a short synchronous reply window. If the shared inbound AI pipeline is still running, the route returns `202 pending` with the inbound demo message id, completes the pipeline after the response, stores outbound bot rows with `demo_chat_reply_to_message_id`, and exposes a polling `GET` route so the browser can attach the eventual answer without retrying the user message.
- **Demo Chat Reply Idempotency (Update 2026-05-21):** Demo Chat outbound metadata includes `demo_chat_reply_kind` so text and image parts of the same answer remain distinct while duplicate recovery runs for the same source message cannot create additional operator-visible bot bubbles. The Inbox thread view also filters older duplicate demo bot rows by source message id/kind instead of requiring destructive cleanup.
- **Next.js Security Baseline (Update 2026-05-22):** Keep `next` and `eslint-config-next` exact-pinned together on the patched 16.2.x line for App Router/proxy deployments. Until the framework no longer bundles an older PostCSS copy, force `postcss` through npm `overrides` to a patched 8.5.x release and verify with targeted tests plus `npm run build`.
- **Knowledge Search Fallback Fan-Out (Update 2026-05-22):** After bounded vector search, independent lexical fallback reads may run in parallel against the same request-scoped Supabase client and scoped filters. Ranking/deduplication remains centralized in `mergeSearchResults`, so the behavior changes latency shape without changing the returned evidence contract.
- **Admin Usage Metrics Aggregation (Update 2026-05-22):** Admin all-time/current-period AI token and credit totals should use an authenticated, organization-scoped Postgres aggregate RPC instead of transferring every `organization_ai_usage` row. Server row paging remains only as a fallback while migrations roll out.
- **Iyzico Card Update Callback Binding (Update 2026-05-22):** Hosted payment-method update callbacks are public provider return URLs, so metadata writes must be bound to the latest provider token stored when the flow starts. Callback payloads with missing/wrong tokens or mismatched conversation ids are treated as failed recovery attempts and cannot mutate subscription metadata.
- **Iyzico Direct REST Client (Security Update 2026-05-22):** The app should not carry the `iyzipay` SDK dependency chain because available SDK versions pull vulnerable HTTP request packages. Qualy signs the required Iyzico v1/v2 REST requests itself, uses built-in `fetch`, and keeps provider response normalization/error mapping in the existing billing client boundary.
- **WhatsApp Webhook Acknowledgement Boundary (Performance Update 2026-05-22):** Meta webhook POST handling should synchronously verify signature/channel state and then return `200`; inbound AI processing, outbound bot sends, and media downloads belong in a deferred `after()` task with logged failures so provider retry behavior is not tied to LLM/RAG latency.
- **Inbound Billing Renewal Gate (Performance Update 2026-05-22):** Manual-admin renewal RPCs are idempotent but should not repeat inside a single inbound AI request. The first entitlement gate may run renewal; later stage gates and deferred lead extraction/fallback calls pass `skipManualRenewal` while still rechecking current billing entitlement before token-consuming work.
- **Redirect Origin Trust Boundary (Update 2026-05-22):** Meta/Google OAuth and auth signout redirects must not accept arbitrary `x-forwarded-host` values as the destination origin. Forwarded origins are valid only when they match configured app/site origins or the request origin; otherwise code falls back to configured URLs or `req.nextUrl.origin`.
- **Instagram Skill Image Proxy Bounds (Update 2026-05-22):** The Instagram JPEG conversion proxy accepts only public Supabase `skill-images` bucket URLs, raster image MIME types, and source payloads at or below 5 MB before Sharp conversion. SVG/unknown types, prefix-trick bucket paths, and oversized responses fail before image processing.
- **Embedding Credit Costing (Implementation 2026-05-18):** Compute AI usage credits with category/model context. Chat completions keep the existing `input + 4×output` weighting, while `text-embedding-3-small` Knowledge Base indexing uses the embedding token price so indexing is not billed at chat-completion rates.
- **Skill Action Editor Rollback Boundary (Implementation 2026-04-05):** Keep `skill_actions` storage and runtime execution live, but hide the Skills-side action/button editor until dedicated editor regression coverage is restored. Temporary rollback should affect only operator editing affordances, not existing data persistence or inbound/outbound WhatsApp action handling.
- **Skill Image Delivery Contract (Implementation 2026-04-06, updated 2026-04-07):** Store one optional image metadata payload directly on `skills`, upload only through Qualy’s in-app flow, enforce `5 MB` max original size, normalize to flattened `JPEG` client-side before storage, derive the active organization server-side before any storage write/delete, use unique object keys per upload, and deliver matched skill replies as `text -> image -> optional human handover`. Image failures must not cancel the text reply, but they must remain visible to operators and in simulator/runtime parity. Older stored WebP assets may still use the Instagram JPEG proxy fallback until they are replaced.
- **Skill Image Suggestion Boundary (Implementation 2026-04-07):** Treat optional skill image add/replace/remove operations on existing Skills as media metadata changes only. Because the Skills editor may submit the unchanged title/response/triggers together with image metadata, server actions must compare against the stored Skill content before refreshing embeddings or creating offering-profile/service-list/required-field suggestions.
- **Trial Countdown Display Semantics (Implementation 2026-04-02):** Trial expiry enforcement still uses the exact `trial_ends_at` timestamp, but user-facing countdown copy must use calendar-day difference relative to the displayed end date so partial remaining hours do not overstate the number of days left.
...
- **Workspace Onboarding State Model (Implementation v1.60):** New-user setup is tracked per organization, not per user. Persist only the manual onboarding signals that cannot be derived (`first_seen_at`, intro acknowledgement), and compute the rest of the checklist from real setup state (`Knowledge Base`, offering profile/service catalog, WhatsApp readiness) so the checklist cannot drift away from actual product configuration.
- **Workspace Intro Surface Dismissal Model (Implementation v1.63):** First-visit educational modals for `/calendar` and `/simulator` are lighter than organization onboarding and should be dismissed per browser-user-workspace-surface via versioned local storage keys, not organization-global server state, so one operator closing the modal does not suppress it for everyone else.
- **Onboarding Shell Fetch Strategy (Implementation v1.60):** Banner, sidebar, mobile navigation, and the `/onboarding` page must consume one shared server-side onboarding snapshot instead of each surface re-querying completion independently. Use React `cache(...)` request dedupe plus explicit route refreshes on known mutation points instead of polling for onboarding completion.
- **New-Workspace Bot Mode Safety Gate (Implementation v1.61):** Newly created organizations default bot mode to `off` plus `bot_mode_unlock_required=true`. Existing organizations are backfilled unlocked. Until onboarding reaches `5/5`, runtime must still enforce effective `off`, and both sidebar quick-switch plus `Settings > Qualy AI > General` must remain read-only with onboarding-required guidance.
- **Onboarding Completion Unlock Contract (Implementation v1.61):** Finishing onboarding does not auto-activate the bot. When `5/5` is reached and bot-mode unlock is still required, the dashboard shell must show a required completion modal that explicitly writes one of three operator choices: `shadow`, `active`, or `off`. Choosing `off` still clears the lock so later bot-mode edits are allowed.
- **Meta Channel Verification UX Contract (Implementation 2026-04-07):** For WhatsApp and Instagram, `pending` means the channel is connected but Qualy has not yet seen the first inbound webhook delivery for that channel row. Operator-facing copy must explicitly instruct the user to send a test message to the connected account; Qualy then auto-verifies the channel after that first inbound message. Pending surfaces may refresh opportunistically on browser focus/visibility return, but verification remains event-driven rather than timer-polled.
- **Explicit Bot-Mode Choice Neutrality (Implementation 2026-04-07):** Meta channel verification and onboarding completion must not auto-switch the bot into `shadow`/`Dinleyici`, and the required completion modal must not visually recommend a default mode. Even when older workspaces are missing explicit unlock metadata, the effective mode stays `off` until the operator makes a deliberate choice.
- **Organization-Settings Extraction Feedback (Implementation v1.62):** Use `knowledge_documents.status = processing` as the contract for entry-time business-setup extraction feedback in `Settings > Organization`. Resolve the initial flag on the server for immediate paint, keep the banner non-blocking so operators can still edit manually, and let the client poll for completion before calling `router.refresh()` to pull the finished offering profile / service list / required-field output.
...
- **Calendar Booking Default Visibility (Implementation 2026-04-07):** Newly created workspaces default `booking_settings.booking_enabled` to `false` through both runtime fallbacks and database defaults. Existing workspaces keep their stored setting, while `/calendar` must expose the active/closed state in the header and block fresh manual booking creation when booking is closed.
- **Calendar Capacity Model (Implementation 2026-04-14):** Use one organization-level `booking_settings.max_concurrent_bookings` value instead of staff/resource groups. Default new/missing settings to `1`; allow overlapping internal appointments only while peak point-in-time pending/confirmed occupied ranges for the requested interval stay below that value. Before/after buffers expand both existing and candidate occupied ranges, Google busy overlay remains a hard block while enabled, nearest AI suggestions stay slot-grid aligned, and staff/resource-specific scheduling stays deferred.
...
- **Usage Ledger Explorer (Implementation v1.68):** `Settings > Usage` credit history must expose both negative usage and positive credit loads. Operators can filter all movements, usage debits, or loads, and aggregate views must load complete local periods rather than raw row pages: daily loads 10-day windows, weekly loads 3-week windows, and monthly loads 3-month windows.
...
- **Settings Missing-Auth Redirect Guard (Implementation v1.59):** `/settings/*` must fail safe after external redirects such as Iyzico checkout returns. If active organization context cannot be resolved for the request, the shared settings layout should redirect to localized login instead of returning `null` and painting a blank white page.
- **Local Billing Callback Origin Guard (Implementation v1.60):** During localhost development, billing callback URLs must prefer the active request origin over configured production app URLs. Iyzico checkout return flows should land back on the running local app so callback finalization updates the local database instead of redirecting the browser to production.
- **Subscription Action Hierarchy Guard (Implementation v1.61):** `Settings > Plans` should keep a single dominant subscription-management CTA. Cancellation belongs as low-emphasis helper text inside the manage-plan modal, while payment-recovery actions should render as lighter text links so they do not compete visually with the primary upgrade/switch action.
- **Billing Price Transparency Guard (Implementation v1.62):** All in-app `Settings > Plans` price surfaces must explicitly say that displayed subscription and one-time extra-credit amounts include VAT/KDV, then immediately show the conversation-range variability disclaimer underneath. This applies to unsubscribed package cards, active-subscriber package management UI, and extra-credit purchase surfaces.
- **Top-up Hosted Checkout Consistency Guard (Implementation v1.63):** One-time extra-credit purchases must initialize the same in-app hosted Iyzico embed pattern as subscription checkout. `Settings > Plans` top-up checkout should persist `checkout_form_content`, redirect to a local embedded checkout route, and avoid sending operators to a separate full-page provider URL that can disrupt auth/session continuity.
- **Billing Renewal Label Guard (Implementation v1.64):** The active monthly-package status card in `Settings > Plans` should label the next cycle with renewal language (`Yenileme / Renews at`) rather than reset language, so the UI matches subscription/billing terminology.
...
- **Pending-Route Chunk Retry Targeting (Implementation v1.65):** When a genuine Next chunk failure happens during dashboard navigation, recovery must retry the pending destination route rather than reloading the currently visible page. Dashboard transition intent is the source of truth for retry targeting so sidebar/mobile/settings navigations do not appear to "bounce back" to the same screen after a stale chunk.
...
- **Dashboard Route Timing Harness (Implementation v1.66):** Dashboard performance work should start from opt-in server timing labels before larger refactors. `DASHBOARD_PERF_DEBUG=1` enables route/action timing logs around dashboard org context, shell messages, billing/onboarding reads, AI settings reads, Settings route messages, Calendar page settings/data reads, and Calendar page-data server actions without changing default production logging.
- **Dashboard Shell Data Loader (Implementation v1.66):** The root dashboard layout should consume one request-scoped `getDashboardShellData` loader instead of coordinating org context, shell messages, onboarding, billing, and AI settings directly. Onboarding shell state should return the billing snapshot it already reads so the trial banner path does not issue a second billing snapshot read.
- **Settings Route Message Boundaries (Implementation v1.66):** The shared Settings layout should provide only navigation-level client messages (`Sidebar`). Page-specific Settings namespaces such as `calendar`, `organizationSettings`, `billingPlans`, `billingUsage`, `Channels`, and `aiQaLab` should be provided at the page boundary so opening `Settings > Calendar` does not serialize unrelated settings copy to the client.
...
- **Tenant Billing IA Split (Implementation v1):** `Settings > Plans` is the action surface for subscription/top-up/trial conversion status, next renewal visibility, purchase history, and billing-profile management, while `Settings > Billing` remains the detailed usage + receipts/ledger surface.
- **Tenant Billing IA Clarification (Implementation v1.1):** `Settings > Billing` stays focused on account snapshot context + immutable ledger + usage analytics and avoids duplicate purchase widgets; plan, renewal, purchase-history, and billing-profile controls live in `Settings > Plans`.
- **Tenant Billing Information Surface (Implementation v1.3):** `Settings > Plans` now also shows the next renewal date, a lightweight successful-purchase `History` modal, and a saved billing-profile form used to prefill Iyzico checkout customer/address fields. Subscription changes and payment actions remain on the same Plans surface.
...
- **Iyzico Plan-Change Popup Guard (Implementation v1.9+):** Active premium plan-change confirmation popups must reflect the real execution path. Hosted subscription/top-up flows can promise the next Iyzico payment screen, but direct premium upgrade/downgrade actions must instead show current vs target monthly pricing, effect timing, and the chosen in-product billing contract for that path. The current immediate-upgrade path applies the target entitlement immediately, unlocks only the credit delta, preserves the renewal anchor, and treats today’s saved-card charge as provider-calculated unless an exact Iyzico order amount is reconciled.
- **Iyzico Past-Due Recovery Strategy (Implementation v1.10):** `Settings > Plans` must expose `Update payment method` and `Retry payment` for `past_due` premium organizations. Card updates run through Iyzico hosted subscription card-update checkout embedded in-app, retry uses the stored failed `orderReferenceCode`, and the workspace remains locked until a later provider success webhook transitions local state back to `premium_active`.
...
- **Credit Legal Characterization (Implementation v1.62):** Credits are technical usage units only, not money, stored value, electronic money, or independently redeemable digital assets.
- **Monthly Package Credit Legal Rule (Implementation v1.62):** Monthly package credits are non-transferable, have no cash value, may be used only while the paid subscription remains active, and expire at the end of the relevant billing period without rollover.
...
- **Admin Manual Premium Credit Consistency (Implementation v1.81):** Admin-assigned premium packages must persist a positive monthly package credit grant, reset package usage, add an audited package-grant ledger row, and render remaining package/trial credits as the primary admin detail values while keeping used/limit as secondary context.
...
- **Plans Status Banner URL Cleanup (Implementation v1.65):** `Settings > Plans` checkout, direct plan-change, renewal, and payment-recovery feedback must remove transient callback query params with browser history replacement rather than App Router navigation. The banner remains mounted in local client state until dismissed so success feedback does not disappear while the page refreshes into the subscriber plan layout.
...
- **Extra Credit Legal Rule (Implementation v1.62):** One-time extra credits are non-transferable, have no cash value, may be used only while the paid subscription remains active, do not auto-renew, and persist across billing periods until consumed unless a narrower campaign rule is explicitly stated.
...
- **Billing Ledger Table Layout Stability (Implementation v1.9+):** `Settings > Usage` credit ledger uses fixed column sizing and separate column sets for record vs aggregate views. Record view shows `Date / Movement / Delta / Balance / Detail`; aggregate view shows `Period / Usage / Added / Net / Balance / Movements`, avoiding implementation-only pool/reason columns and avoiding truncation where wrapping is more readable. Ledger filters should stay compact: `Period`, `Movement`, and `View` sit side by side on desktop with labels above each control and wrap on narrow screens.
...
- **Iyzico Checkout Finalization Strategy (Implementation v1):** Initial subscription and top-up purchases finalize from checkout callback token retrieval (`retrieve` APIs) and apply idempotent ledger/account updates server-side before user redirect; recurring subscription lifecycle then continues through provider webhook sync for renewal success/failure/cancellation.
  ...
  - **Iyzico Premium Upgrade Strategy (Implementation v1.67):** Existing `premium_active` subscriptions no longer use provider `upgradePeriod=NOW` for same-cycle charging because that flow charges the full target-plan amount. Qualy upgrades now collect the fixed plan-price difference through a hosted one-time checkout, apply the target entitlement immediately after that payment succeeds, preserve current-cycle usage, add only the credit delta to the package ledger, and schedule the subscription’s target pricing plan with `upgradePeriod=NEXT_PERIOD` for the next renewal.
  - **Iyzico Upgrade Checkout Traceability (Implementation v1.67):** Fixed-difference upgrade payments must persist merchant `conversationId`, checkout order id, payment id, payment conversation id, stored provider schedule response, and target-plan metadata across the order row, subscription row, and package-grant ledger so `Settings > Plans > Geçmiş / History` can show the actual collected upgrade difference and later support recovery/debugging if payment succeeds but provider scheduling or local entitlement apply fails.
  - **Iyzico Upgrade Collected Amount Reconciliation (Implementation v1.66):** When a matched subscription order includes a successful `paymentAttempts[].paymentId`, direct saved-card upgrade completion and webhook settlement should query Iyzico payment detail and prefer the returned `paidPrice` over subscription-order `price` for `charged_amount_try`. Store the order reference plus payment id/conversation id in ledger metadata so purchase history reflects the actual collected transaction amount while remaining provider-evidenced.
...
- **KB Suggestion Progress Banner:** While any organization Knowledge Base document is still `processing`, `Knowledge` must show a visible wait-state suggestions helper; once processing ends and pending suggestions exist, it switches to the review CTA state.
...
- **Offering Profile Suggestion Reliability:** Automatic/manual regeneration must include actual ready Knowledge Base content plus Skill trigger/response detail, retry one stricter repair pass when the first repair is still too terse, and fail loudly on suggestion insert errors so pending-suggestion indicators do not disappear silently.
...
- **AI Settings Simplification:** Always-on flexible mode with a single match threshold (Skill + KB) and structured AI assistant instructions for customer-facing replies.
- **Assistant Instruction Starter Locale Repair:** Exact saved starter instruction sets are not treated as custom copy. Resolve known EN/TR default sets into the active UI locale at read time, but preserve any partial or custom operator-written instruction fields.
...
- **Bot Disclaimer (Implementation v1.20, updated 2026-05-15):** Store org-level `bot_disclaimer_enabled` + localized `bot_disclaimer_message_tr/en` in AI settings; when enabled, append a disclaimer footer to outbound bot replies using `\n\n> ...` for WhatsApp, Instagram, and Telegram.
...
- **Skill Matching Intent Gate:** Before deterministic skill replies run, ambiguous matches must pass a general semantic-confidence + fuzzy trigger-anchor gate. This is not tied to any single skill or sector; rejected candidates continue via KB/RAG/fallback.
...
- **Skill Image Delivery (Implementation 2026-04-06, updated 2026-04-07):** Each skill may include one optional uploaded image. Enforce a `5 MB` original-file limit, normalize to flattened `JPEG` on the client before upload, scope storage upload/delete to the active organization server-side, generate unique upload object keys, and send matched skill replies as two sequential outbound turns: text first, then image. The Skills editor must show a visible loading/disabled state while the asset is processing or uploading, and Simulator should mirror the same order. Older stored WebP assets may still use the Instagram JPEG proxy fallback until they are replaced.
...
- **Skills Read-Path Budget (Implementation v1.46.4):** Normal Skills navigation must stay focused on existing-row reads and skip embedding-maintenance scans on the request path. Localized default guardrail skills are provisioned at owner-membership time and backfilled for older organizations; the empty-workspace read-path seed remains only as a last-resort recovery path.
...
- **Default Guardrail Provisioning:** Seed localized default guardrail skills when an organization receives its owner membership, using signup locale when present and billing-region fallback otherwise; also backfill older organizations that predate create-time provisioning. Manage those defaults in the same list as user-created skills.
...
- **Skill Matching Intent Gate (Implementation 2026-04-07):** Generalize false-positive protection beyond handover skills. All semantic skill candidates now pass through a sector-independent gate that allows strong semantic matches, clearly separated top candidates, or fuzzy non-generic trigger/title anchors, and rejects ambiguous overlaps such as generic service-information turns matching unrelated deterministic skills.
...
- **Assistant Instruction Locale Repair:** When structured instruction fields are untouched, resolve locale-aware starter instruction text; when an older custom prompt exists, carry it into `Diğer talimatlar` without changing internal extraction/router/summary prompts.
...
- **Launch Copy Positioning (Implementation v1.34, updated 2026-04-12):** For launch and marketing assets, position Qualy around `Qualy AI` managing an AI-powered unified inbox, conversation clarity, person extraction, operator control, and monthly software subscription with included usage allowance. Visual prompts must keep a visible Qualy logo/wordmark, represent the AI actor as `Qualy AI` rather than a bare `Assistant`/`asistan`, and when channel coverage is shown use only `WhatsApp`, `Instagram`, `Telegram`, and `Messenger` unless a later product decision changes the approved set. Avoid `credit/token/coin` sales framing on public pricing assets, avoid English `lead`/`qualification` terminology, and avoid team-based primary framing in Turkish copy.
...
- **App Entry Redirect Strategy (2026-04-30):** Keep the root locale entry route lightweight. It may use cookie/auth and slim active-organization context to choose the workspace destination, but it must not block on onboarding shell reads; onboarding status is advisory chrome owned by the dashboard shell and `/onboarding` page.
...
| RAG Completion Model Policy | **Default `gpt-4o-mini`, opt-in `OPENAI_RAG_MODEL` override** | Grounding quality should come first from retrieval/repair/source validation. GPT-5-family RAG completion is supported with reasoning-compatible `max_completion_tokens` and minimal reasoning effort, but production should not blanket-upgrade to `gpt-4o` when the live challenge passes on the cheaper default. |
| RAG Query Planning | **Retrieval-only LLM planner in auto mode** | For noisy, abbreviated, policy-like, or conversational KB questions, the planner may create search variants and must-have evidence terms, but it must not generate answer facts; final answers still come only from retrieved chunks. Runtime searches the original query first, skips the planner entirely when that returns enough strong grounded evidence, fails planner calls fast after a default 2.5s budget (`KNOWLEDGE_QUERY_PLANNER_TIMEOUT_MS`), and can disable planning with `KNOWLEDGE_QUERY_PLANNER_ENABLED=false`. |
| RAG Focused Evidence Latency | **High-confidence evidence first, bounded evidence scans** | Focused retrieval should run precise current-listing/location evidence before broad address scans when the query asks where a named unit/program is located. Evidence scans have per-query timeouts, and acronym/expanded-name matching relies on subject coverage, initialism matching, and current evidence signals rather than customer-specific wording; extractive duration repair follows the same acronym matching before falling back to slower completion-model recovery, may compact dense course-table rows to the matching duration fact, and preserves all matching duration brackets when a broad policy question has no user-specified threshold. |
| RAG Source Evidence Filtering | **Source links must support concrete answer values** | Canonical source links should be selected from chunks that contain the concrete values shown in the answer, including phone/e-mail values, numeric or worded durations, percentages, and short platform names such as UZEM/MEDU/ÖBS. Broad topical matches may rank lower or be excluded when they omit the named concrete evidence, and persisted source metadata should follow the visible source URLs when those URLs exist in the final reply. |
| Public Demo Fast RAG Repair | **Deterministic grounded repair can bypass LLM authoring** | After retrieval and context anchoring, if deterministic repair can turn a no-information seed into a non-empty source-backed answer, Public Demo Chat returns that answer with canonical sources without loading organization AI settings, calling grounded generation, polishing, or metering LLM usage. Grounded generation/polish remains available when repair cannot safely answer. |
...
| Runtime Safety | **Fail-closed AI settings for webhooks** | Customer-facing webhook paths must resolve bot mode to `Kapalı` when AI settings cannot be loaded, preventing accidental replies in `Dinleyici`. |
| Dashboard Performance | **Server shell data seeds nav chrome** | Dashboard nav should reuse server-loaded billing, AI, and org context where possible, with client refresh reserved for explicit realtime/update signals and viewport-visible surfaces only. |
| Manual Billing Renewal | **Read-path idempotent renewal RPC** | Sales-led `manual_admin` premium packages renew before billing snapshot/entitlement reads, avoiding a cron dependency while keeping monthly package grants idempotent. |
| Usage Ledger Presentation | **Compact KB indexing usage and display** | Crawl imports should write one compact embedding usage row per completed import; older/import-batch audit rows can remain in the ledger, but customer history should collapse same-day `Bilgi Bankası indeksleme` rows for readability while preserving correction adjustments for auditability. |
| University Mevzuat RAG Retrieval | **Hybrid lexical safeguards for exact institutional evidence** | Large university PDF imports must augment vector search with exact document-code, acronym/initialism, title-phrase, and regulation/PDF source ranking; oversized top chunks should be trimmed into RAG context rather than skipped; RAG output repair may fill missing full document numbers or Senato meeting metadata only when the retrieved chunk already contains that evidence. |
| University Admissions GTM | **Pilot package over existing inbox scope** | Rector/IT-facing university sales should position Qualy as an admissions-season WhatsApp/Instagram message operations layer with IT-owned content control, not as a replacement for internally built web knowledge assistants. |
| Supabase Auth Callback | **Server-side code exchange from `/login?code=...`** | Email-confirmation links must create the Supabase SSR session before dashboard routing, then clear stale auth codes from the visible login URL on failures. |
| Post-Auth Redirect Safety | **Fail soft after successful auth** | Once Supabase creates a valid session, optional onboarding/workspace redirect lookups must log and fall back to `/inbox` instead of returning a login POST 500. |
| Supabase Client Calls | **Preserve method context** | Supabase methods that rely on client internals, especially `rpc`, must be called as `supabase.rpc(...)` rather than detached and invoked later. |
| Onboarding Shell Sync | **Client snapshot event for dashboard chrome** | The `/onboarding` page owns the freshest checklist read; it publishes that snapshot to persistent sidebar/mobile chrome so same-layout route transitions cannot leave progress stale. |
