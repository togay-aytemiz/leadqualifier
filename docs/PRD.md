...
> **Last Updated:** 2026-04-10 (`Settings > Plans` upgrades now charge only the fixed package-price difference through hosted checkout, unlock delta credits immediately, and schedule the new Iyzico recurring price for the next renewal instead of using provider `NOW` full-price charging.)
...
- **Launch Copy Positioning (Implementation v1.34):** For launch and marketing assets, position Qualy around one inbox, conversation clarity, person extraction, operator control, and monthly software subscription with included usage allowance; avoid `credit/token/coin` sales framing on public pricing assets, avoid English `lead`/`qualification` terminology, and avoid team-based primary framing in Turkish copy.
