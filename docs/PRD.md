...
> **Update Note (2026-04-03):** `Settings > Plans > Geçmiş / History` must show the real same-day upgrade true-up amount, not the full target monthly price, even when older upgrade ledger rows are missing `charged_amount_try`. When exact charge metadata is absent, the history layer should derive the upgrade delta from the prior package state when it can do so safely; repeated pre-effective downgrades should also replace the previously scheduled target plan cleanly instead of stacking stale pending metadata.
...
>
> **Update Note (2026-04-02):** The emphasized `today's charge` row in direct plan-change confirmation should use compact amount-first copy. Prefer short labels like `₺300 plan farkı` instead of longer full-sentence payment explanations inside the highlighted row.
>
...
> **Last Updated:** 2026-04-07 (Freeform Knowledge Base creation now includes a persistent `Qualy AI ile doldur` helper card with the filled AI icon, stacked CTA, reusable mobile-friendly drafting modal input, explicit in-modal loading state before close, content-body replacement on each re-run, title fill only when blank, updated `Yapay zeka asistanının dikkat etmesi gerekenler` wording, usage tracked inside the existing `İçerik işleme / Content processing` billing breakdown, Knowledge-driven offering-profile suggestions now use real Skill/KB body content plus a stricter second repair pass so pending suggestion banners stay reliable after AI-filled saves, the `Knowledge` page now keeps a visible preparation banner until it can switch into the ready-state review helper, Instagram bot replies now persist provider ids while new skill images are stored as JPEG, legacy WebP skill images are proxied as JPEG, provider-id/text/image echo duplicate rows are hidden in Inbox, existing Skill image-only edits no longer create Organization Detail AI suggestions, and Skill matching now uses a general semantic-confidence + fuzzy trigger-anchor gate before deterministic skill replies.)
...
- **Plan Purchase (Online Payment):**
  ...
  - Active premium tier upgrades now execute immediately via Iyzico subscription upgrade API, switch the current cycle entitlement to the target plan without stacking old+new monthly credits, preserve the existing recurring renewal anchor by sending `resetRecurrenceCount=false` with `upgradePeriod=NOW`, and expose the chosen operator summary as a full true-up model (full delta charge today, full delta credits today, next renewal date unchanged).
