...
> **Update Note (2026-05-28):** RAG answer quality should favor grounded naturalness over brittle deterministic prose. Source-quote-validated generated RAG answers can now run through the same bounded polish layer used by extractive answers when the answer is safe but lacks a grounded engagement offer; the polish layer must preserve every critical value, reject raw URLs, and require exact context evidence for follow-up suggestions. Demo polling recovery must cap sync waits below serverless platform timeout even if env configuration is too high, return pending on timeout, and avoid binding timed-out GET recovery to Next `after()` when the deploy platform can hold that response open; subsequent polls can retry recovery by message id. Source URL rendering must keep appended canonical URLs on standalone lines after e-mail/contact answers so the demo can label them as sources instead of exposing long raw links in the answer text.

> **Update Note (2026-05-28):** Knowledge Base RAG should use model reasoning and conversation context for answer wording without trusting unsupported output. Public Demo Chat polling now attempts the shared inbound AI pipeline before extractive fallback so demo users exercise the same history-aware behavior as production channels. Shared Knowledge Base retrieval passes recent conversation turns into the query planner, and grounded answer generation receives recent history only to resolve references while still requiring exact support quotes from retrieved context. The generator must apply organization AI voice instructions, reject missing support or invented critical values, and allow one role-neutral engagement question only when its evidence quote exists in the same context. Deterministic repair remains a fallback for bounded recovery, not the default answer author.
