...
- Important-info display sits inside its own compact wrapper section in lead details; the field list is fully left-aligned for width efficiency, the single `Edit` action stays aligned with the section header, and editing opens in one dedicated modal/sheet-style surface instead of inline row-level editing in the details panel.
...
- Tags and private note live as compact plain sections below lead extraction in Details; tag input stays hidden until the operator explicitly opens add mode, empty tag state stays silent (no `No tags / Etiket yok` copy), private-note stale protection is keyed to `private_note_updated_at` so unrelated conversation updates do not create false conflicts, and note metadata is shown as two compact lines (`who`, then `when`) without extra prefix labels.
...
> Finalized: 2026-01-31 (updated with implementation decisions through 2026-03-14)
...
- **Channels Discovery IA:** Render Settings > Channels as a responsive business-messaging gallery with richer channel descriptions, marketing/status badge chips, subdued card tones, and a direct grid layout capped at 3 desktop columns so non-technical users can scan options quickly.
...
- **Next.js Dev Stability:** Avoid exporting the locale entry redirect page component as `Home` under Next.js 16.1.6 Turbopack; a neutral component name prevents the dev-only `performance.measure(...): 'Home' cannot have a negative time stamp` overlay crash.
