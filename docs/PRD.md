...
> **Update Note (2026-03-27):** `Settings > Plans` extra-credit balance must derive its displayed total from the live topup pool, not only paid orders. Carried-over trial credits should therefore remain visible against their original pool size after premium activation, so remaining balance like `153.5` is shown against `200` rather than `153.5 / 153.5`.
...
- **Plans Extra-Credit Pool Total (Implementation v1.34):** The extra-credit balance card in `Settings > Plans` must derive its displayed total from the live topup pool (`current topup balance + already consumed topup usage`, including `mixed` debit rows via `metadata.topup_debit`) so carried-over trial credits remain visible against their original pool size after premium activation.
