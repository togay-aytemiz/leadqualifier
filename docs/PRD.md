...
> **Last Updated:** 2026-04-08 (`Settings > Plans` keeps billing success banners visible during silent callback-param cleanup, and direct/webhook Iyzico same-cycle upgrade history now reconciles collected payment-detail `paidPrice` before displaying provider-confirmed upgrade amounts.)
...
- **Plan Purchase (Online Payment):**
  ...
  - Active premium tier upgrades now execute immediately via the documented Iyzico subscription upgrade REST API, switch the current cycle entitlement to the target plan without stacking old+new monthly credits, preserve the existing recurring renewal anchor by sending `resetRecurrenceCount=false` with `upgradePeriod=NOW`, and expose the operator summary as provider-calculated for today’s saved-card charge while showing the credit delta and unchanged next renewal date.
...
- **Iyzico Checkout Finalization Strategy (Implementation v1):** Initial subscription and top-up purchases finalize from checkout callback token retrieval (`retrieve` APIs) and apply idempotent ledger/account updates server-side before user redirect; recurring subscription lifecycle then continues through provider webhook sync for renewal success/failure/cancellation.
  ...
  - **Iyzico Premium Upgrade Strategy (Implementation v1):** Existing `premium_active` subscriptions upgrade immediately through the documented Iyzico subscription upgrade REST API; current-cycle entitlement becomes the target-plan entitlement (`non-stacking`), package usage is preserved, and only the delta grant is added to ledger history.
  - **Iyzico Direct Plan-Change Traceability (Implementation v1.64):** Direct saved-card plan changes must send a merchant-controlled `conversationId`, persist that id in subscription and package-grant ledger metadata, and reconcile provider order charges from either direct `data.orders` or legacy `data.items[].orders` retrieve payloads. Local plan-price differences are estimates only and must not be shown as confirmed payments without an Iyzico order reference and amount.
