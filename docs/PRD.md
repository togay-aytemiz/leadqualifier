...
> **Last Updated:** 2026-04-15 (dashboard performance hardening now prevents current-route/click-time manual prefetch races that duplicated Calendar/Leads page chunks, adds shared client caches for hot route data plus Inbox current-user/profile hydration, removes duplicate Knowledge pending-suggestion entry reads, applies the cache model to Calendar window loading/prefetch and Leads page-data reuse, and keeps the existing Next App Router server-owned auth/billing/action boundaries; calendar capacity-aware booking also ships the simple org-level `max_concurrent_bookings` model.)
