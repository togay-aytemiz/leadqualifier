...
- Instagram connect-time identity discovery should merge Instagram Login profile metadata with real page discovery when Meta exposes it, so stored `page_id` matches webhook delivery identity instead of defaulting to the Instagram business account id.
- Instagram outbound manual replies should use Instagram Login graph host first (`https://graph.instagram.com/{IG_ID}/messages`) and only fall back to Facebook Graph host for compatibility.
...
> Finalized: 2026-01-31 (updated with implementation decisions through 2026-03-17)
...
- **Instagram MVP Channel Strategy:** Implemented via Meta Instagram Messaging API with OAuth-based channel setup (auto-resolved `page_id` + `instagram_business_account_id`), inbound text + image handling, webhook signature verification, and reactive replies only (first turn must come from customer inbound message).
- **Instagram Webhook Readiness Contract (Implementation v1.35):** Instagram channel records may exist before the callback is fully verified. Persist webhook provisioning metadata (`pending` / `verified` / `error`, verified-at timestamp, error detail) in channel config, treat UI/debug connection state as pending until verification succeeds, and record verification directly from `/api/webhooks/instagram` GET validation so operators can distinguish `connected` from `Inbox ready`.
- **Instagram Delivery Identity Contract (Implementation v1.35):** Prefer real `page_id` discovery when available, merge it with Instagram Login profile fields (`instagram_business_account_id`, `instagram_user_id`, `instagram_app_scoped_id`, username), and allow webhook ingest/reconciliation to match any of those ids. This prevents silent DM drops when Meta delivers the webhook under `entry.id = page_id`.
