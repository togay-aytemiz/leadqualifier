...
- Changed billing currency resolution to automatic request-region detection (`TR` => `TRY`, non-TR => `USD`) and removed manual billing-region selection UI from `Settings > Organization`; `Settings > Plans` now reads geo headers with `Accept-Language` fallback (`src/lib/billing/request-region.ts`, `src/lib/billing/request-region.test.ts`, `src/app/[locale]/(dashboard)/settings/plans/page.tsx`, `src/app/[locale]/(dashboard)/settings/organization/page.tsx`, `src/app/[locale]/(dashboard)/settings/organization/OrganizationSettingsClient.tsx`).
