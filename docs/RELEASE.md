...
- Added shared billing pricing-catalog resolver + tests to centralize plan/top-up credits, locale currency mapping (`tr` -> TRY, `en` -> USD), and safe conversation-range estimates (`src/lib/billing/pricing-catalog.ts`, `src/lib/billing/pricing-catalog.test.ts`).
...
- Changed `Settings > Plans` user-facing copy to remove technical/internal phrasing, localized TR package names (`Başlangıç/Büyüme/Kurumsal`), and added explicit current monthly package visibility in both status and package sections (`src/app/[locale]/(dashboard)/settings/plans/page.tsx`, `messages/tr.json`, `messages/en.json`).
...
- Changed tenant `Settings > Plans` to a full package-ladder + top-up-ladder experience: Starter/Growth/Scale cards now show locale-based prices, included credits, safe monthly conversation ranges, and one-time top-up pack comparisons (`src/app/[locale]/(dashboard)/settings/plans/page.tsx`, `src/app/[locale]/(dashboard)/settings/plans/TopupCheckoutCard.tsx`, `messages/en.json`, `messages/tr.json`).
