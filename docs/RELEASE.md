...
### Fixed
- Fixed system-admin organization picker/context fallback to exclude the internal `AI QA LAB` tenant (`AI QA LAB` / `ai-qa-lab`), preventing admin panel org selection from defaulting to or showing the QA-only workspace (`src/lib/organizations/active-context.ts`, `src/lib/organizations/active-context.test.ts`).
