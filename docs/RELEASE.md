...
- Changed QA Lab preset intensity to increase test depth: `Quick` is now `20 scenarios x up to 6 turns` and `Regression` is now `36 scenarios x up to 6 turns`, with generator constraints enforcing `min 3 / max 6 turns` and stateful turn continuity across each scenario (`src/lib/qa-lab/presets.ts`, `src/lib/qa-lab/presets.test.ts`, `src/lib/qa-lab/executor.ts`, `messages/en.json`, `messages/tr.json`, `docs/PRD.md`).
