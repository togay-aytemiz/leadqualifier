...
- Changed monetization requirements docs to lock stricter subscription-first policy: recurring monthly premium package is required after trial, top-up is blocked during trial, top-up is only enabled for active premium users after package-credit exhaustion, package price (`X TL`) + included credits (`Y`) are admin-configurable, monthly package credits are non-rollover, trial/package progress visibility is required via desktop sidebar + mobile More menu entry points, and system-admin manual billing operations (trial extension, credit adjustment, premium assignment) are now explicit requirements with audit logging (`docs/PRD.md`, `docs/ROADMAP.md`).
